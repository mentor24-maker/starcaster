...
function workInProgressFor(task, probe) {
  ...
  if (!task || !Array.isArray(task.tags)) {
    return { verdict: 'cannot-tell', work: [], unlooked: [], unseen: [{ machine: here, why: 'the ticket itself was not read, so which repo to look in is unknown — nothing was probed' }] };
  }
  const resolved = taskRepo.resolveTaskRepo(task.tags);
  if (resolved.action !== 'build' || !resolved.repo) {
    return { verdict: 'cannot-tell', work: [], unlooked: [], unseen: [{ machine: here, why: `the task's repo could not be resolved (${resolved.reason})` }] };
  }
  const home = taskRepo.repoHome(resolved.repo);
  if (!home) {
    return { verdict: 'cannot-tell', work: [], unlooked: [], unseen: [{ machine: here, why: `no checkout path is known for repo:${resolved.repo}` }] };
  }
