...
/** The first line with anything on it -- what the spec keys off. */
...
/**
 * Every fenced block in this message whose first line hands over a command CC
 * could have run. Returns [{ command, rule }].
 */
function offendingCommands(message) {
  const { blocks } = splitFences(message);
  const found = [];
  for (const block of blocks) {
    const line = stripPrompt(firstCommandLine(block));
    if (!line) continue;
