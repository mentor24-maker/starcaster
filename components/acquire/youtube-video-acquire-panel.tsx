...
export type MediaInfo = { job_id?: string; status?: string; error?: string } | null;

/**
 * What the media half of a finished acquire should show.
 *
 * Extracted from the submit handler on purpose. Slice 1 ships the server half
 * INERT — `POST /api/acquire/youtube-media` answers 503 naming Settings > APIs
 * when no worker is configured — and this panel's matching promise is that a
 * missing worker still leaves a complete acquire: details and transcript land,
 * and only the .mp4/.mp3 half reports itself unavailable.
 *
 * That promise is a branch inside a 60-line async callback, and the repo has
 * no React render tests to reach it there. As a pure function the real code is
 * callable, so the test exercises this rather than re-stating it.
 */
export function mediaOutcomeFor(mediaInfo: MediaInfo): MediaState {
  if (mediaInfo?.job_id) {
    return { status: safeText(mediaInfo.status) || 'queued', progress: 0, mp4: null, mp3: null, error: '' };
  }
  return {
    status: 'failed',
    progress: 0,
...
export function YoutubeVideoAcquirePanel(): React.ReactElement {
  ...
  const acquire = useCallback(async () => {
    ...
    try {
      ...
      // NOTE: this runs AFTER the dispatch above, and must. A dead worker may
      // never cost the details and transcript that already landed.
      const mediaInfo = (body.media || null) as MediaInfo;
      if (includeMedia) {
        setMedia(mediaOutcomeFor(mediaInfo));
        if (mediaInfo?.job_id) {
