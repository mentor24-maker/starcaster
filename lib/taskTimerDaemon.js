...
async function checkExpiredTimers() {
  try {
    ...
    for (const task of tasksRes.data) {
      ...
      // The Dev Agent worker this woke was retired 2026-08-17, so the trigger
      // is gone. Note what that leaves: this daemon still writes a
      // '[SYSTEM::PROCESSING]' chat row on every due timer, and nothing now
      // consumes those rows. It is very likely vestigial in full — but deciding
      // that is a separate question from retiring the Dev Agent, so it is
      // flagged here rather than removed in the same change.
      void processingRes;
    }
  } catch (error) {
