...
/**
 * A clone's name, inside the three words the messaging vocabulary allows.
 *
 * "Clone Tag" appends " Copy", which works until the tag already has three
 * words: `authoredText` drops the fourth, the clone comes out identical to its
 * original, and the button 409s every single time — a control that can only
 * fail (86bbu4gdu, round 2). It cannot be fixed by appending anything, because
 * a fourth word is exactly what gets dropped.
 *
 * So the counter goes INSIDE the three words: as its own word where there is
 * room ("Courts Copy" -> "Courts Copy 2"), and fused onto the last word where
 * there is not ("Center Court North" -> "Center Court North2"). Both shapes
 * survive `normalizeMessagingTag` unchanged, which is the property that makes
 * them safe to insert — a candidate the vocabulary would reshape could collide
 * again after being reshaped.
 *
 ...
 */
function uniqueAuthoredTag(base, takenKeys, limit = 50) {
  const authored = String(base || '');
  if (!authored) return '';
  if (!takenKeys.has(tagKey(authored))) return authored;
  const words = authored.split(' ');
  for (let n = 2; n <= limit; n += 1) {
    const candidate = words.length < 3
      ? `${authored} ${n}`
      : `${words.slice(0, 2).join(' ')} ${words[2]}${n}`;
...
async function createMessagingTag(input, scope = null) {
  ...
  if (Array.isArray(existing.data) && existing.data.length) {
    // `uniquify` is what "Clone Tag" asks for: number me rather than refuse me.
    // The Add form does NOT ask for it — there, a duplicate is the admin's
    // mistake and a sentence is the right answer. A clone has no mistake in it,
    // and for a three-word tag no name it could send would ever be free.
    if (!input?.uniquify) {
      return {
        ok: false,
        status: 409,
    ...
    if (!listed.ok) return listed;
    const taken = new Set(
      (Array.isArray(listed.data) ? listed.data : []).map((row) => tagKey(row.tag))
    );
    finalTag = uniqueAuthoredTag(tag, taken);
    if (!finalTag) {
      return {
        ok: false,
        status: 409,
