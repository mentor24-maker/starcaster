...
function createMessagingTextStore(options) {
  ...
  // Like clean(), but keeps author-entered line breaks: collapses runs of
  // spaces/tabs within a line, trims whitespace hugging each newline, and caps
  // consecutive blank lines at one. Used for multi-line body copy (e.g. post
  // text) where paragraphs must survive the round-trip and reach the publisher.
  function cleanMultiline(value, max = maxLength) {
    return String(value || '')
      .replace(/\r\n?/g, '\n')
      .replace(/[^\S\n]+/g, ' ')
      .replace(/ ?\n ?/g, '\n')
  ...
  const cleanField = multiline ? cleanMultiline : clean;

  function normalizeFieldValue(value) {
    return cleanField(normalize(cleanField(value, maxLength)) || '', maxLength);
  }
  ...
  function errorText(res) {
    return String(res?.error || res?.message || res?.raw?.message || '').toLowerCase();
  }
  ...
  function shouldRetryWithoutSplitCategoryColumn(res) {
    if (!splitTopicCategory) return false;
    const text = errorText(res);
    return text.includes('category') && (text.includes('column') || text.includes('schema cache'));
  }

  function withoutSplitCategory(row) {
    if (!splitTopicCategory) return row;
    const copy = { ...row };
  ...
  function rowToItem(row) {
    if (!row) return null;
    const topic = splitTopicCategory
      ? clean(row.topic, 240)
      : clean(row.topic != null ? row.topic : row.category, 240);
    const category = splitTopicCategory
      ? clean(row.category, 240)
      : topic;
    const item = {
    ...
    if (!leanSchema) {
      item.feedback = clean(row.feedback, 12000);
      item.prompt_id = Number(row.prompt_id || 0) || null;
      item.quality_score = Math.max(0, Math.min(Number(row.quality_score || 0) || 0, 5));
      item.campaign_id = row.campaign_id != null ? (Number(row.campaign_id) || null) : null;
    }
    extraFields.forEach((extra) => {
      const key = String(extra?.key || '').trim();
      if (!key) return;
      if (extra.type === 'number') {
        item[key] = Number(row[key] || 0) || null;
      } else {
        item[key] = clean(row[key], Number(extra.maxLength || 4000) || 4000);
      }
    });
    return item;
  }

  function shouldRetryWithoutCampaignId(res) {
    const text = errorText(res);
    return text.includes('campaign_id') && (text.includes('column') || text.includes('schema cache'));
  }

  function inputToRow(input) {
    const row = {
      [field]: normalizeFieldValue(input?.[field]),
      topic: splitTopicCategory ? clean(input?.topic, 240) : cleanTopic(input),
    };
    if (!leanSchema) {
      row.feedback = clean(input?.feedback, 12000);
      row.prompt_id = Number(input?.prompt_id || 0) || null;
      row.quality_score = Math.max(0, Math.min(Number(input?.quality_score || 0) || 0, 5));
    }
    if (splitTopicCategory) {
      row.category = clean(input?.category, 240);
    }
    extraFields.forEach((extra) => {
      const key = String(extra?.key || '').trim();
      if (!key) return;
      if (extra.type === 'number') {
        row[key] = Number(input?.[key] || 0) || null;
      } else {
        const normalized = typeof extra.normalize === 'function' ? extra.normalize(input?.[key]) : input?.[key];
        row[key] = clean(normalized, Number(extra.maxLength || 4000) || 4000);
      }
    });
    if (input?.campaign_id != null) {
      const cid = Number(input.campaign_id) || 0;
      if (cid) row.campaign_id = cid;
    }
    return row;
  }

  function shouldRetryWithoutExtraFields(res) {
    if (!extraFields.length) return false;
    const text = errorText(res);
    if (!text.includes('column') && !text.includes('schema cache')) return false;
    return extraFields.some((extra) => {
      const key = String(extra?.key || '').trim().toLowerCase();
      return key && text.includes(key);
    });
  }

  function withoutExtraFields(row) {
    const copy = { ...row };
    extraFields.forEach((extra) => {
      const key = String(extra?.key || '').trim();
  ...
  async function create(input, scope = null) {
    ...
    if (!res.ok && shouldRetryWithoutCampaignId(res)) {
      const noCampaignRow = { ...row };
      delete noCampaignRow.campaign_id;
      res = await sbQuery({
        method: 'POST',
        table: table(),
        query: 'select=*',
    ...
    if (!res.ok && shouldRetryWithoutExtraFields(res)) {
      res = await sbQuery({
        method: 'POST',
        table: table(),
        query: 'select=*',
        headers: { Prefer: 'return=representation' },
        body: [withoutExtraFields(row)],
      });
    }
    if (!res.ok && shouldRetryWithoutSplitCategoryColumn(res)) {
      res = await sbQuery({
        method: 'POST',
        table: table(),
        query: 'select=*',
        headers: { Prefer: 'return=representation' },
        body: [withoutSplitCategory(withoutExtraFields(row))],
      });
    }
    if (!res.ok && shouldRetryWithLegacyCategory(res)) {
      const legacyBody = withoutExtraFields({ ...row, category: cleanTopic(input) });
      delete legacyBody.topic;
