...
const REQUIRED_TOOLS = [
  ...
  },
  {
    ...
    versionArgs: ['-version'],
    brew: 'ffmpeg',
    why: 'The Studio reads what a video file is with it, and the node suite that gates every ship goes red without it.',
  },
  {
    id: 'claude',
    command: 'claude',
 ...
 *
 * `blocked` is the honest entry for a job THIS PROVISIONER CANNOT INSTALL, and
 * the reason is the whole of what the field carries. Usually the installer does
 * not exist yet; sometimes it exists somewhere this script cannot reach it (the
 * two pulse pipelines below). Reading the field as "no installer exists" is what
 * put a false sentence in doctor:node's report about two live, working jobs
 * (task 86bbw9nbj, round 3), so every reader states the meaning and prints this
 * reason for the why.
...
const JOB_SCHEDULES = {
  ...
  'studio-worker': {
    installer: 'scripts/install_studio_worker.sh',
    status: 'scripts/install_studio_worker.sh --status',
    label: 'com.starcaster.studio-worker',
  },
  'weekly-report': {
    installer: 'scripts/install_weekly_report.sh',
    status: 'scripts/install_weekly_report.sh --status',
    label: 'com.starcaster.weekly-report',
  },
  // THE TWO PULSE PIPELINES. Still blocked, and the REASON has changed — which
  // is the whole point of writing it out rather than leaving a ticket id
  // (task 86bbw9nbj, 2026-09-12).
  //
  // It used to read "pulse's bin/install-launchd.sh is NODES Slice B and is not
  // written yet". That installer now exists: pulse PR #4 generates each plist
  // from the machine it runs on, which is exactly what Slice B asked for. So the
  // old sentence would have sent a reader looking for a script that was already
  // there.
  //
  // What blocks these rows now is narrower and is about THIS file, not pulse:
  // `scripts/verify_node_roles.mjs` resolves `installer` against the starcaster
  // checkout (`path.join(MAIN_CHECKOUT, job.installer)`), so a path in another
  // repo is not something this provisioner can express, let alone run. Giving
  // these rows an installer they cannot reach would make the provisioner report
  // a green check for a schedule it never touched — the precise failure the
  // NODES plan was written against, arriving through the fix for it.
  //
  // NO `blockedBy`, deliberately. There is no ticket for cross-repo installers,
  // and inventing one to fill this field would be worse than leaving it empty:
  // `doctor_node.mjs` prints it as "Blocked by ticket X", and a reader who goes
  // looking for a ticket that says nothing about this loses more time than one
  // who reads the sentence above.
  'channel-steward': {
    blocked: 'The installer exists but lives in the pulse repo (bin/install-launchd.sh, which generates each '
      + 'plist from the machine it runs on). This provisioner only runs installers inside the starcaster '
      + 'checkout — scripts/verify_node_roles.mjs joins `installer` onto MAIN_CHECKOUT — so it cannot call it, '
      + 'and claiming it could would report a green check on a schedule nothing installed. Install it by hand '
      + 'from the pulse checkout, and `bin/install-launchd.sh --status` says whether it took.',
  },
  'librarian-sweep': {
    blocked: 'Same installer, same repo, same limitation as channel-steward above — pulse\'s '
      + 'bin/install-launchd.sh installs both pipelines, and this provisioner can only reach installers inside '
      + 'the starcaster checkout. Install it by hand from the pulse checkout; `bin/install-launchd.sh --status` '
      + 'says whether it took.',
  },
  'pipeline-pulse': {
    installer: 'scripts/install_pipeline_pulse.sh',
    status: 'scripts/install_pipeline_pulse.sh --status',
    label: 'com.starcaster.pipeline-pulse',
  },
  'youtube-media': {
    blocked: 'Installing it needs two things only the operator can produce — the shared secret the worker '
      + 'and StarCaster both hold, and the Settings > APIs record pointing at it — so the launchd job is '
      + 'slice 4 (86bbjve6q) by design, not an oversight. The numbered sequence is workers/youtube-media/README.md.',
    blockedBy: '86bbjve6q',
  },
};
