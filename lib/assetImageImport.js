'use strict';

const { inferAspectFromDimensions, normalizeAspect } = require('./assetAspect');
const { readImageDimensionsFromBuffer } = require('./assetImageDimensions');
const { generateThumbnailJpeg, DEFAULT_MAX_EDGE } = require('./assetThumbnail');
const { uploadAssetFile } = require('./assetStorage');
const { createAsset, updateAsset, rowToAsset } = require('./assetsStore');
const { ensureAssetRenditions } = require('./assetRenditions');

function cleanAssetName(name, fallback = 'image') {
  const base = String(name || fallback)
    .trim()
    .replace(/\.[^.]+$/, '')
    .replace(/[^a-zA-Z0-9._-]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, 120);
  return base || fallback;
}

function resolveDimensions(uploadData, buffer) {
  const fromUpload = {
    width: Math.max(0, Number(uploadData?.imageWidth || 0) || 0),
    height: Math.max(0, Number(uploadData?.imageHeight || 0) || 0),
  };
  if (fromUpload.width > 0 && fromUpload.height > 0) {
    return fromUpload;
  }
  return readImageDimensionsFromBuffer(buffer);
}

/**
 * Upload one image, assign aspect from full-size dimensions, generate and attach thumbnail.
 */
async function importImageAssetWithThumbnail(input, scope = null) {
  const fileName = String(input.fileName || '').trim();
  const mimeType = String(input.mimeType || 'image/jpeg').trim();
  const fileBase64 = String(input.fileBase64 || '').trim();
  const category = String(input.category || '').trim();
  const assetName = String(input.assetName || fileName || '').trim() || fileName;
  const tags = Array.isArray(input.tags) ? input.tags : [];
  const maxEdge = Math.max(64, Number(input.maxEdge || DEFAULT_MAX_EDGE) || DEFAULT_MAX_EDGE);
  const skipThumbnail = Boolean(input.skipThumbnail);

  if (!fileName || !fileBase64) {
    return { ok: false, status: 400, error: 'fileName and fileBase64 are required' };
  }

  let buffer;
  try {
    buffer = Buffer.from(fileBase64, 'base64');
  } catch {
    return { ok: false, status: 400, error: 'fileBase64 is not valid base64 content' };
  }

  if (!buffer.length) {
    return { ok: false, status: 400, error: 'Uploaded file is empty' };
  }

  const upload = await uploadAssetFile({
    assetType: 'Image',
    category,
    fileName,
    mimeType,
    fileBase64,
    makePublic: true,
  });
  if (!upload.ok) {
    return { ok: false, status: upload.status || 500, error: upload.error || 'Image upload failed' };
  }

  const { width: imageWidth, height: imageHeight } = resolveDimensions(upload.data, buffer);
  const aspectOverride = normalizeAspect(input.aspect);
  const aspect = aspectOverride || inferAspectFromDimensions(imageWidth, imageHeight);

  const save = await createAsset(
    {
      assetName,
      assetType: 'Image',
      category,
      location: String(upload.data.location || '').trim(),
      tags,
      size: Math.max(0, Number(upload.data.sizeBytes || buffer.length) || 0),
      imageWidth: imageWidth || null,
      imageHeight: imageHeight || null,
      aspect,
      source: input.source,
    },
    scope
  );
  if (!save.ok) {
    return { ok: false, status: save.status || 500, error: save.error || 'Failed to save asset record' };
  }

  const createdRow = Array.isArray(save.data) ? save.data[0] : save.data;
  const assetId = Number(createdRow?.id || 0) || 0;
  if (!assetId) {
    return { ok: false, status: 500, error: 'Asset record was not returned after create' };
  }

  let thumbnailGenerated = false;
  let thumbnailError = '';
  // Tracks the freshest row so both the thumbnail and the rendition pass can
  // contribute to what the caller gets back. This used to be two separate
  // returns, which is why a second post-create step had nowhere to land.
  let latestRow = createdRow;

  if (!skipThumbnail) {
    const thumb = await generateThumbnailJpeg(buffer, { maxEdge, mimeType });
    if (!thumb.ok) {
      thumbnailError = thumb.error || 'Thumbnail generation failed';
    } else {
      const thumbUpload = await uploadAssetFile({
        assetType: 'Image',
        category: 'Asset Thumbnail',
        fileName: `${cleanAssetName(assetName, `asset_${assetId}`)}_thumb.jpg`,
        mimeType: 'image/jpeg',
        fileBase64: thumb.data.buffer.toString('base64'),
        makePublic: true,
      });

      if (!thumbUpload.ok) {
        thumbnailError = thumbUpload.error || 'Thumbnail upload failed';
      } else {
        const thumbLocation = String(thumbUpload.data.location || '').trim();
        if (!thumbLocation) {
          thumbnailError = 'Thumbnail upload did not return a location';
        } else {
          const updated = await updateAsset(
            assetId,
            {
              thumbnailLocation: thumbLocation,
              thumbnailWidth: thumb.data.width || null,
              thumbnailHeight: thumb.data.height || null,
              thumbnailSize: thumb.data.buffer.length,
              thumbnailGeneratedAt: new Date().toISOString(),
            },
            scope
          );
          if (!updated.ok) {
            thumbnailError = updated.error || 'Failed to save thumbnail metadata';
          } else {
            thumbnailGenerated = true;
            const updatedRow = Array.isArray(updated.data) ? updated.data[0] : updated.data;
            if (updatedRow) latestRow = updatedRow;
          }
        }
      }
    }
  }

  // Oversized uploads get their scaled-down copies now, so a picture is never
  // live on a page at full weight. Failure here is reported, never fatal.
  const renditions = await ensureAssetRenditions(latestRow, buffer, scope);
  if (renditions.row) latestRow = renditions.row;

  return {
    ok: true,
    status: 201,
    data: {
      asset: rowToAsset(latestRow),
      aspect,
      thumbnailGenerated,
      thumbnailError: thumbnailError || undefined,
      renditionsGenerated: renditions.generated,
      renditionsError: renditions.error || undefined,
    },
  };
}

/**
 * Upload a new image file and replace an existing image asset (metadata + thumbnail).
 */
async function replaceImageAssetWithThumbnail(assetId, input, scope = null) {
  const id = Number(assetId || 0);
  if (!Number.isFinite(id) || id <= 0) {
    return { ok: false, status: 400, error: 'Valid asset id is required' };
  }

  const { getAssetById } = require('./assetsStore');
  const existing = await getAssetById(id, scope);
  if (!existing.ok) {
    return { ok: false, status: existing.status || 500, error: existing.error || 'Asset not found' };
  }
  const row = existing.data;
  const assetType = String(row?.asset_type || row?.assetType || '').trim();
  if (assetType !== 'Image') {
    return { ok: false, status: 400, error: 'Only image assets can be replaced with edited images' };
  }

  const fileName = String(input.fileName || '').trim();
  const mimeType = String(input.mimeType || 'image/jpeg').trim();
  const fileBase64 = String(input.fileBase64 || '').trim();
  const category = String(input.category || row?.category || '').trim();
  const assetName = String(input.assetName || row?.asset_name || fileName || '').trim() || fileName;
  const tags = Array.isArray(input.tags) ? input.tags : (Array.isArray(row?.tags) ? row.tags : []);
  const maxEdge = Math.max(64, Number(input.maxEdge || DEFAULT_MAX_EDGE) || DEFAULT_MAX_EDGE);
  const skipThumbnail = Boolean(input.skipThumbnail);

  if (!fileName || !fileBase64) {
    return { ok: false, status: 400, error: 'fileName and fileBase64 are required' };
  }

  let buffer;
  try {
    buffer = Buffer.from(fileBase64, 'base64');
  } catch {
    return { ok: false, status: 400, error: 'fileBase64 is not valid base64 content' };
  }

  if (!buffer.length) {
    return { ok: false, status: 400, error: 'Uploaded file is empty' };
  }

  const upload = await uploadAssetFile({
    assetType: 'Image',
    category,
    fileName,
    mimeType,
    fileBase64,
    makePublic: true,
  });
  if (!upload.ok) {
    return { ok: false, status: upload.status || 500, error: upload.error || 'Image upload failed' };
  }

  const { width: imageWidth, height: imageHeight } = resolveDimensions(upload.data, buffer);
  const aspectOverride = normalizeAspect(input.aspect);
  const aspect = aspectOverride || inferAspectFromDimensions(imageWidth, imageHeight);

  const patch = {
    location: String(upload.data.location || '').trim(),
    size: Math.max(0, Number(upload.data.sizeBytes || buffer.length) || 0),
    imageWidth: imageWidth || null,
    imageHeight: imageHeight || null,
    aspect,
    // The row now points at a different picture, so the old scaled copies show
    // the PREVIOUS image. Clear them in the same write that swaps the location —
    // leaving them for a moment would serve the old picture at every size but
    // full width. They are rebuilt below.
    renditions: [],
    renditionsGeneratedAt: null,
  };

  let thumbnailGenerated = false;
  let thumbnailError = '';

  if (!skipThumbnail) {
    const thumb = await generateThumbnailJpeg(buffer, { maxEdge, mimeType });
    if (!thumb.ok) {
      thumbnailError = thumb.error || 'Thumbnail generation failed';
    } else {
      const thumbUpload = await uploadAssetFile({
        assetType: 'Image',
        category: 'Asset Thumbnail',
        fileName: `${cleanAssetName(assetName, `asset_${id}`)}_thumb.jpg`,
        mimeType: 'image/jpeg',
        fileBase64: thumb.data.buffer.toString('base64'),
        makePublic: true,
      });

      if (!thumbUpload.ok) {
        thumbnailError = thumbUpload.error || 'Thumbnail upload failed';
      } else {
        const thumbLocation = String(thumbUpload.data.location || '').trim();
        if (!thumbLocation) {
          thumbnailError = 'Thumbnail upload did not return a location';
        } else {
          patch.thumbnailLocation = thumbLocation;
          patch.thumbnailWidth = thumb.data.width || null;
          patch.thumbnailHeight = thumb.data.height || null;
          patch.thumbnailSize = thumb.data.buffer.length;
          patch.thumbnailGeneratedAt = new Date().toISOString();
          thumbnailGenerated = true;
        }
      }
    }
  }

  const save = await updateAsset(id, patch, scope);
  if (!save.ok) {
    return { ok: false, status: save.status || 500, error: save.error || 'Failed to update asset record' };
  }

  let updatedRow = Array.isArray(save.data) ? save.data[0] : save.data;

  const renditions = await ensureAssetRenditions(updatedRow, buffer, scope);
  if (renditions.row) updatedRow = renditions.row;

  return {
    ok: true,
    status: 200,
    data: {
      asset: rowToAsset(updatedRow),
      aspect,
      thumbnailGenerated,
      thumbnailError: thumbnailError || undefined,
      renditionsGenerated: renditions.generated,
      renditionsError: renditions.error || undefined,
    },
  };
}

module.exports = {
  importImageAssetWithThumbnail,
  replaceImageAssetWithThumbnail,
};
