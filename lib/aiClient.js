 ...
 * API keys resolved from Settings > APIs (falling back to env vars) and token
 * usage reported to observeStore.logUsage().
 *
 ...
 *
 * Two known rough edges, both inherited from that origin and both worth knowing
 ...
 *    other modules hand-rolled their own fetch() instead of reusing this file.
 *  - Because of that, this is the ONLY LLM caller in the repo that reports token
 *    usage. Spend from the other call sites is invisible.
 *
 * `queryAgentPersona()` is the Dev Agent ("Ask Roger") persona wrapper and is
 * used only by routes/devAgent.js. Everything else should call the provider
 * helpers directly.
 */

const { getProviderValues } = require('./apiSettings');

const DEV_AGENT_SYSTEM_INSTRUCTION = `You are Roger Thorson, a Technical Consultant, Software Architect, and collaborative engineering partner specializing in AI integrations.
...
const DEV_AGENT_CODER_SYSTEM_INSTRUCTION = `You are Archie (formerly Antigravity), a powerful agentic AI coding assistant designed by the Google Deepmind team working on Advanced Agentic Coding.
...
// always used. Deliberately NOT back-filled with DEV_AGENT_SYSTEM_INSTRUCTION:
// that would change live behaviour while archiving, and queryAgentPersona() routes
...
const ARCHIVED_PERSONA_INSTRUCTION = '';
...
async function queryAgentPersona(messagesOrPrompt, extraOptions = {}) {
  let systemPrompt = DEV_AGENT_SYSTEM_INSTRUCTION;
  if (extraOptions.agentRole === 'angie' || extraOptions.agentRole === 'roger') {
    systemPrompt = ARCHIVED_PERSONA_INSTRUCTION;
  } else if (extraOptions.agentRole === 'antigravity') {
    systemPrompt = DEV_AGENT_SYSTEM_INSTRUCTION; // antigravity alias acts as Roger Thorson
  }
...
module.exports = { queryAgentPersona, queryGemini, queryAnthropic, queryOpenAI, DEV_AGENT_SYSTEM_INSTRUCTION, DEV_AGENT_CODER_SYSTEM_INSTRUCTION };
