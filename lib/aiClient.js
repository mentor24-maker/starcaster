 ...
 *  - **Nothing here reports token usage.** It used to, but only on the Dev
 *    Agent path: the observeStore.logUsage() call lived inside
 *    queryAgentPersona() and was removed with it. Every other AI call site in
 *    the repo already reported nothing, so no coverage was lost — but the
 *    honest position is that AI spend is now invisible repo-wide. The two
 *    surviving logUsage() calls (routes/index.js, routes/assets.js) meter API
 *    requests and video jobs, not tokens.
 *
 * Folding the scattered callers back in here — with JSON opt-in per call, and
 * token logging restored at this layer so it covers all of them — is the open
 * follow-up. Restoring logUsage() to one provider helper alone would only
 * re-create the blind spot in a new place.
 */
