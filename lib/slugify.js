...
function slugify(text) {
  return String(text || '')
    ...
    .replace(/[̀-ͯ]/g, '')
    .toLowerCase()
    // Apostrophes close up rather than becoming a dash: "Player's Guide" is
    // "players-guide", not "player-s-guide". Both straight and curly, because
    // real titles are typed in both.
    .replace(/['‘’]/g, '')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
}
