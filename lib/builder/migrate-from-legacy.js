...
/**
 * Kept in step with the three-column entries of LAYOUT_SPECS in
 * lib/builder-client/builder-template.ts. A three-column layout missing
 * from this list loses its centre column's modules on migration: they fall
 * through the two-column map and land wherever `center` happens to resolve.
 */
const THREE_COLUMN_LAYOUTS = new Set([
  'three-column',
  'one-four-one',
  'one-three-one',
  'one-two-one',
...
function usesThreeColumnLayout(layout) {
  return THREE_COLUMN_LAYOUTS.has(migrateLegacyLayout(layout));
}
