...
function normalizeLegacyBackgroundSettings(value, legacyColor = '') {
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    if (['color', 'gradient', 'image', 'style', 'video'].includes(mode)) {
      return {
        mode,
 ...
 *
 * `cellBackgrounds` is here because mergeNormieCellFields already resolves it
 * with the same "modern value wins if present" rule, one call earlier.
 */
 ...
 * carries none of these fields. It is wrong for an ordinary save, and ordinary
 * saves land here: public/js/builder.js adds `rowSettings` and
 * `containerSettings` to every section it sends (line 5446), which makes
 * isLegacySectionArray call the page legacy.
 *
 ...
 * fields came back at their defaults. `widthMode` is the one a person notices
 * -- a full-width row silently boxed itself back in on every save, with no
 * message -- but it arrived with the row's padding, margins, column widths,
...
function migrateLegacySection(section, index) {
  ...
  return carryModernSectionFields(migrated, section);
}
