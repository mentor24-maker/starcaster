...
__export(builder_template_exports, {
  ...
  finalizeBackgroundSettings: () => finalizeBackgroundSettings,
  finalizeThemeStylesPageBackground: () => finalizeThemeStylesPageBackground,
  formatRichTextContent: () => formatRichTextContent,
  getBuilderBackgroundLayerOpacity: () => getBuilderBackgroundLayerOpacity,
  ...
  normalizeRowOverlayScreenSettings: () => normalizeRowOverlayScreenSettings,
  ...
  promoteThemeStylesPageBackground: () => promoteThemeStylesPageBackground,
  resolveBuilderModuleType: () => resolveBuilderModuleType,
  resolveModuleColumnForLayout: () => resolveModuleColumnForLayout,
...
// lib/builder-client/game-audience.ts
...
// lib/builder-client/module-game-audience.ts
...
// lib/builder-client/module-trigger.ts
...
function clampBuilderOpacity(value, fallback = 100) {
  const num = Number(value);
  if (!Number.isFinite(num)) return fallback;
  return Math.min(100, Math.max(0, Math.round(num)));
}
function applyBuilderColorOpacity(color, opacityPercent) {
...
// lib/builder-client/builder-reminder-module.ts
...
// lib/builder-client/headline-rotator.ts
...
var RICH_TEXT_IMAGE_CLASS = "rich-text-editor-image";
...
var BUILDER_PREVIEW_STORAGE_KEY = "starcaster_builder_preview_draft";
var BUILDER_PREVIEW_DEVICE_STORAGE_KEY = "starcaster_builder_preview_device";
...
function resolveModuleColumnForLayout(column, layout) {
  ...
  const map = normalizedLayout === "single" ? legacyMapSingle : normalizedLayout === "three-column" || normalizedLayout === "one-four-one" ? legacyMapThree : legacyMapTwo;
...
function normalizeDecimalValue(value, fallback, min, max) {
  const parsed = Number.parseFloat(String(value ?? fallback));
  const fallbackValue = Number.parseFloat(fallback);
  const normalized = Number.isFinite(parsed) ? Math.min(Math.max(parsed, min), max) : Math.min(Math.max(Number.isFinite(fallbackValue) ? fallbackValue : min, min), max);
  return String(Number(normalized.toFixed(2)));
}
var HEADING_FONT_KEYS = /* @__PURE__ */ new Set([
  "",
  "inter",
  "poppins",
  "montserrat",
...
var HEADING_TEXT_ALIGN_KEYS = /* @__PURE__ */ new Set(["left", "center", "right"]);
var HEADING_TEXT_TRANSFORM_KEYS = /* @__PURE__ */ new Set(["none", "uppercase", "lowercase", "capitalize"]);
var HEADING_FONT_WEIGHTS = /* @__PURE__ */ new Set(["400", "500", "600", "700", "800", "900"]);
function normalizeHeadingFontWeight(value, bold) {
...
function finalizeBackgroundSettings(background) {
  const normalized = normalizeBackgroundSettings(background);
  if (normalized.mode !== "none") {
    return normalized;
  }
  const raw = background && typeof background === "object" && !Array.isArray(background) ? background : null;
  const rawMode = (0, import_builder_asset_url.safeText)(raw?.mode, 20).toLowerCase();
  if (rawMode === "none") {
    return createDefaultBackgroundSettings();
  }
...
function promoteThemeStylesPageBackground(background) {
  ...
  const defaults = createDefaultBackgroundSettings();
  const colorChanged = normalized.color !== defaults.color && !isTransparentBuilderColor(normalized.color);
  const color2Changed = normalized.color2 !== defaults.color2;
  const opacityChanged = clampBuilderOpacity(normalized.opacity) !== clampBuilderOpacity(defaults.opacity);
  if (colorChanged && color2Changed) {
    return { ...normalized, mode: "gradient" };
...
function finalizeThemeStylesPageBackground(background) {
  return promoteThemeStylesPageBackground(background) ?? createDefaultBackgroundSettings();
}
function getBuilderBackgroundLayerOpacity(background) {
...
var THEME_ELEMENT_KEYS = /* @__PURE__ */ new Set([
  "h1",
  "h2",
  "h3",
  "h4",
...
var THEME_TEXT_TRANSFORMS = /* @__PURE__ */ new Set(["none", "uppercase", "lowercase", "capitalize"]);
...
function normalizeThemeNumber(value, min, max) {
  const num = Number(value);
...
function normalizeThemeFontKey(value) {
  const key = (0, import_builder_asset_url.safeText)(value, 40);
  return HEADING_FONT_KEYS.has(key) ? key : "";
}
function normalizeThemeElementStyle(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const raw = value;
  ...
  const transform = (0, import_builder_asset_url.safeText)(raw.textTransform, 20);
  ...
  const textShadow = (0, import_builder_asset_url.safeText)(raw.textShadow, 200);
...
function normalizeTheme(value) {
  ...
  const theme = value;
  const typography = theme.typography && typeof theme.typography === "object" && !Array.isArray(theme.typography) ? theme.typography : {};
  const fonts = typography.fonts ?? {};
  const scale = typography.scale ?? {};
  const colors = typography.colors ?? {};
  const rawElements = typography.elements && typeof typography.elements === "object" && !Array.isArray(typography.elements) ? typography.elements : {};
  const elements = {};
  for (const key of Object.keys(rawElements)) {
    if (!THEME_ELEMENT_KEYS.has(key)) continue;
    const normalized = normalizeThemeElementStyle(rawElements[key]);
    if (normalized) elements[key] = normalized;
  }
  return {
    typography: {
      fonts: {
      ...
      colors: {
        ...
        linkUnderline: colors.linkUnderline === false ? false : void 0,
        linkHoverUnderline: colors.linkHoverUnderline === false ? false : void 0
      },
      elements
    }
  };
...
function normalizeModuleType(value) {
  ...
  if (type === "navigation" || type === "heading" || type === "headline-rotator" || type === "code" || type === "merch" || type === "image" || type === "floating-image" || type === "video" || type === "quote" || type === "speech-bubble" || type === "reminder" || type === "button" || type === "contact-form" || type === "player-portal" || type === "table" || type === "slider" || type === "social" || type === "social-share" || type === "previous-results" || type === "current-poll" || type === "poll-category-list" || type === "confetti" || type === "tractor-nav" || type === "breadcrumb" || type === "blog-post-list" || type === "blog-post-card" || type === "blog-author-bio" || type === "crm-form" || type === "crm-contacts-table" || type === "blog-toc" || type === "blog-newsletter-subscribe" || type === "blog-related-posts" || type === "blog-category-filter" || type === "blog-post" || type === "blog-tag-cloud" || type === "blog-post-tags" || type === "blog-post-create" || type === "blog-post-manager" || type === "blog-category-manager" || type === "blog-card-manager" || type === "blog-search" || type === "blog-search-results" || type === "messaging-topic-list" || type === "messaging-tag-list" || type === "admin-team-users" || type === "admin-modules" || type === "admin-login" || type === "admin-nav-link") {
    return type;
  }
  return "text";
}
...
function normalizeBuilderModuleSettingsForType(type, value, moduleContext) {
  ...
  if (type === "heading") {
    ...
    settings.textAlign = HEADING_TEXT_ALIGN_KEYS.has(settings.textAlign ?? "") ? settings.textAlign : "left";
    settings.textTransform = HEADING_TEXT_TRANSFORM_KEYS.has(settings.textTransform ?? "") ? settings.textTransform : "none";
...
function normalizeLayoutSections(value) {
  ...
  return value.map((section, sectionIndex) => {
    ...
    return {
      ...
      rowBorderStyle: ["solid", "dashed", "dotted"].includes(normalizedSection.rowBorderStyle) ? normalizedSection.rowBorderStyle : "solid",
...
function rowToBuilderPage(row) {
  ...
  return {
    ...
    themeId: (0, import_builder_asset_url.safeText)(row.theme_id ?? row.themeId, 120) || void 0,
    pageBackground: document.pageBackground,
    theme: document.theme,
    layoutSections: document.layoutSections,
    createdAt: (0, import_builder_asset_url.safeText)(row.created_at ?? row.createdAt, 120),
