...
function getButtonModuleStyle(settings) {
  ...
  const resolvedBorderColor = borderStyle === "none" || settings.borderColor === "transparent" ? "transparent" : settings.borderColor || "#214c71";
...
function renderEmailModule(module2) {
  ...
  if (module2.type === "image") {
    ...
    return `<tr><td align="${alignAttr}" style="padding:0 40px 12px;${marginStyle}"><img src="${escapeHtml(url)}" alt="${escapeHtml(module2.settings.alt || module2.name || "Image")}" width="${Math.round(600 * (width / 100))}" style="display:block;max-width:100%;height:auto;border:0;" /></td></tr>`;
  }
