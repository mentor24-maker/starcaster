...
async function saveDirectAcquireRun(run, scope = null) {
  ...
  store.runs = store.runs.filter((item) => String(item.run_id) !== summary.run_id);
...
async function listDirectAcquireRuns(limit = 20, scope = null) {
  ...
  if (await supportsSupabaseRuns()) {
    const query = await scopedListQuery(
      table(),
      `select=run_id,source_url,pages_succeeded,pages_failed,started_at,finished_at,project_id,owner_user_id,created_at&order=created_at.desc&limit=${safeLimit}`,
      scope
    );
