...
async function sendEmail({ to, subject, html, text, fromName, fromEmail }) {
  ...
  const fallbackFromName = String(resend?.email_from_name || config.get('emailFromName') || 'Alphire Internal Platform');
  const fallbackFromEmail = String(resend?.email_from_address || config.get('emailFromAddress') || 'noreply@alphire.ai');
