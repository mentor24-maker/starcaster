...
function config() {
  ...
  return {
    clientId: String(values.client_id || '').trim(),
    clientSecret: String(values.client_secret || '').trim(),
    refreshToken: String(values.refresh_token || '').trim(),
...
async function getAccessToken() {
  ...
  if (!cfg.clientId || !cfg.clientSecret || !cfg.refreshToken) {
    return { ok: false, status: 400, error: 'Google Drive credentials are not configured in Settings > APIs.' };
  }
