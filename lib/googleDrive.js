...
async function getAccessToken() {
  ...
  if (!res.ok || !payload.access_token) {
    // BOTH halves, and the code FIRST. Google's `error_description` for a dead
    // refresh token is the words "Bad Request", which names no cause and
    // suggests no move; the actionable half is `error`, which says
    // `invalid_grant` — the one string that means "this sign-in is finished,
    // only a browser login renews it". Preferring the description alone threw
    // that away, and did on the live credential on 2026-09-14: a caller written
    // to recognise an expired sign-in could not see one (task 86bc0nbwq).
    const code = String(payload.error || '').trim();
    const detail = String(payload.error_description || '').trim();
    const both = [code, detail].filter(Boolean).join(': ');
    return {
      ok: false,
      status: res.status || 500,
      error: both || 'Failed to obtain Google Drive access token',
    };
  }
...
/**
 * A NON-folder file by name inside one folder, or null.
 *
 * The weekly report re-uploads the same names every week (`2026-09-14.html`,
 * `index.html`). Drive has no unique-name constraint, so a plain upload leaves
 * two files with the same name in the folder and no way for a reader to tell
 * which is this week's — so the uploader looks first and updates in place.
 */
async function findFileByName(token, parentId, fileName) {
  const q = [
    `mimeType!='application/vnd.google-apps.folder'`,
    `trashed=false`,
    `'${escapeDriveQueryLiteral(parentId)}' in parents`,
...
/**
 * Every non-folder file in one Drive folder, with the link that opens it.
 *
 * The token-taking sibling of listImageFilesInFolder, which signs in for itself
 * — that one is for a route handler that has nothing in hand; this one is for a
 * caller already holding a token and part-way through a batch of calls, where
 * minting a second one would be a second chance to fail.
 *
 * IT PAGES, and the reason is not hypothetical: the weekly report's folder
 * gains three files a week for as long as the report runs, so a single-page
 * read would silently start dropping the oldest editions from the index about
 * eight months in — an index that is wrong only once it has been right for long
 * enough that nobody is checking it any more.
 */
async function listFilesInFolder(token, folderId, options = {}) {
  const id = String(folderId || '').trim();
  if (!id) return { ok: false, status: 400, error: 'folderId is required' };

  const maxFiles = Math.max(1, Math.min(1000, Number(options.limit || 500) || 500));
  const q = [
  ...
  while (files.length < maxFiles) {
    ...
    const listRes = await driveRequest({ token, path: '/files', query: params.toString() });
    if (!listRes.ok) return listRes;

    for (const file of Array.isArray(listRes.data.files) ? listRes.data.files : []) {
...
/** Replace the CONTENT of an existing Drive file, keeping its id and link. */
async function updateFileMedia({ token, fileId, mimeType, fileBuffer }) {
  const url = `${DRIVE_UPLOAD_BASE}/files/${encodeURIComponent(fileId)}`
    + `?uploadType=media&fields=id,name,mimeType,size,webViewLink`;
  let res;
  try {
    res = await fetch(url, {
      method: 'PATCH',
    ...
    return { ok: false, status: 502, error: `Google Drive update failed: ${err.message}` };
  }
  ...
  if (!res.ok) {
    return {
      ok: false,
      status: res.status,
      error: payload?.error?.message || 'Google Drive update failed',
    };
  }
  return { ok: true, status: res.status, data: payload };
}
