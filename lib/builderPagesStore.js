...
const { applyTemplateFrame, bodySectionsOf } = require('./builder/template-frame');
...
//   - A template with no sections. It carries no frame, so applying it would
//     strip each page's shared header and footer and put nothing back — which
//     is not "change template" either. (Until 2026-09-14 it was worse than
//     that: the change re-poured, so an empty template deleted the content of
//     every selected page. The wording below was written for that behaviour
//     and is corrected with it.) The built-in stub the page-templates route
//     injects is declared with layoutSections: [], and five production pages
//     carry its id, so this is reachable, not theoretical.
function describeBulkTemplateTarget(template, requestedId) {
  ...
  if (!sections.length) {
    return {
      ok: false,
      status: 400,
      error: `"${name}" has no sections, so it has no shared header or footer to apply — moving pages onto it would `
        + 'strip the shared sections they have now and put nothing in their place.',
    };
  }
...
async function setOnePageTemplate(pageId, target, scope, options = {}) {
  ...
  // THREE things are compared, and the third is the one this operation exists
  // to protect: the page's own content count, unchanged. Checking the total
  // alone would pass a row that swapped body for frame and kept the same
  // length — which is exactly the shape of the damage being guarded against.
    ...
    error: verified
      ...
      : safeText(
        `Saved, but read back with template "${savedTemplateId || 'none'}", ${sectionsAfter.length} section(s) `
          + `and ${bodyAfter} content section(s) instead of "${target.id}", ${framed.sections.length} `
          + `and ${bodyBefore}.`,
        500,
      ),
  };
