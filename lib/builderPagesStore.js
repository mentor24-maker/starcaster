...
async function propagateCanonicalSection(savedSectionId, updatedSection, scope, options = {}) {
  if (!savedSectionId || !updatedSection) return { ok: false, total: 0, updated: 0, failed: 0, skipped: [], runId: '' };
  const pagesResult = await listPages(5000, scope);
  if (!pagesResult.ok) return { ok: false, total: 0, updated: 0, failed: 0, skipped: [], runId: '' };
  ...
  for (let i = 0; i < targets.length; i += CANONICAL_PROPAGATION_CONCURRENCY) {
    ...
    await Promise.all(batch.map(async (page) => {
      ...
      const updatedSections = page.layoutSections.map((s) => {
        ...
        if (!overwriteDrifted && hasSectionDrifted(s, previousSection)) {
  ...
  return { ok: failed === 0, total: targets.length, updated, failed, skipped, runId: updated ? runId : '' };
}
