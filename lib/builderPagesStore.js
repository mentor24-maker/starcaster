...
async function setOnePageTemplate(pageId, target, scope) {
...
async function bulkSetPageTemplate(pageIds, pageTemplateId, scope = null) {
  const ids = (Array.isArray(pageIds) ? pageIds : [])
    .map((id) => Number(id || 0) || 0)
    .filter((id) => id > 0);
  if (!ids.length) return { ok: false, status: 400, error: 'pageIds is required' };
  ...
  for (let start = 0; start < ids.length; start += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(batch.map((pageId) => setOnePageTemplate(pageId, target, scope)));
