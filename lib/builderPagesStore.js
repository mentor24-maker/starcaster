...
const { recordPageRevision } = require('./builderPageRevisionsStore');
const {
...
async function propagateCanonicalSection(savedSectionId, updatedSection, scope) {
  if (!savedSectionId || !updatedSection) return { ok: false, total: 0, updated: 0, failed: 0 };
  const pagesResult = await listPages(5000, scope);
  if (!pagesResult.ok) return { ok: false, total: 0, updated: 0, failed: 0 };
    ...
    await Promise.all(batch.map(async (page) => {
      ...
      try {
        ...
        const res = await updatePage(
          ...
          scope,
          { previous: page, reason: 'propagate' }
        );
