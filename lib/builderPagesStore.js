...
function pageLayoutSectionCount(page) {
  return Array.isArray(page?.layoutSections) ? page.layoutSections.length : 0;
}

async function restoreLayoutShellFromPage({
  sourcePageId,
  sourceSlug,
  targetPageIds,
  overwrite = false,
}, scope = null) {
  const targetIds = (Array.isArray(targetPageIds) ? targetPageIds : [])
    .map((id) => Number(id || 0) || 0)
    .filter((id) => id > 0);
  if (!targetIds.length) return { ok: false, status: 400, error: 'targetPageIds is required' };
  ...
  let source = null;
  const sourceId = Number(sourcePageId || 0) || 0;
  if (sourceId) {
    source = pages.find((page) => Number(page.id || 0) === sourceId) || null;
  }
  if (!source && sourceSlug !== undefined) {
    const normalizedSlug = safeText(sourceSlug, 160).toLowerCase();
    source = pages.find((page) => safeText(page.slug, 160).toLowerCase() === normalizedSlug) || null;
    if (!source && normalizedSlug === 'home') {
      source = pages.find((page) => safeText(page.slug, 160) === '') || null;
    }
  }
  if (!source) {
    source = pages.find((page) => pageLayoutSectionCount(page) > 0) || null;
  }
  if (!source || pageLayoutSectionCount(source) === 0) {
    return { ok: false, status: 400, error: 'Source page with layout content not found' };
  }
  ...
  const shell = {
    ...
    theme: source.theme,
    layoutSections: JSON.parse(JSON.stringify(source.layoutSections || [])),
  };

  const results = await Promise.all(
    targetIds.map(async (pageId) => {
      const target = pages.find((page) => Number(page.id || 0) === pageId) || null;
      if (!target) return { id: String(pageId), ok: false, error: 'Page not found' };
      if (!overwrite && pageLayoutSectionCount(target) > 0) {
        return { id: String(pageId), ok: true, skipped: true, reason: 'already_has_layout' };
      }
      const res = await updatePage(pageId, shell, scope);
      return {
        id: String(pageId),
        ok: res.ok,
        skipped: false,
  ...
  const restored = results.filter((row) => row.ok && !row.skipped).length;
  const skipped = results.filter((row) => row.skipped).length;
  const failed = results.filter((row) => !row.ok);
  if (!restored && failed.length === results.length) {
    return { ok: false, status: 500, error: failed[0]?.error || 'Could not restore pages' };
  }
  ...
  return {
    ...
    status: 200,
    data: {
      sourcePageId: String(source.id || ''),
      sourceSlug: safeText(source.slug, 160),
      restored,
      skipped,
      results,
    },
