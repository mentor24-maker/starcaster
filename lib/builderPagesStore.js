...
async function propagateCanonicalSection(savedSectionId, updatedSection, scope, options = {}) {
  if (!savedSectionId || !updatedSection) return { ok: false, total: 0, updated: 0, failed: 0, runId: '' };
  const pagesResult = await listPages(5000, scope);
  if (!pagesResult.ok) return { ok: false, total: 0, updated: 0, failed: 0, runId: '' };
  ...
  return { ok: failed === 0, total: targets.length, updated, failed, runId: updated ? runId : '' };
}
...
async function listPublishedPagesForProject(projectIdInput) {
  ...
  const data = await enrichPagesWithThemeShell(filtered, projectId);
  return {
    ok: true,
    status: 200,
    data,
