...
const CANONICAL_PROPAGATION_CONCURRENCY = 8;

/**
 * After a saved section is updated, push the new content to every page that
 * has a canonical instance of that saved section.
 *
 * MUST be awaited before the response is sent.  Serverless freezes the
 * function once the response goes out, so calling this fire-and-forget kills
 * it partway down the page list — leaving some pages on the new content and
 * the rest silently stale, with a different split every save.
 *
 * Returns a per-run tally so callers can surface a partial failure instead of
 * reporting a save that only half landed.
 *
 * `options.previousSection`, when given, is the master's content BEFORE this
 * save — an instance whose content already differs from THAT (not from the
 * new proposal) was edited directly on its own page while still marked
 * Following, and is skipped rather than flattened, unless
 * `options.overwriteDrifted` says otherwise. Omitting `previousSection`
 * disables the check entirely (nothing is ever skipped) — the safe default
 * for a caller that does not have it, matching this function's behavior
 * before drift detection existed.
 */
async function propagateCanonicalSection(savedSectionId, updatedSection, scope, options = {}) {
  const empty = { ok: false, total: 0, updated: 0, failed: 0, skipped: [], overwritten: [], runId: '' };
  if (!savedSectionId || !updatedSection) return empty;
  const pagesResult = await listPages(5000, scope);
  if (!pagesResult.ok) return empty;
  ...
  // `options.onlyPageIds`, when given, narrows the push to THOSE pages — the
  // explicit "overwrite the copies you skipped" opt-in. Without it a force
  // run would rewrite every follower (46 writes to overwrite 2 drifted
  // copies), inflating the undo run and the loop length the serverless
  // timeout note below warns about. The banner knows exactly which pages
  // were skipped, so it says so; recomputing drift here at force time is not
  // possible — the master has already been saved and the "before" is gone.
  const onlyPageIds = Array.isArray(options.onlyPageIds)
    ? new Set(options.onlyPageIds.map((id) => String(id)))
    : null;

  const targets = pagesResult.data.filter((page) => (
    (!onlyPageIds || onlyPageIds.has(String(page.id))) &&
    page.layoutSections.some(
  ...
  let updated = 0;
  let failed = 0;
  // Two buckets, never one: `skipped` is a drifted copy LEFT ALONE, and
  // `overwritten` is a drifted copy written anyway because the caller opted
  // in. The first version put both in `skipped`, so the shared copy told the
  // operator an overwrite had been a skip.
  const skipped = [];
  const overwritten = [];

  for (let i = 0; i < targets.length; i += CANONICAL_PROPAGATION_CONCURRENCY) {
    const batch = targets.slice(i, i + CANONICAL_PROPAGATION_CONCURRENCY);
    await Promise.all(batch.map(async (page) => {
      let pageHadDrift = false;
      ...
      const updatedSections = page.layoutSections.map((s) => {
        ...
        // push, so an overwritten copy can be NAMED as overwritten rather
        // than silently folded into the ordinary count.
        const drifted = previousSection ? hasSectionDrifted(s, previousSection) : false;
        if (drifted) pageHadDrift = true;
      ...
      if (!pageHasChange) {
        // Every matching instance on this page drifted — nothing to write,
        // so no PATCH, no revision, and no undo-run entry for a page the
        // push never actually touched.
        if (pageHadDrift) skipped.push({ pageId: page.id, name: page.name });
        return;
      }

      try {
        // `page` is the pre-propagation state and we already hold it, so hand
        // it over rather than making updatePage re-read every one of these.
        // This is also what gives a mass propagation an undo: on 2026-07-21 a
        // single master save rewrote 46 Marinoff pages with no way back.
        const res = await updatePage(
          page.id,
          { ...page, layoutSections: updatedSections },
          scope,
          {
            previous: page,
            reason: 'propagate',
        ...
        if (res && res.ok === false) failed += 1;
          ...
          updated += 1;
          if (pageHadDrift) overwritten.push({ pageId: page.id, name: page.name });
        }
      } catch {
        failed += 1;
      }
  ...
  // rewritten are exactly the ones an undo needs to reach.
  return { ok: failed === 0, total: targets.length, updated, failed, skipped, overwritten, runId: updated ? runId : '' };
}
