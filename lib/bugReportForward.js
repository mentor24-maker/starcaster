...
const { createHeldTask, isConfigured: isClickupConfigured } = require('./clickupForward');
...
async function forwardBugReport(report, scope, deps = {}) {
  ...
  try {
    ...
    const created = await createHeldTask(task, deps);
    if (!created.ok) return await fail(created.error, created.code);
