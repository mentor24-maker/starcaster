...
function getProviderValues(provider) {
  ...
  const storedValues = (node && typeof node === 'object' && typeof node.values === 'object')
    ? { ...node.values }
    : {};
  if (key !== 'openclaw') return storedValues;

  const envValues = {
    base_url: String(process.env.OPENCLAW_BASE_URL || '').trim(),
    api_key: String(process.env.OPENCLAW_API_KEY || '').trim(),
    timeout_ms: String(process.env.OPENCLAW_TIMEOUT_MS || '').trim(),
  };
  // In serverless environments, persisted file values can be stale/read-only.
  // For OpenClaw, prefer explicit env vars when present.
...
function listApiConfigsMasked() {
  ...
  return Object.entries(API_SCHEMAS).map(([provider, schema]) => {
    ...
    const values = node && typeof node.values === 'object' ? node.values : {};
    const maskedValues = {};
