...
const { inspectProviderValues, inspectValue } = require('./credentialShape');
...
const API_SCHEMAS = {
  supabase: {
    ...
    fields: [
      ...
      { key: 'builder_themes_table', label: 'Builder Themes Table', required: false, secret: false },
      { key: 'builder_email_templates_table', label: 'Builder Email Templates Table', required: false, secret: false },
      { key: 'builder_page_templates_table', label: 'Builder Page Templates Table', required: false, secret: false },
  ...
  youtube_media_worker: {
    label: 'YouTube Media Worker (yt-dlp)',
    fields: [
      { key: 'base_url', label: 'Worker URL', required: true, secret: false },
      { key: 'api_key', label: 'Shared Secret', required: true, secret: true },
      { key: 'timeout_ms', label: 'Timeout (ms)', required: false, secret: false }
    ]
  },
  buffer: {
  ...
  x: {
    label: 'X (Twitter)',
    // TWO credentials for one platform, and they are not interchangeable.
    // The four `api_*`/`access_token*` values are OAuth 1.0a and are what
    // Starcaster's OWN posting signs with. The two `client_*` values are
    // OAuth 2.0 and are what lets a CLIENT sign in and post as themselves
    // (Connections 7 of 7, 86bbpz1hu). X issues them on separate screens of
    // the developer portal. They are `required: false` because Dane's own
    // posting works without them and must not start reporting itself broken.
    fields: [
      { key: 'api_key', label: 'API Key', required: true, secret: true },
      ...
      { key: 'client_id', label: 'OAuth 2.0 Client ID (for client sign-in)', required: false, secret: true },
      { key: 'client_secret', label: 'OAuth 2.0 Client Secret (for client sign-in)', required: false, secret: true },
...
// A value saved in Settings > APIs wins; env vars are only a fallback for
// fields Settings hasn't set. This is what the operator expects: what you type
// in the UI takes effect. It stays safe in serverless/prod because the store
// file (data/api_settings.json) is gitignored and read-only there, so
// storedValues is empty and the env fallback supplies everything.
function resolveProviderValues(storedValues, envValues) {
...
function getProviderValues(provider, options = {}) {
  ...
  const storedValues = (node && typeof node === 'object' && typeof node.values === 'object')
    ? { ...node.values }
    : {};
  return resolveProviderValues(storedValues, envValues);
}
...
function envVarNamesForProviderField(provider, fieldKey) {
  ...
  if (key === 'x') {
    ...
    if (field === 'client_id') names.push('X_OAUTH_CLIENT_ID');
    if (field === 'client_secret') names.push('X_OAUTH_CLIENT_SECRET');
...
function getProviderCredentialDiagnostics(provider, options = {}) {
  ...
  const resolveEnvName = (fieldKey) =>
    envVarNamesForProviderField(key, fieldKey).find(
      (name) => String(process.env[name] || '').trim()
    ) || envVarNamesForProviderField(key, fieldKey)[0];

  schema.fields.forEach((field) => {
    const value = String(merged[field.key] || '').trim();
    preview[field.key] = field.secret ? maskValue(value) : (value || '(empty)');
    // Mirror getProviderValues precedence: a saved Settings value wins; env is
    // the fallback. envAvailable records that an env var also exists so the UI
    // can flag "Settings is overriding an env var" for this field.
    const hasStored = !envOnly && Boolean(String(storedValues[field.key] || '').trim());
    const hasEnv = Boolean(String(envValues[field.key] || '').trim());
    envAvailable[field.key] = hasEnv;
    if (hasStored) {
      sources[field.key] = 'file';
      envKeys[field.key] = hasEnv ? resolveEnvName(field.key) : null;
    } else if (hasEnv) {
      sources[field.key] = 'env';
      envKeys[field.key] = resolveEnvName(field.key);
    } else {
  ...
  // Inspect the values actually in effect, not just ones typed into the form.
  // This is the half that reaches production: an env var set in the Vercel
  // dashboard never passes through upsertApiConfig, so a trailing newline or a
  // quoted paste there would otherwise go unnoticed until it caused a 401.
  // Only the problem descriptions cross this boundary — never a value.
  const shapeProblems = [];
  schema.fields.forEach((field) => {
    const value = String(merged[field.key] || '');
    if (!value) return;
    inspectValue({ provider: key, field: field.key, value, isSecret: Boolean(field.secret) })
      .forEach((problem) => {
        shapeProblems.push({
          field: field.key,
          label: field.label,
          ...
          source: sources[field.key] || 'unknown',
        });
      });
  });
...
function getProviderEnvValues(provider) {
  ...
  if (key === 'x') {
    ...
    out.client_id = firstEnvValue(['X_CLIENT_ID', 'X_OAUTH_CLIENT_ID']);
    out.client_secret = firstEnvValue(['X_CLIENT_SECRET', 'X_OAUTH_CLIENT_SECRET']);
...
function upsertApiConfig(input) {
  ...
  // Inspect BEFORE trimming, so problems that trimming would hide — a trailing
  // space, a wrapping quote pair — are still reported to the operator.
  const inspection = inspectProviderValues(provider, rawValues, schema.fields);
  if (!inspection.ok) {
    ...
    return {
      ...
      data: { errors: inspection.errors, warnings: inspection.warnings },
    };
  }

  // The form no longer prefills secrets, so a blank field means "leave this one
  // as it is" rather than "clear it". Without this, opening the form and saving
  // one changed field would wipe every other secret. Use DELETE on the provider
  // to clear credentials deliberately.
  const store = readStoreSync();
  const existing = (store.apis[provider] && typeof store.apis[provider].values === 'object')
    ? store.apis[provider].values
    : {};
  // Env vars count as "already set" for the required check: a field supplied by
  // the environment does not need to be retyped into the form.
  const effective = getProviderValues(provider);

  for (const field of schema.fields) {
    const submitted = String(rawValues[field.key] || '').trim();
    const kept = String(existing[field.key] || '').trim();
    const value = submitted || kept;
    const satisfied = value || String(effective[field.key] || '').trim();
  ...
  // Warnings ride along with a successful save: they are heuristics ("Anthropic
  // keys usually start with sk-ant-"), and refusing a valid key because our
  // pattern is out of date would be worse than the problem being prevented.
  return {
    ok: true,
    status: 200,
    data: { provider, label: schema.label, warnings: inspection.warnings },
  };
}

/**
 * The real value of ONE field, for an explicit "Reveal" click.
 *
 * Deliberately one field per call rather than a whole provider: revealing an
 * API key should not also hand over the secret next to it. The caller is
 * responsible for logging the reveal — see routes/settings.js.
 */
function revealApiProviderField(providerInput, fieldInput) {
  const provider = normalizeKey(providerInput);
  if (!provider || !API_SCHEMAS[provider]) {
    return { ok: false, status: 400, error: 'Unsupported provider' };
  }
  const schema = API_SCHEMAS[provider];
  const fieldKey = String(fieldInput || '').trim().toLowerCase();
  const field = schema.fields.find((f) => f.key === fieldKey);
  if (!field) {
    return { ok: false, status: 400, error: 'Unknown field for this provider' };
  }
  const values = getProviderValues(provider);
  const value = String(values[field.key] || '');
  if (!value) {
    return { ok: false, status: 404, error: `${field.label} is not set` };
  }
  return {
    ok: true,
    status: 200,
    data: { provider, field: field.key, label: field.label, value },
  };
}
...
function getApiConfig(providerInput) {
  ...
  // Secrets are masked here. This endpoint used to return them in full so the
  // edit form could prefill, which meant every visit to Settings > APIs pulled
  // live credentials into the browser — including ones that exist only as
  // production environment variables. The real value is now fetched one field
  // at a time through revealApiProviderField(), on an explicit click that gets
  // logged. See docs/CREDENTIALING_HARDENING_PLAN.md Phase 5.
  const safeValues = {};
  const hasValue = {};
  schema.fields.forEach((field) => {
    const value = String(values[field.key] || '').trim();
    hasValue[field.key] = value.length > 0;
    safeValues[field.key] = field.secret ? maskValue(value) : value;
  });

  // Per-field source metadata (env | file | missing) + envAvailable, so the
  // Settings UI can show which value is actually in effect. All non-secret
  // (masked preview + env var NAMES only) — safe to send to the browser.
  ...
  return {
    ...
    data: {
      ...
      label: schema.label,
      values: safeValues,
      // Which fields already hold something, so the form can offer Reveal and
      // treat a blank submission as "leave this one alone".
      hasValue,
      diagnostics: diag.ok
        ? { sources: diag.sources, envAvailable: diag.envAvailable, envKeys: diag.envKeys }
        : null
    }
  };
}
