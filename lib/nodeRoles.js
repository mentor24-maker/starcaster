...
const ROLES = {
  ...
  /*
   * TEMPORARY, 2026-09-20 — 86bc3wn1v. MOVE BOTH BACK TO 'mac-mini' AS SOON AS
   * THE MINI IS SIGNED IN AGAIN.
   *
   * The Mini's Claude sign-in expired on 2026-09-18 and both lanes have built
   * nothing since: every pass exits in about a second with "OAuth session
   * expired and could not be refreshed" (278 consecutive failures by 09-20).
   * Repairing it needs a shell on the machine, and there is no route to one
   * until Dane is home on 2026-10-01 — he is travelling on a different
   * network and the Mini's Tailscale went offline on 2026-09-10. The machine
   * is powered on, on the internet, running its other jobs, and unreachable.
   *
   * So the queue was draining only as fast as a session could hand-build it,
   * and the laptop is the only machine whose Claude sign-in works.
   *
   * THE REASON THE MINI OWNED THESE IS STILL TRUE AND IS NOT BEING DISPUTED:
   * a laptop closes, and a build loop on a sleeping machine is a queue that
   * stops moving overnight. Dane accepted that cost explicitly rather than
   * having it discovered for him — verbatim, 2026-09-20: "I'll lose the
   * evening work... but then, I always run out of quota anyway, so running
   * overnight just accelerates the quota exhaustion."
   *
   * The claim race is unchanged and is why this is a COMMIT rather than a
   * setting: check-then-act is sound with exactly one claimant and unsound
   * with two. It is safe today only because the Mini's lanes cannot claim
   * anything at all — they fail before they reach the queue. The moment that
   * machine is signed in again, both owners must come back here in one edit,
   * or two machines will claim the same tickets.
   */
  'loop-build': {
    owner: 'macbook-pro',
    why: 'TEMPORARY (2026-09-20, task 86bc3wn1v): the Mini cannot sign in to Claude and cannot '
      + 'be reached to fix it until 2026-10-01, so its build lane has been dead since 09-18. '
      + 'The laptop is the only machine that can run a pass. Claiming a ticket is check-then-act '
      + 'and is only safe with a single claimant — this is safe solely because the Mini\'s lane '
      + 'fails before it can claim. Move back to mac-mini the day the Mini signs in.',
  },
  'loop-review': {
    owner: 'macbook-pro',
    why: 'TEMPORARY (2026-09-20, task 86bc3wn1v): moved with loop-build, and for the same reason. '
      + 'Review is the lane that backs up, so leaving it on a machine that cannot run it would '
      + 'just move the queue from Queued to In review. Move back to mac-mini with loop-build — '
      + 'the two belong on one machine.',
  },
  'weekly-report': {
    owner: 'mac-mini',
    why: 'It uploads the week\'s edition to a shared Google Drive folder. Two machines '
      + 'holding the schedule would race to write the same three filenames there every '
      + 'week, and the duplicate is only visible after both have already written. The Mini '
  ...
  'channel-steward': {
    owner: 'mac-mini',
    why: 'One of the two Pulse pipelines. Scheduled pipelines double-post if two machines both '
      + 'hold the schedule, so it belongs to exactly one. THE MINI, since 2026-09-07 at Dane\'s '
      + 'request, for the reason the relay moved there: these are launchd StartInterval jobs, and '
      + 'on a LAPTOP they stop whenever the lid closes and resume on wake. Measured that day, '
      + 'channel-steward ran at 10:30 and not again until 12:17 — an hour and three-quarters dark '
      + 'with nothing reporting the gap. The move needed pulse PR #4 first: every path in that repo '
      + 'was written as /Users/mentor, and the Mini\'s home is /Users/daneofearth, so the '
      + 'schedules would have failed silently — launchd runs a job whose program does not exist and '
      + 'logs it where nobody looks. Its bin/install-launchd.sh now GENERATES each plist from the '
      + 'machine it runs on. '
      + 'NOTHING ENFORCES THIS ROW AT RUN TIME: pulse has no nodeRoles check of its own, so the '
      + 'guard against double-posting is that the schedule is installed on exactly one machine. It '
      + 'was uninstalled from the MacBook the same day, before being installed here, and '
      + '`bin/install-launchd.sh --status` is how you check.',
  },
  'librarian-sweep': {
    owner: 'mac-mini',
    why: 'The other Pulse pipeline, and it sits here for exactly the reasons channel-steward does — '
      + 'see that row for the cutover and for why nothing enforces this at run time. It is listed '
      + 'separately rather than folded in with it because the two run at wildly different cadences '
      + '(15 minutes against 24 hours), and one row could only carry one of them. That is not '
      + 'bookkeeping: the roll call sizes each role\'s overdue window from its own cadence, so a '
      + 'single shared row would have judged a daily job against a quarter-hourly one and reported '
      + 'it dead six times a day while it was perfectly healthy.',
  },
  'pipeline-pulse': {
    owner: 'mac-mini',
    why: 'Not the same thing as the two Pulse pipelines above, despite the name — this is `npm run pulse`, '
  ...
  'studio-worker': {
    owner: 'mac-mini',
    why: 'The Studio pipeline\'s long-running daemon (workers/studio/daemon.js). It claims one job '
      + 'at a time from a LOCAL sqlite queue on the machine it runs on, so a second copy on another '
      + 'machine is not a spare — it is a second queue, holding different work, invisible to the '
      + 'first. Worse, the two failures that matter are both silent: the cache folder and every '
      + 'downloaded file live on one disk (~/Studio), so a daemon elsewhere re-downloads footage the '
      + 'first one already has and re-encodes proxies that already exist, while the disk-floor alarm '
      + 'on each machine reads a disk the other one is not filling. THE MINI, and for the same reason '
      + 'youtube-media sits there: this work is measured in hours of ffmpeg over multi-gigabyte files '
      + 'and needs a machine whose lid does not close. It is also why none of it can live on Vercel — '
      + 'see .vercelignore and npm run check:vercel-bundle.',
  },
  'youtube-media': {
    owner: 'mac-mini',
