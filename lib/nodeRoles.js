...
const ROLES = {
  ...
  'pulse-pipelines': {
    ...
    why: 'Scheduled pipelines double-post if two machines both hold the schedule. '
      + 'MOVED TO THE MINI on 2026-09-07 at Dane\'s request, for the reason the relay '
      + 'moved there: these are launchd StartInterval jobs on a LAPTOP, so they stop '
      + 'whenever the lid closes and resume on wake. Measured that day, channel-steward '
      + 'ran at 10:30 and not again until 12:17 — an hour and three-quarters dark, with '
      + 'nothing reporting the gap, because the roll call reads this pair as NOT '
      + 'REPORTING (their runner lives in the pulse repo, not this checkout). '
      + 'The move needed pulse PR #4 first: every path in that repo was written as '
      + '/Users/mentor, and the Mini\'s home is /Users/daneofearth, so the schedules '
      + 'would have failed silently — launchd runs a job whose program does not exist '
      + 'and logs it where nobody looks. Its bin/install-launchd.sh now GENERATES each '
      + 'plist from the machine it runs on, which is the NODES Slice B installer this '
      + 'row and doctor:node were both waiting on (ticket 86bbh9kh2). '
      + 'NOTHING ENFORCES THIS ROW AT RUN TIME: pulse has no nodeRoles check of its own, '
      + 'so the guard against double-posting is that the schedule is installed on '
      + 'exactly one machine. It was uninstalled from the MacBook the same day, before '
      + 'being installed here, and `bin/install-launchd.sh --status` is how you check.',
  },
  'pipeline-pulse': {
    owner: 'mac-mini',
