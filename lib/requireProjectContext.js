...
function isProjectContextOptional(pathname, method) {
  ...
  if (path === '/api/contacts/accept-project-invite' && requestMethod === 'POST') return true;

  // Invitations are how someone GETS a project, so requiring one first would
  // be circular. The route decides for itself which project (if any) an
  // invitation may name, and who is allowed to name it.
  if (path.startsWith('/api/invitations')) return true;
  if (path.startsWith('/api/platform-screenshots')) return true;

  // Public CRM routes — form rendering and submission don't require a project context
  if (path === '/api/crm/contact-submit' && requestMethod === 'POST') return true;
  if (/^\/api\/crm\/forms\/[^/]+$/.test(path) && requestMethod === 'GET') return true;
