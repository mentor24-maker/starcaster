...
async function ensureSeeded() {
  ...
  return sbQuery({
    ...
    body: DEFAULT_MESSAGING_FORMATS,
  });
}

async function listMessagingFormats(limit = 5000) {
  const seeded = await ensureSeeded();
  if (!seeded.ok) return seeded;
  const result = await sbQuery({
    table: t().messagingFormats,
    query: `select=*&order=sort_order.asc.nullslast,format.asc&limit=${Number(limit || 5000) || 5000}`,
  });
...
async function createMessagingFormat(input) {
  const row = {
  ...
  if (!row.format) return { ok: false, status: 400, error: 'format is required' };
  if (!row.family) return { ok: false, status: 400, error: 'family is required' };
  const existing = await sbQuery({
    table: t().messagingFormats,
    query: `select=*&format=eq.${encodeURIComponent(row.format)}&limit=1`,
  });
...
async function updateMessagingFormat(id, input) {
...
async function deleteMessagingFormat(id) {
  const formatId = Number(id || 0) || 0;
  if (!formatId) return { ok: false, status: 400, error: 'id is required' };
  const result = await sbQuery({
    method: 'DELETE',
