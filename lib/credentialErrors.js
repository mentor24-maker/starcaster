...
const CODES = {
  ...
  /**
   * The credentials are fine and the platform accepted them. The ACCOUNT they
   * point at simply cannot do this, and no credential will ever change that.
   *
   * Added for Instagram (Connections 5 of 7, 86bbpz1gk), where the two things
   * that go wrong are both of this kind: a personal Instagram account cannot be
   * posted to by any API at all, and an Instagram account with no Facebook Page
   * behind it has nothing to post through. Neither is a token problem, and
   * every existing code above would have sent the reader to check a credential
   * that is perfectly good — which is the exact failure this whole file was
   * written to stop.
   *
   * The test for belonging here is whether the code changes what the reader
   * should DO. It does: change the account, not the key.
   */
...
const HEADLINES = {
  ...
  [CODES.ACCOUNT_INELIGIBLE]: 'the credentials work, but this account is not allowed to do this. Changing a key will not help.',
};
...
function environmentNotes(provider) {
  ...
  if (anyFromEnv) {
    // Name the running build where we can. "If you changed it recently" invites
    // the operator to guess; "this deployment was built 3 days ago" lets them
    // check. Falls back to the generic sentence when there is no build stamp.
    let buildLine = '';
    try {
      // Required lazily: this module is loaded by every client, and buildInfo
      // touches the filesystem.
      buildLine = require('./buildInfo').describeRunningBuild();
    } catch (_) {
      buildLine = '';
    }
    notes.push(buildLine ? `${REDEPLOY_NOTE} ${buildLine}` : REDEPLOY_NOTE);
  }
...
function describeCredentialFailure(input = {}) {
  ...
  // UNREACHABLE / ACCOUNT_INELIGIBLE the credentials are not implicated at all
  // — telling someone to redeploy because their Instagram account is personal
  // is the confidently-wrong cause this file exists to stop.
  const CREDENTIALS_NOT_IMPLICATED = [
    CODES.MISSING, CODES.RATE_LIMITED, CODES.UNREACHABLE, CODES.ACCOUNT_INELIGIBLE,
  ];
  if (!CREDENTIALS_NOT_IMPLICATED.includes(code)) {
...
function codeFromHttpStatus(status) {
  ...
  // NB: 400 is deliberately NOT mapped to MISSING here. Our own clients use 400
  // for "credentials are incomplete" (their pre-flight guard), but external
  // providers also return 400 for a key they refuse — YouTube answers an invalid
  // API key with 400, and calling that "not configured" sends the operator
  // looking for a value that is already there. The MISSING translation belongs
  // where the 400 came from our own guard: see fromLegacyResult in
  // lib/credentialVerify.js.
