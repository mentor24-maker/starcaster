...
function codeFromHttpStatus(status) {
  ...
  // Every client in lib/ uses 400 for "credentials are missing/incomplete" —
  // it is their own guard firing before any request goes out, not a provider
  // rejecting a value. Reporting that as REJECTED sends the operator to a
  // developer portal to re-check credentials they never entered.
  if (n === 400) return CODES.MISSING;
