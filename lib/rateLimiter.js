...
const LIMITS = {
  ...
  'blog.import': {
    max:      30,
    windowMs: 60 * 1000,          // blog bulk-import: candidates + batched writes (admin tool)
  },
  ...
  // Assets › Footage previews (routes/studio.js): each one is two calls to
  // Google Drive. The screen fetches them only as rows scroll into view and
  // the browser keeps each for a day, so a normal visit is a few dozen; this
  // stops a runaway loop from spending the Drive quota, not a person scrolling.
  'studio.thumbnail': {
    max:      240,
    windowMs: 60 * 1000,
  },
  ...
  // Reading a schedule flyer with Claude (task 86bbztj0e). Platform-only, and
  // each call is a large PDF or image, so the ceiling is per hour, not minute.
  'events.harvest': {
    max:      12,
    windowMs: 60 * 60 * 1000,     // 12 flyers / hour per client
  },
  'builder.seoAltText': {
    max:      20,
