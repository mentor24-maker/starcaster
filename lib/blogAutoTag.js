...
function buildTagProfiles(posts, options = {}) {
  ...
  for (const g of groups.values()) {
    // Canonical spelling: most posts, then alphabetical — deterministic. A
    // space sorts before any letter, so on a tie the spaced, readable form
    // wins ("round robin" over "roundrobin") without a rule of its own; a
    // rule for it was dead code, proved by a break test that could not fail.
