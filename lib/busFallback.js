...
function renderFallbackSeed() {
  return [
    ...
    '',
    '**This ticket assigns itself to Dane when an alarm lands on it**, so it shows',
    'up in his ClickUp "Assigned to me" list. Saving an alarm is not the same as',
    'delivering one: from 2026-09-16 to 2026-09-19 the pipeline was dead for 90',
    'hours, `npm run throughput` got the answer right and posted it, the party line',
    'refused it, it was saved here — and nobody saw it, because nothing on this',
    'ticket reaches any view anyone opens. Unassign yourself when you have read it.',
    '',
...
function renderRouteLine({ via, channel, why, url, assignedToOperator, assignWhy }) {
  ...
  if (via === 'ticket') {
    return [
      ...
      url ? `  ${url}` : '',
      // SAVED AND SEEN ARE DIFFERENT CLAIMS, and this line used to make only
      // the first one (task 86bc3t0n1). The 90-hour outage of 2026-09-16 was
      // reported correctly by `throughput --check`, saved correctly here, and
      // read by nobody — the ticket sits in `Live` with no assignee, so it
      // appears in no view Dane opens.
      assignedToOperator
        ? 'That ticket is assigned to Dane, so it is in his ClickUp "Assigned to me" list — a route'
          + ' that does not touch the chat API at all. It leaves that list when he unassigns himself.'
        : `NOBODY HAS BEEN PUT ON IT: ${assignWhy || 'the assignment was not attempted'}. The alarm is saved`
