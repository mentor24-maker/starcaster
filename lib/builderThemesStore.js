...
function inputToRow(input) {
  ...
  // A caller that sends `palette` wins; one that round-trips a typography
  // object with palette inside keeps it; otherwise no palette key is written.
  ...
  if (input?.typography && typeof input.typography === 'object') {
    row.typography = { ...input.typography, pageLayout, ...(palette ? { palette } : {}) };
  } else {
    row.typography = { pageLayout, ...(palette ? { palette } : {}) };
  }
