...
function videosTable() { return tableConfig().acquireYoutubeVideos; }
function detailsTable() { return tableConfig().acquireYoutubeDetails; }
function commentsTable() { return tableConfig().acquireYoutubeComments; }
...
/**
 * "The schema isn't ready for this write" — treated as a soft failure so the
 * caller falls back to an in-memory record instead of erroring.
 *
 * Missing *columns* count too, not just missing tables: the media_* columns
 * arrive in a migration, and code deployed ahead of that migration would
 * otherwise fail every upsert and take the whole video repository down with
 * it. Degrading to unsaved-but-working is the right trade there.
 */
function isMissingTableError(result) {
  if (!result || result.ok) return false;
  const text = `${safeText(result.error)} ${JSON.stringify(result.raw || {})}`;
  return /relation .* does not exist/i.test(text)
...
function makeBaseRecord(patch) {
  ...
  return {
    ...
    media_status: safeText(patch.media_status) || 'none',
    media_job_id: safeText(patch.media_job_id),
    media_mp4_url: safeText(patch.media_mp4_url),
    media_mp3_url: safeText(patch.media_mp3_url),
    media_error: safeText(patch.media_error),
...
function mergeRecord(existing, patch) {
  ...
  return {
    ...
    topic: firstNonEmpty(existing.topic, patchHasData.topic),
    ...
    // Media is the one group where a patch outranks what is stored: re-running
    // a download is meant to replace the old file, and every other upsert path
    // (detail/comment/miner runs) simply carries no media fields.
    media_status: firstNonEmpty(patch.media_status, existing.media_status) || 'none',
    media_job_id: firstNonEmpty(patch.media_job_id, existing.media_job_id),
    media_mp4_url: firstNonEmpty(patch.media_mp4_url, existing.media_mp4_url),
    media_mp3_url: firstNonEmpty(patch.media_mp3_url, existing.media_mp3_url),
    media_error: firstNonEmpty(patch.media_error, existing.media_error),
...
async function readStoredYoutubeVideos(limit, scope = null) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 200, 1000));
  const query = await scopedListQuery(
    videosTable(),
    `select=*&order=updated_at.desc&limit=${safeLimit}`,
    scope
  );
...
async function readYoutubeDetailsRows(limit, scope = null) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 400, 2000));
  const query = await scopedListQuery(
    detailsTable(),
    `select=run_id,video_url,video_id,topic,tags,title,channel_name,transcript_status,transcript_source,transcript_provider,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`,
    scope
  );
  return sbQuery({
    method: 'GET',
...
async function readYoutubeCommentRows(limit, scope = null) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 400, 2000));
  const query = await scopedListQuery(
    commentsTable(),
    `select=run_id,video_url,video_id,title,channel_name,comment_count,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`,
    scope
  );
  return sbQuery({
    method: 'GET',
...
async function readYoutubeMinerRows(limit, scope = null) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 120, 600));
  const query = await scopedListQuery(
    commentsTable(),
    `run_id=like.ytminer_*&select=run_id,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`,
    scope
  );
  return sbQuery({
    method: 'GET',
...
async function synthesizeYoutubeVideos(limit = 200, scope = null) {
  const sourceLimit = Math.max(Number(limit) * 4, 200);
  const [detailsRes, commentsRes, minerRes] = await Promise.all([
    readYoutubeDetailsRows(sourceLimit, scope),
    readYoutubeCommentRows(sourceLimit, scope),
    readYoutubeMinerRows(Math.max(Number(limit) * 2, 60), scope),
  ]);
  ...
  patches
    .filter((row) => safeText(row.video_url))
...
function toStoredRow(record) {
  return {
    ...
    media_status: safeText(record.media_status) || 'none',
    media_job_id: safeText(record.media_job_id),
    media_mp4_url: safeText(record.media_mp4_url),
    media_mp3_url: safeText(record.media_mp3_url),
    media_error: safeText(record.media_error),
    created_at: toIso(record.created_at) || new Date().toISOString(),
    updated_at: toIso(record.updated_at) || new Date().toISOString(),
  };
}

async function upsertStoredYoutubeVideo(record, scope = null) {
  const baseRow = toStoredRow(record);
  if (!baseRow.video_record_id || !baseRow.video_url) {
    return { ok: false, status: 400, error: 'video record is incomplete' };
  }
  const row = await scopedInsertRow(videosTable(), baseRow, scope);
  return sbQuery({
    method: 'POST',
    table: videosTable(),
...
async function trySyncRecords(records, scope = null) {
  ...
  for (const record of rows) {
    payload.push(await scopedInsertRow(videosTable(), toStoredRow(record), scope));
  }
  const res = await sbQuery({
    method: 'POST',
    table: videosTable(),
...
async function listYoutubeVideos(limit = 200, scope = null) {
  const storedRes = await readStoredYoutubeVideos(limit, scope);
  if (!storedRes.ok && !isMissingTableError(storedRes)) return storedRes;
  const synthesizedRes = await synthesizeYoutubeVideos(limit, scope);
  ...
  if (storedRes.ok) {
    storedRes.data.forEach((row) => {
      ...
      // Pass the DB row first so its manual overrides take precedence over synthesized data
      map.set(key, mergeRecord(row, map.get(key)));
    });
  } else {
    await trySyncRecords(synthesizedRes.data, scope);
  }
...
async function getYoutubeVideo(videoRecordId, scope = null) {
  const id = safeText(videoRecordId);
  if (!id) return { ok: false, status: 400, error: 'video_record_id is required' };
  const listRes = await listYoutubeVideos(1000, scope);
...
async function updateYoutubeVideo(videoRecordId, patch, scope = null) {
  const id = safeText(videoRecordId);
  if (!id) return { ok: false, status: 400, error: 'video_record_id is required' };
  const existingRes = await getYoutubeVideo(id, scope);
  ...
  const storedRes = await upsertStoredYoutubeVideo(merged, scope);
...
async function deleteYoutubeVideo(videoRecordId, scope = null) {
  const id = safeText(videoRecordId);
  if (!id) return { ok: false, status: 400, error: 'video_record_id is required' };
  const query = await scopedIdQuery(
    videosTable(),
    `video_record_id=eq.${encodeURIComponent(id)}`,
    scope
  );
  const res = await sbQuery({
    method: 'DELETE',
...
async function upsertYoutubeVideoFromDetailRun(row, scope = null) {
  return trySyncRecords([patchFromDetailRun(row)], scope);
}

async function upsertYoutubeVideoFromCommentRun(row, scope = null) {
  return trySyncRecords([patchFromCommentRun(row)], scope);
}

async function upsertYoutubeVideosFromMinerRun(row, scope = null) {
  return trySyncRecords(patchesFromMinerRun(row), scope);
}
