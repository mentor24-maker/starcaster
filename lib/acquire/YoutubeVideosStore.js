...
async function readYoutubeDetailsRows(limit) {
  ...
  return sbQuery({
    method: 'GET',
    table: detailsTable(),
    query: `select=run_id,video_url,video_id,topic,tags,title,channel_name,transcript_status,transcript_source,transcript_provider,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`,
  });
}
...
async function readYoutubeMinerRows(limit) {
  ...
  return sbQuery({
    method: 'GET',
    table: commentsTable(),
    query: `run_id=like.ytminer_*&select=run_id,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`,
  });
}
