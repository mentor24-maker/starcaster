...
async function createYoutubeHarvestRun(input, result) {
  ...
  await upsertYoutubeVideoFromDetailRun(created).catch(() => null);
  return {
    ok: true,
    status: 201,
    data: created
...
async function listYoutubeHarvestRuns(limit = 20) {
  ...
  const res = await supabaseRequest({
    method: 'GET',
    table: cfg.table,
    query: `select=run_id,video_url,video_id,topic,tags,title,channel_name,transcript_status,transcript_source,transcript_provider,request_json,result_json,created_at,updated_at&order=created_at.desc&limit=${safeLimit}`
  });
  if (!res.ok) return res;
  return {
    ok: true,
...
async function updateYoutubeHarvestRun(runId, patch) {
  ...
  return supabaseRequest({
    ...
    upsertYoutubeVideoFromDetailRun(updated).catch(() => null);
