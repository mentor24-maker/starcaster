 ...
 * Acquire comments from a YouTube video using the YouTube Data API v3.
 *
 ...
 *
 * ── Usage ─────────────────────────────────────────────────────────────────
 *   const { runYoutubeCommentAcquire } = require('./youtubeComments');
...
// ---------------------------------------------------------------------------
// URL helpers (reuse the same pattern as YoutubeDetailsRun.js)
// ---------------------------------------------------------------------------
...
/**
 * Acquire comments from a YouTube video.
 *
 * @param {object} input
 ...
 */
async function runYoutubeCommentAcquire(input) {
  ...
  if (includeReplies) {
    ...
    for (const threadId of threadsWithReplies) {
      try {
        ...
        // Don't let one failed reply thread abort the whole acquire run
...
module.exports = { runYoutubeCommentAcquire };
