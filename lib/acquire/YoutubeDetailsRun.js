...
function extractMetaContent(html, name) {
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) return decodeHtmlEntity(match[1]).trim();
  }
  return '';
}
  ...
  try {
    const response = await fetch(endpoint, {
      headers: {
        ...
        'user-agent': 'APH-Youtube-Acquire/1.0'
      },
...
async function runYoutubeAcquire(input) {
  ...
  const fallbackVideoId = videoIdFromWatchUrl(watchUrl);
  const title = String(
    videoDetails.title ||
  ...
  const videoId = fallbackVideoId || String(videoDetails.videoId || '');
  ...
  return {
    video: {
      url: watchUrl,
      id: String(videoDetails.videoId || videoId || ''),
      title,
      description,
    ...
    channel_owner: {
      name: String(videoDetails.author || oembed?.author_name || '').trim(),
      channel_id: String(videoDetails.channelId || '').trim(),
