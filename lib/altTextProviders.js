 ...
 *
 * That fork used to mean AI spend here was invisible. It is metered as of
 * 2026-08-17 via lib/aiUsage.js, which sits beside each fetch() rather than in a
 * shared client — precisely because forks like this one keep happening. Folding
 * these providers back into a shared client, with JSON as an opt-in per call
 * rather than a hardcoded default, is still tracked work.
 *
 * This runs once per image, so a page of 30 images is 30 metered calls.
 *
...
async function queryAnthropicPlain(prompt, scope) {
  ...
  await recordAiUsage({ provider: 'anthropic', model: ANTHROPIC_MODEL, feature: FEATURE, body: data, scope });
  return data?.content?.[0]?.text || '';
}

async function queryOpenAIPlain(prompt, scope) {
  ...
  if (!res.ok) return null;
  const data = await res.json();
  await recordAiUsage({ provider: 'openai', model: OPENAI_MODEL, feature: FEATURE, body: data, scope });
  return data?.choices?.[0]?.message?.content || '';
}

async function queryGeminiPlain(prompt, scope) {
  ...
  if (!res.ok) return null;
  const data = await res.json();
  await recordAiUsage({ provider: 'gemini', model: GEMINI_MODEL, feature: FEATURE, body: data, scope });
