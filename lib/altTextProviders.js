 ...
 *
 * This is deliberately separate from lib/rogerClient.js. Roger's query helpers
 * wrap every call in the Tri-Agent JSON persona and force JSON-object output —
 * both wrong for "write one sentence of alt text". So the SEO tool gets its own
 * clean call here, ordered Anthropic → OpenAI → Gemini (the operator's chosen
 * order), using a fast/cheap model tier because it fires once per image.
 *
