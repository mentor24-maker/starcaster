...
async function updateProjectForUser(projectIdInput, patch, userIdInput) {
  ...
  if (patch && Object.prototype.hasOwnProperty.call(patch, 'supportAlertEmail')) {
    // Merged rather than assigned: site_settings is one jsonb column shared
    // with the tenant's own contactAlertEmail, so writing this key alone would
    // erase theirs.
    const currentBlob = await readCurrentSiteSettingsBlob(projectId);
    const merged = mergePlatformSiteSettings(currentBlob, {
      supportAlertEmail: patch.supportAlertEmail,
    });
