...
const {
  ...
  readRawSiteSettings,
  PLATFORM_SETTINGS,
  SITE_SETTINGS_COLUMN,
} = require('./projectSiteSettingsStore');
...
function projectRowToModel(row) {
  return {
    ...
    // Platform-only settings, surfaced on Settings > Projects > Edit. Safe to
    // include here: every caller of this model holds a platform session, and
    // /api/projects is denied to tenant-admin sessions.
    ...readPlatformSettingsFromRow(row),
...
async function resolveCurrentProject(input) {
  ...
  const autoSelectFirst = input?.autoSelectFirst === true;
  ...
  if (!projects.length && autoCreateDefault) {
    // The slug MUST be per-user. Slugs are globally unique, so asking for the
    // name alone made every default project want the slug "default-project":
    // the first user to reach this path took it, and every user created after
    // that got a 409 here, silently ended up with no project at all, and saw
    // "Active project is required" on every screen. Same shape as
    // virtualDefaultProject().
    const createResult = await createProjectForUser(
      { name: 'Default Project', slug: `default-${userId}` },
      userId
    );
    if (createResult.ok) {
      ...
      // Still non-fatal — the caller falls through to "no project" rather than
      // failing the whole boot — but it must not be invisible: this is the
      // difference between a working account and one that cannot use any
      // screen, and it left no trace anywhere for weeks.
      console.error(
        `[projects] auto-create of a default project failed for user ${userId}:`,
        createResult.error || createResult.status
      );
    }
  }
  ...
  // Last resort, and ONLY for the explicit boot call (/api/projects/current).
  // A freshly created session has no pinned project, and a browser with no
  // cached id sends no X-Project-ID header — so without this a user who owns
  // projects still resolves to none, and every scoped API call answers
  // "Active project is required". Never enabled on the per-request middleware
  // path, which must not guess a scope for data reads and writes.
  if (!project && autoSelectFirst) {
    // Prefer a real project over the auto-created "Default Project" so users
    // don't land in an empty workspace while their own work sits next to it.
    ...
    project = preferred || projects[0] || null;
    if (project) resolvedFrom = 'auto';
  }
...
/**
 * Current site_settings blob for a project, from whichever store is in play.
 * Returns {} rather than failing: a missing or unreadable blob just means
 * there is nothing to merge over.
 */
async function readCurrentSiteSettingsBlob(projectId) {
  if (useSupabase()) {
    const result = await readRawSiteSettings(projectId);
    return result.ok ? result.data : {};
  }
  const store = readFileStore();
  const row = store.projects.find((r) => safeText(r.id) === projectId);
  const blob = row?.[SITE_SETTINGS_COLUMN];
...
async function updateProjectForUser(projectIdInput, patch, userIdInput) {
  ...
  // Platform-only site settings (support alert/display email, support phone).
  // Driven off PLATFORM_SETTINGS so adding another needs no change here.
  const platformPatch = {};
  if (patch) {
    for (const key of Object.keys(PLATFORM_SETTINGS)) {
      if (Object.prototype.hasOwnProperty.call(patch, key)) platformPatch[key] = patch[key];
    }
  }
  if (Object.keys(platformPatch).length) {
    // Merged rather than assigned: site_settings is one jsonb column shared
    // with the tenant's own contactAlertEmail, so writing these keys alone
    // would erase theirs.
    const currentBlob = await readCurrentSiteSettingsBlob(projectId);
    const merged = mergePlatformSiteSettings(currentBlob, platformPatch);
    if (!merged.ok) return merged;
    updates[SITE_SETTINGS_COLUMN] = merged.data;
  }
