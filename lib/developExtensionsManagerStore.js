...
function table() {
  return tableConfig().developExtensionsManager;
}
...
async function getManagerConfig() {
  const res = await sbQuery({
    method: 'GET',
    table: table(),
    query: 'select=*&manager_key=eq.default&limit=1',
  });
...
async function saveManagerConfig(input) {
  ...
  const updated = await sbQuery({
    method: 'PATCH',
    table: table(),
    query: `id=eq.${Number(row.id || 0) || 0}&select=*`,
    headers: { Prefer: 'return=representation' },
    body: payload,
  });
