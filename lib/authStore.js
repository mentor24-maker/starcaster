'use strict';

const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const { sbQuery, isConfigured: isSupabaseConfigured } = require('./supabase');
const { writeJsonAtomic, ensureJsonFile } = require('./localDataFs');

const USERS_FILE = path.join(__dirname, '..', 'data', 'auth_users.json');
const SESSIONS_FILE = path.join(__dirname, '..', 'data', 'auth_sessions.json');
const SESSION_TTL_MS = 14 * 24 * 60 * 60 * 1000; // 14 days

const AUTH_USERS_TABLE = String(process.env.SUPABASE_AUTH_USERS_TABLE || 'app_auth_users').trim();
const AUTH_SESSIONS_TABLE = String(process.env.SUPABASE_AUTH_SESSIONS_TABLE || 'app_auth_sessions').trim();

function useSupabase() {
  return isSupabaseConfigured();
}

function ensureFile(filePath, seed) {
  ensureJsonFile(filePath, seed, { mode: 0o600 });
}

function readJson(filePath, fallback) {
  try {
    ensureFile(filePath, fallback);
    const raw = fs.readFileSync(filePath, 'utf8');
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === 'object' ? parsed : fallback;
  } catch {
    return fallback;
  }
}

function writeJson(filePath, data) {
  ensureFile(filePath, data);
  writeJsonAtomic(filePath, data, { mode: 0o600 });
}

function safeText(value) {
  return String(value || '').trim();
}

function normalizeEmail(value) {
  return safeText(value).toLowerCase();
}

function makeId(prefix) {
  return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}

function hashPassword(passwordInput, saltInput = '') {
  const password = String(passwordInput || '');
  const salt = saltInput || crypto.randomBytes(16).toString('hex');
  const derived = crypto.scryptSync(password, salt, 64).toString('hex');
  return `${salt}:${derived}`;
}

function verifyPassword(passwordInput, storedHash) {
  const text = safeText(storedHash);
  if (!text.includes(':')) return false;
  const [salt, existing] = text.split(':');
  if (!salt || !existing) return false;
  const next = hashPassword(passwordInput, salt).split(':')[1];
  try {
    return crypto.timingSafeEqual(Buffer.from(existing, 'hex'), Buffer.from(next, 'hex'));
  } catch {
    return false;
  }
}

function sanitizeUser(user) {
  return {
    id: safeText(user?.id),
    email: normalizeEmail(user?.email),
    name: safeText(user?.name),
    role: safeText(user?.role) || 'user',
    createdAt: safeText(user?.created_at || user?.createdAt),
  };
}

function listUsersFile() {
  const store = readJson(USERS_FILE, { users: [] });
  const users = Array.isArray(store.users) ? store.users : [];
  return users.filter((user) => user && typeof user === 'object');
}

async function findUserByEmail(emailInput) {
  const email = normalizeEmail(emailInput);
  if (!email) return null;
  if (useSupabase()) {
    const res = await findUserByEmailSupabase(email);
    return res.ok && res.row ? sanitizeUser(res.row) : null;
  }
  const users = listUsersFile();
  const user = users.find((row) => normalizeEmail(row.email) === email);
  return user ? sanitizeUser(user) : null;
}

function writeUsersFile(users) {
  writeJson(USERS_FILE, { users });
}

function listSessionsFile() {
  const store = readJson(SESSIONS_FILE, { sessions: [] });
  const sessions = Array.isArray(store.sessions) ? store.sessions : [];
  return sessions.filter((row) => row && typeof row === 'object');
}

function writeSessionsFile(sessions) {
  writeJson(SESSIONS_FILE, { sessions });
}

async function countUsersSupabase() {
  const query = 'select=id&limit=1';
  const result = await sbQuery({ method: 'GET', table: AUTH_USERS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to query users' };
  return { ok: true, count: Array.isArray(result.data) ? result.data.length : 0 };
}

async function findUserByEmailSupabase(email) {
  const query = `select=*&email=eq.${encodeURIComponent(email)}&limit=1`;
  const result = await sbQuery({ method: 'GET', table: AUTH_USERS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to query users' };
  const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
  return { ok: true, row };
}

async function findUserByIdSupabase(userId) {
  const query = `select=*&id=eq.${encodeURIComponent(userId)}&limit=1`;
  const result = await sbQuery({ method: 'GET', table: AUTH_USERS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to query users' };
  const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
  return { ok: true, row };
}

async function insertUserSupabase(user) {
  const result = await sbQuery({
    method: 'POST',
    table: AUTH_USERS_TABLE,
    body: user,
    headers: { Prefer: 'return=representation' },
  });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to create user' };
  const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
  return { ok: true, row };
}

async function updateUserPasswordSupabase(userId, newPasswordHash) {
  const result = await sbQuery({
    method: 'PATCH',
    table: AUTH_USERS_TABLE,
    query: `id=eq.${encodeURIComponent(userId)}`,
    body: { password_hash: newPasswordHash, updated_at: new Date().toISOString() },
  });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to update password' };
  return { ok: true };
}

async function insertSessionSupabase(session) {
  const result = await sbQuery({
    method: 'POST',
    table: AUTH_SESSIONS_TABLE,
    body: session,
    headers: { Prefer: 'return=representation' },
  });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to create session' };
  const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
  return { ok: true, row };
}

async function findSessionByTokenSupabase(token) {
  const query = `select=*&token=eq.${encodeURIComponent(token)}&limit=1`;
  const result = await sbQuery({ method: 'GET', table: AUTH_SESSIONS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to query sessions' };
  const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
  return { ok: true, row };
}

async function deleteSessionByTokenSupabase(token) {
  const query = `token=eq.${encodeURIComponent(token)}`;
  const result = await sbQuery({ method: 'DELETE', table: AUTH_SESSIONS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to delete session' };
  return { ok: true };
}

async function updateSessionActiveProjectSupabase(token, projectIdInput) {
  const projectId = safeText(projectIdInput);
  const body = { active_project_id: projectId || null };
  const query = `token=eq.${encodeURIComponent(token)}`;
  const result = await sbQuery({
    method: 'PATCH',
    table: AUTH_SESSIONS_TABLE,
    query,
    body,
  });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to update session project' };
  return { ok: true };
}

async function createUser({ email, password, name }) {
  const userEmail = normalizeEmail(email);
  const userName = safeText(name);
  const pwd = String(password || '');
  if (!userEmail) return { ok: false, status: 400, error: 'Email is required' };
  if (!pwd || pwd.length < 8) return { ok: false, status: 400, error: 'Password must be at least 8 characters' };
  if (!userName) return { ok: false, status: 400, error: 'Name is required' };

  if (useSupabase()) {
    const existing = await findUserByEmailSupabase(userEmail);
    if (!existing.ok) return existing;
    if (existing.row) return { ok: false, status: 409, error: 'An account with that email already exists' };

    const count = await countUsersSupabase();
    if (!count.ok) return { ok: false, status: count.status || 500, error: count.error || 'Unable to determine first user' };

    const now = new Date().toISOString();
    const user = {
      id: makeId('usr'),
      email: userEmail,
      name: userName,
      password_hash: hashPassword(pwd),
      role: count.count === 0 ? 'owner' : 'user',
      created_at: now,
      updated_at: now,
    };
    const inserted = await insertUserSupabase(user);
    if (!inserted.ok) return inserted;
    return { ok: true, status: 201, data: sanitizeUser(inserted.row || user) };
  }

  const users = listUsersFile();
  if (users.some((user) => normalizeEmail(user.email) === userEmail)) {
    return { ok: false, status: 409, error: 'An account with that email already exists' };
  }

  const now = new Date().toISOString();
  const user = {
    id: makeId('usr'),
    email: userEmail,
    name: userName,
    passwordHash: hashPassword(pwd),
    role: users.length === 0 ? 'owner' : 'user',
    createdAt: now,
    updatedAt: now,
  };
  users.unshift(user);
  writeUsersFile(users);
  return { ok: true, status: 201, data: sanitizeUser(user) };
}

async function authenticateUser({ email, password }) {
  const userEmail = normalizeEmail(email);
  const pwd = String(password || '');
  if (!userEmail || !pwd) return { ok: false, status: 400, error: 'Email and password are required' };

  if (useSupabase()) {
    const found = await findUserByEmailSupabase(userEmail);
    if (!found.ok) return found;
    const user = found.row;
    if (!user) return { ok: false, status: 401, error: 'Invalid email or password' };
    if (!verifyPassword(pwd, user.password_hash)) return { ok: false, status: 401, error: 'Invalid email or password' };
    return { ok: true, status: 200, data: sanitizeUser(user) };
  }

  const users = listUsersFile();
  const user = users.find((row) => normalizeEmail(row.email) === userEmail);
  if (!user) return { ok: false, status: 401, error: 'Invalid email or password' };
  if (!verifyPassword(pwd, user.passwordHash)) return { ok: false, status: 401, error: 'Invalid email or password' };
  return { ok: true, status: 200, data: sanitizeUser(user) };
}

async function updateUserPassword({ email, newPassword }) {
  const userEmail = normalizeEmail(email);
  const pwd = String(newPassword || '');
  if (!userEmail || !pwd || pwd.length < 8) return { ok: false, status: 400, error: 'Valid email and new password (min 8 chars) required' };

  if (useSupabase()) {
    const found = await findUserByEmailSupabase(userEmail);
    if (!found.ok) return found;
    const user = found.row;
    if (!user) return { ok: false, status: 404, error: 'User not found' };

    const updated = await updateUserPasswordSupabase(user.id, hashPassword(pwd));
    return updated;
  }

  const users = listUsersFile();
  const index = users.findIndex((row) => normalizeEmail(row.email) === userEmail);
  if (index === -1) return { ok: false, status: 404, error: 'User not found' };

  users[index].passwordHash = hashPassword(pwd);
  users[index].updatedAt = new Date().toISOString();
  writeUsersFile(users);
  return { ok: true, status: 200 };
}

async function createSession(userIdInput, overrideToken = null, ttlMs = SESSION_TTL_MS) {
  const userId = safeText(userIdInput);
  if (!userId) return { ok: false, status: 400, error: 'user_id is required' };

  if (useSupabase()) {
    const userResult = await findUserByIdSupabase(userId);
    if (!userResult.ok) return userResult;
    if (!userResult.row) return { ok: false, status: 404, error: 'User not found' };

    const token = overrideToken || crypto.randomBytes(32).toString('hex');
    const now = Date.now();
    const session = {
      token,
      user_id: userId,
      created_at: new Date(now).toISOString(),
      expires_at: new Date(now + ttlMs).toISOString(),
    };
    const inserted = await insertSessionSupabase(session);
    if (!inserted.ok) return inserted;

    return {
      ok: true,
      status: 201,
      data: {
        token,
        userId,
        createdAt: session.created_at,
        expiresAt: session.expires_at,
      },
    };
  }

  const users = listUsersFile();
  const user = users.find((row) => safeText(row.id) === userId);
  if (!user) return { ok: false, status: 404, error: 'User not found' };

  const token = overrideToken || crypto.randomBytes(32).toString('hex');
  const now = Date.now();
  const session = {
    token,
    userId,
    createdAt: new Date(now).toISOString(),
    expiresAt: new Date(now + ttlMs).toISOString(),
  };
  const sessions = listSessionsFile().filter((row) => new Date(String(row.expiresAt || 0)).getTime() > now);
  sessions.unshift(session);
  writeSessionsFile(sessions);
  return { ok: true, status: 201, data: session };
}

async function deleteSession(tokenInput) {
  const token = safeText(tokenInput);
  if (!token) return { ok: true, status: 200, data: { deleted: false } };

  if (useSupabase()) {
    const result = await deleteSessionByTokenSupabase(token);
    if (!result.ok) return result;
    return { ok: true, status: 200, data: { deleted: true } };
  }

  const sessions = listSessionsFile();
  const next = sessions.filter((row) => safeText(row.token) !== token);
  writeSessionsFile(next);
  return { ok: true, status: 200, data: { deleted: sessions.length !== next.length } };
}

async function getAuthSession(tokenInput) {
  const token = safeText(tokenInput);
  if (!token) return null;

  if (useSupabase()) {
    const session = await findSessionByTokenSupabase(token);
    if (!session.ok) {
      console.error('[authStore] findSessionByTokenSupabase error:', session.error);
      return null;
    }
    if (!session.row) return null;
    const expiresAtMs = new Date(String(session.row.expires_at || 0)).getTime();
    if (!Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()) return null;

    const userResult = await findUserByIdSupabase(safeText(session.row.user_id));
    if (!userResult.ok) {
      console.error('[authStore] findUserByIdSupabase error:', userResult.error);
      return null;
    }
    if (!userResult.row) return null;

    return {
      token,
      userId: safeText(session.row.user_id),
      user: sanitizeUser(userResult.row),
      activeProjectId: safeText(session.row.active_project_id),
      expiresAt: String(session.row.expires_at || ''),
    };
  }

  const now = Date.now();
  const sessions = listSessionsFile();
  const active = sessions.find((row) => safeText(row.token) === token && new Date(String(row.expiresAt || 0)).getTime() > now);
  if (!active) return null;
  const users = listUsersFile();
  const user = users.find((row) => safeText(row.id) === safeText(active.userId));
  if (!user) return null;

  return {
    token,
    userId: safeText(active.userId),
    user: sanitizeUser(user),
    activeProjectId: safeText(active.activeProjectId),
    expiresAt: String(active.expiresAt || ''),
  };
}

async function getUserFromSessionToken(tokenInput) {
  const session = await getAuthSession(tokenInput);
  return session?.user || null;
}

async function setSessionActiveProject(tokenInput, projectIdInput) {
  const token = safeText(tokenInput);
  const projectId = safeText(projectIdInput);
  if (!token) return { ok: false, status: 401, error: 'Session token is required' };

  if (useSupabase()) {
    const session = await findSessionByTokenSupabase(token);
    if (!session.ok) return { ok: false, status: session.status || 500, error: session.error || 'Unable to read session' };
    if (!session.row) return { ok: false, status: 401, error: 'Session not found' };
    const expiresAtMs = new Date(String(session.row.expires_at || 0)).getTime();
    if (!Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()) {
      return { ok: false, status: 401, error: 'Session expired' };
    }
    return updateSessionActiveProjectSupabase(token, projectId);
  }

  const now = Date.now();
  const sessions = listSessionsFile();
  const index = sessions.findIndex((row) => safeText(row.token) === token && new Date(String(row.expiresAt || 0)).getTime() > now);
  if (index < 0) return { ok: false, status: 401, error: 'Session not found' };
  sessions[index] = {
    ...sessions[index],
    activeProjectId: projectId || '',
  };
  writeSessionsFile(sessions);
  return { ok: true, status: 200, data: { activeProjectId: projectId || '' } };
}

async function listUsersSupabase() {
  const query = 'select=id,email,name,role,created_at,updated_at&order=created_at.asc';
  const result = await sbQuery({ method: 'GET', table: AUTH_USERS_TABLE, query });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to list users' };
  return { ok: true, rows: Array.isArray(result.data) ? result.data : [] };
}

async function listUsers() {
  if (useSupabase()) {
    const res = await listUsersSupabase();
    if (!res.ok) return { ok: false, status: res.status, error: res.error };
    return { ok: true, data: res.rows.map(sanitizeUser) };
  }
  const users = listUsersFile();
  return { ok: true, data: users.map(sanitizeUser) };
}

async function getUserById(userIdInput) {
  const userId = safeText(userIdInput);
  if (!userId) return { ok: false, status: 400, error: 'User ID is required' };
  if (useSupabase()) {
    const res = await findUserByIdSupabase(userId);
    if (!res.ok) return res;
    if (!res.row) return { ok: false, status: 404, error: 'User not found' };
    return { ok: true, data: sanitizeUser(res.row) };
  }
  const users = listUsersFile();
  const user = users.find((row) => safeText(row.id) === userId);
  if (!user) return { ok: false, status: 404, error: 'User not found' };
  return { ok: true, data: sanitizeUser(user) };
}

async function updateUser(userIdInput, patch) {
  const userId = safeText(userIdInput);
  if (!userId) return { ok: false, status: 400, error: 'User ID is required' };
  if (useSupabase()) {
    const allowedPatch = { updated_at: new Date().toISOString() };
    if (patch.name !== undefined) allowedPatch.name = safeText(patch.name);
    if (patch.role !== undefined) allowedPatch.role = safeText(patch.role);
    const result = await sbQuery({
      method: 'PATCH',
      table: AUTH_USERS_TABLE,
      query: `id=eq.${encodeURIComponent(userId)}`,
      body: allowedPatch,
      headers: { Prefer: 'return=representation' },
    });
    if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to update user' };
    const row = Array.isArray(result.data) ? (result.data[0] || null) : null;
    if (!row) return { ok: false, status: 404, error: 'User not found' };
    return { ok: true, data: sanitizeUser(row) };
  }
  const users = listUsersFile();
  const index = users.findIndex((row) => safeText(row.id) === userId);
  if (index === -1) return { ok: false, status: 404, error: 'User not found' };
  if (patch.name !== undefined) users[index].name = safeText(patch.name);
  if (patch.role !== undefined) users[index].role = safeText(patch.role);
  users[index].updatedAt = new Date().toISOString();
  writeUsersFile(users);
  return { ok: true, data: sanitizeUser(users[index]) };
}

async function deleteUser(userIdInput) {
  const userId = safeText(userIdInput);
  if (!userId) return { ok: false, status: 400, error: 'User ID is required' };
  if (useSupabase()) {
    const result = await sbQuery({
      method: 'DELETE',
      table: AUTH_USERS_TABLE,
      query: `id=eq.${encodeURIComponent(userId)}`,
    });
    if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to delete user' };
    return { ok: true, data: { deleted: true, id: userId } };
  }
  const users = listUsersFile();
  const index = users.findIndex((row) => safeText(row.id) === userId);
  if (index === -1) return { ok: false, status: 404, error: 'User not found' };
  users.splice(index, 1);
  writeUsersFile(users);
  return { ok: true, data: { deleted: true, id: userId } };
}

module.exports = {
  createUser,
  authenticateUser,
  createSession,
  deleteSession,
  getAuthSession,
  getUserFromSessionToken,
  setSessionActiveProject,
  updateUserPassword,
  findUserByEmail,
  listUsers,
  getUserById,
  updateUser,
  deleteUser,
  useSupabase,
  AUTH_USERS_TABLE,
  AUTH_SESSIONS_TABLE,
};
