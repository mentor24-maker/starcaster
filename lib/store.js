...
async function createSegment(segment, scope = null) {
  const row = await scopedInsertRow(t().segments, {
    ...
    segment_type: segment.segment_type || 'email_list'
  }, scope);
  return sbRequest({
    method: 'POST',
    table: t().segments,
...
async function updateSegment(id, patch, scope = null) {
  ...
  if (patch.rules !== undefined) rowPatch.rules = patch.rules;
  if (patch.definition !== undefined) rowPatch.definition = patch.definition;
  if (patch.segment_type !== undefined) rowPatch.segment_type = patch.segment_type;
  rowPatch.updated_at = new Date().toISOString();

  const scopedPatch = await scopedPatchRow(t().segments, rowPatch, scope);
...
async function deleteCampaign(id, scope = null) {
  const query = await scopedIdQuery(t().campaigns, `id=eq.${encodeURIComponent(id)}`, scope);
  return sbRequest({
    method: 'DELETE',
    table: t().campaigns,
    query
