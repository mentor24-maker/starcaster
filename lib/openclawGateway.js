...
function incompatibilityMessage(action, status, parsed) {
  ...
  if (action === 'create_job' && status === 405) {
    return 'The configured OpenClaw gateway is running, but it does not support the legacy APP job API (`POST /api/v1/jobs`). This OpenClaw build is protocol-incompatible with the current APP OpenClaw relay.';
  }
  if (action === 'job_status' && status === 405) {
    return 'The configured OpenClaw gateway is running, but it does not support the legacy APP job status API. This OpenClaw build is protocol-incompatible with the current APP OpenClaw relay.';
  }
  return detail || '';
}
