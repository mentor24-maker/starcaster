...
const PLATFORM_SPECS = {
  ...
  facebook: {
    ...
    humanGates: [
      { key: 'meta_app_ready', label: 'Meta app ID/secret configured (Vercel env or Settings > APIs)' },
      { key: 'saved_credentials', label: 'Facebook Page connected via OAuth or saved credentials' },
  ...
  facebook_personal: {
    ...
    provider: 'openclaw',
    requiredLocalFields: ['base_url'],
    recommendedLocalFields: ['api_key'],
    humanGates: [
      { key: 'openclaw_gateway_ready', label: 'OpenClaw gateway is configured and reachable' },
      { key: 'browser_profile_ready', label: 'OpenClaw browser profile is logged into Facebook' },
      { key: 'channel_profile_mapped', label: 'Facebook Personal channel has OpenClaw Profile set' },
      { key: 'completed_smoke_test', label: 'Completed preview + approve smoke test from Promote > Social' },
    ],
    playbook: [
      {
        code: 'FB_PERSONAL_CHECKPOINT',
        title: 'Facebook security checkpoint',
        likelyCause: 'Meta flagged the browser session and requires manual verification.',
        fixSteps: [
          'Open the OpenClaw browser profile and complete the checkpoint manually.',
          'Retry preview from Promote > Social.',
        ],
        portalUrl: 'https://www.facebook.com/',
      },
      {
        code: 'FB_PERSONAL_2FA',
        title: 'Two-factor authentication required',
        likelyCause: 'Facebook session expired or needs a human login step.',
        fixSteps: [
          'Log into Facebook inside the OpenClaw profile.',
          'Confirm the session persists, then queue preview again.',
        ],
        portalUrl: 'https://www.facebook.com/',
      },
      {
        code: 'FB_PERSONAL_SELECTOR',
        title: 'Composer UI changed',
        likelyCause: 'Facebook DOM selectors in the OpenClaw job no longer match.',
        fixSteps: [
          'Update the OpenClaw engage.facebook.personal_post playbook.',
          'Retry preview after selectors are fixed.',
        ],
        portalUrl: 'https://github.com/openclaw/openclaw',
      },
    ],
  },
  instagram: {
...
function computeNextAction(platform, readiness, values, gates) {
  ...
  if (platform === 'facebook') {
    if (!String(values?.app_id || '').trim() || !String(values?.app_secret || '').trim()) {
      return 'Set Meta app_id and app_secret in Vercel env or Settings > APIs > Meta.';
    }
    if (!String(values?.access_token || '').trim() || !String(values?.page_id || '').trim()) {
      return 'Click Connect Facebook Page in Connection Ops, or save page access_token and page_id manually.';
    }
    if (!gates?.saved_credentials) return 'Mark "Saved credentials" gate complete.';
  ...
  if (platform === 'facebook_personal') {
    if (!String(values?.base_url || '').trim()) {
      return 'Save OpenClaw gateway base URL in Settings > APIs > OpenClaw.';
    }
    if (!gates?.openclaw_gateway_ready) return 'Mark OpenClaw gateway ready after confirming the gateway is reachable.';
    if (!gates?.browser_profile_ready) return 'Log into Facebook in the OpenClaw browser profile and mark the gate complete.';
    if (!gates?.channel_profile_mapped) return 'Create a Facebook Personal channel with OpenClaw Profile set.';
    if (!gates?.completed_smoke_test) return 'Run a preview + approve smoke test from Promote > Social.';
    if (readiness.level === 'green') return 'Ready for Facebook Personal browser publishing.';
  }
...
async function getConnectionOps(platformInput, scope = null) {
  ...
  let values = getProviderValues(spec.provider);
  if (platform === 'facebook') {
    const projectId = String(scope?.projectId || '').trim();
    if (projectId) {
      const cred = await projectSocialCredentialsStore.getFacebookPage(projectId);
      if (cred?.pageId && cred?.hasAccessToken) {
        values = {
          ...values,
          page_id: cred.pageId,
          access_token: 'oauth_connected',
        };
      }
    }
  }
