...
const { getProviderValues, testApiProviderConfig } = require('./apiSettings');
...
function ensureStoreFile() {
  ...
  if (!fs.existsSync(STORE_FILE)) {
    fs.writeFileSync(STORE_FILE, JSON.stringify({ platforms: {} }, null, 2), { mode: 0o600 });
...
function ensurePlatformNode(store, platform) {
  if (!store.platforms[platform] || typeof store.platforms[platform] !== 'object') {
  ...
  if (!store.platforms[platform].gates || typeof store.platforms[platform].gates !== 'object') {
    store.platforms[platform].gates = {};
  }
  if (!Array.isArray(store.platforms[platform].attempts)) {
    store.platforms[platform].attempts = [];
  }
...
function getConnectionOps(platformInput) {
  const platform = normalizePlatform(platformInput);
  const spec = PLATFORM_SPECS[platform];
  if (!spec) return { ok: false, status: 400, error: 'Unsupported platform' };

  const store = readStoreSync();
  const node = ensurePlatformNode(store, platform);
  const values = getProviderValues(spec.provider);
  const configTest = testApiProviderConfig(spec.provider);
  ...
  return {
    ...
    data: {
      ...
      latestAttempt,
      attempts,
      providerTest: configTest?.data || null,
    },
  };
}

function updateGate(platformInput, gateKeyInput, doneInput) {
  const platform = normalizePlatform(platformInput);
  const spec = PLATFORM_SPECS[platform];
  if (!spec) return { ok: false, status: 400, error: 'Unsupported platform' };
  ...
  return getConnectionOps(platform);
}

function addAttempt(platformInput, input = {}) {
  const platform = normalizePlatform(platformInput);
  const spec = PLATFORM_SPECS[platform];
  if (!spec) return { ok: false, status: 400, error: 'Unsupported platform' };
