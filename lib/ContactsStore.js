...
function contactToRow(data, { includeUndefined = false } = {}) {
  ...
  return {
    ...set('contactType',  'contact_type',  v => VALID_TYPES.has(v) ? v : 'lead'),
 ...
 */
async function listContacts({ contactType, limit = 5000, orderBy = 'created_at', orderDir = 'desc' } = {}) {
  const typeFilter = contactType && VALID_TYPES.has(contactType)
    ? `&contact_type=eq.${encodeURIComponent(contactType)}`
    : '';
  return sb({
    table: CONTACTS_TABLE,
    query: `select=*&order=${orderBy}.${orderDir}&limit=${limit}${typeFilter}`,
  });
}
 ...
 */
async function importContacts(rows, { defaultType = 'lead' } = {}) {
...
/**
 * List all active contact types from the reference table.
 */
async function listContactTypes() {
  return sb({
    table: CONTACT_TYPES_TABLE,
