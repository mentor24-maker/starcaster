...
function plan(entries, taken) {
  ...
  for (const e of loose) {
    ...
    // The error test comes BEFORE the size test, on purpose (ticket 86bc7eh7m,
    // 2026-09-24). The indexer records a folder or file it could not open as
    // size 0 plus an error, so testing size first filed every unreadable row
    // as "empty (0 bytes)" — a whole folder of videos the walk never entered
    // read as needing no copy, and the erase guard said "all backed up" over
    // files nobody had seen.
...
function verdict(loose, hashOf, presentOnDrive) {
  ...
  for (const e of loose) {
    // Error before size, same reason as in plan(): an unreadable row carries
    // size 0, and this is the count the erase is gated on. A file nobody could
    // open is MISSING, not "needs no copy".
    if (idx.skipReason(e.path)) { out.noCopyNeeded += 1; continue; }
    if (e.error) { out.missing.push({ path: e.path, size: e.size, reason: `could not be read on MaxOne: ${e.error}` }); continue; }
    if (!(e.size > 0)) { out.noCopyNeeded += 1; continue; }
