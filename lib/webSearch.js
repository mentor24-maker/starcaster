...
function attachSearchPaging(config) {
  if (!config || !config.configured) return config;
  if (config.provider === PROVIDERS.brave) {
    return { ...config, pageSize: BRAVE_PAGE_SIZE, maxPageIndex: MAX_PAGE_INDEX };
  }
  return { ...config, pageSize: DEFAULT_PAGE_SIZE, maxPageIndex: MAX_PAGE_INDEX };
}
...
function resolveWebSearchConfig() {
  ...
  if (preference === PROVIDERS.brave) {
    return attachSearchPaging(brave.configured ? brave : { ...brave, configured: false });
  }
  if (preference === PROVIDERS.google_custom_search) {
    return attachSearchPaging(google.configured ? google : { ...google, configured: false });
  }

  if (brave.configured) return attachSearchPaging(brave);
  if (google.configured) return attachSearchPaging(google);
  return attachSearchPaging({
    provider: '',
    apiKey: '',
    searchEngineId: '',
    configured: false,
...
async function fetchBraveSearchBatch(query, pageIndex, config) {
  const pageSize = Math.max(1, Math.min(Number(config?.pageSize) || BRAVE_PAGE_SIZE, 20));
...
async function fetchGoogleSearchBatch(query, pageIndex, config) {
  const pageSize = Math.max(1, Math.min(Number(config?.pageSize) || DEFAULT_PAGE_SIZE, 10));
  const startIndex = Math.max(1, (Number(pageIndex) || 0) * pageSize + 1);
