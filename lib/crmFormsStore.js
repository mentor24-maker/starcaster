...
function sanitize(input) {
  ...
  return {
    ...
    styles,
    fields: publicFormFields(input.fields).map(sanitizeFormField),
...
function inputToRow(input, options = {}) {
  ...
  const fields = options.includeStylesMeta
    ...
    : (Array.isArray(sourceFields) ? sourceFields.map(sanitizeFormField) : []);
  return {
    id: f.id,
...
module.exports = {
  ...
  // Exported for the node suite: these two ARE the store's round trip
  // (write shape and read shape), and the only way to assert what a field
  // survives without a live database or a write to data/crm_forms.json.
  sanitizeForm: sanitize,
  formInputToRow: inputToRow,
};
