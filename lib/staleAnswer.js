...
const { DEFAULT_OVERLAP_MS } = require('../scripts/builder/busRelayPlan.js');
...
function answerFindings(records, { staleAfterMinutes = STALE_AFTER_MINUTES } = {}) {
  const findings = [];
  const fresh = [];
  // Tickets still inside the clock whose comments would not read. Not findings
  // — a transient read failure on a ticket four minutes old is not an alarm —
  // but not silence either: they were not measured, and the report says so.
  const unmeasured = [];
  ...
  return { findings, fresh, unmeasured, staleAfterMinutes };
}
