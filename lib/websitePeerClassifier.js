...
Rules:
- website_model MUST be exactly one category name from taxonomy.labels.
- Use taxonomy.guide identify_by text to choose the best-fitting category.
- suggested_reference_role MUST be "peer" or "model".
- Use "peer" ONLY when website_model is "Direct Competitors" (same offering and market at similar scale).
- Use "model" for every other category (hubs, directories, media, institutions, agencies at different scale, etc.).
...
- rationale is one concise sentence citing the strongest identification signal.
...
function buildClassificationContext({
  ...
  return {
    ...
    taxonomy: {
      labels: WEBSITE_PEER_MODELS.slice(),
      guide: getWebsitePeerModelGuideForPrompt(),
    },
...
function normalizeClassificationEntry(entry) {
  ...
  const role = suggestedReferenceRole(websiteModel);
...
async function classifyCandidateBatch(candidates, context) {
  ...
  const prompt = [
    ...
    '',
    'Taxonomy (labels + identification guide):',
...
function applyClassificationOverride(candidate, override) {
  ...
  const websiteModel = normalizeWebsiteModel(override.website_model);
  const referenceRole = suggestedReferenceRole(websiteModel);
  const peerBoost = referenceRole === 'peer' ? 4 : 0;
  const directCompetitorBoost = websiteModel === 'Direct Competitors' ? 3 : 0;
