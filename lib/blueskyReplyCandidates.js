...
function buildPrompt(input) {
  ...
  return [
    ...
    '- 2 candidates may be neutral/context-first replies with no explicit project mention.',
    '- 2 candidates should subtly reflect the shared project framing when it naturally fits.',
    '- 1 candidate may make a light explicit reference to ISITAS, the ISIT Construct, or the ISIT Game only if the post meaningfully overlaps those ideas.',
    'Constraints:',
    '- Plain text only.',
    ...
    '- Do not force a brand reference if the post context does not support it, but do not default to five generic replies when the overlap is obvious.',
