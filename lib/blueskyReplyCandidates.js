...
function buildPrompt(input) {
  ...
  return [
    ...
    `- Target about ${explicitPercent}% of candidates to include a light explicit reference to the active project when the post meaningfully overlaps its ideas.`,
    `- Target about ${subtlePercent}% of candidates to subtly reflect the active project framing without naming the project directly.`,
    ...
    '- References to the active project or related ideas must be understated, context-relevant, and never feel like a pitch.',
    ...
    '- Prefer the active project framing only when it is supported by the active project context and the post itself.',
    ...
    '- When relevant, connect the post to the active project vocabulary, themes, or point of view.',
    '- If using an explicit project mention, keep it brief and understated.',
...
async function generateReplyCandidates(input) {
  ...
  await recordAiUsage({
    provider: 'anthropic',
    model: modelRes.model,
    feature: 'bluesky_reply_candidates',
    body,
    scope: input?.scope,
  });
...
async function buildBlueskyReplyCandidateInput(options = {}) {
  ...
  const trainingKey = projectTrainingKey(scope || {
    userId: options.user_id || options.userId || options.userEmail || '',
  });
  const rawPromptConfig = await getTrainingPromptConfig(trainingKey);
  const promptConfig = constrainTrainingToProject(rawPromptConfig || {}, scope);
  const settingsRes = await getTrainingSettings(trainingKey);
