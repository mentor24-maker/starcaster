...
const { getYoutubeMinerPromptConfig } = require('./profileStore');
...
async function buildBlueskyReplyCandidateInput(options = {}) {
  ...
  const promptConfig = await getYoutubeMinerPromptConfig(options.user_id || options.userId || options.userEmail || '');
