...
function nameMatches(row, wanted) {
  ...
  const fields = [row?.name, row?.type_name, row?.format, row?.label];
...
async function findContentTypeByName(name, scope = null) {
  ...
  for (const field of ['name', 'type_name', 'format', 'label']) {
    const query = await scopedListQuery(
      CONTENT_TYPES_TABLE,
      `select=*&${field}=eq.${encodeURIComponent(wanted)}&limit=1`,
      scope
    );
    const res = await sbQuery({
      method: 'GET',
  ...
  const match = (listRes.data || []).find((entry) => nameMatches(entry, wanted));
  if (match?.id) {
...
async function ensureContentTypeByName(name, scope = null) {
  ...
  const row = await scopedInsertRow(CONTENT_TYPES_TABLE, { name: label }, scope);
  const res = await sbQuery({
    method: 'POST',
    table: CONTENT_TYPES_TABLE,
    query: 'select=*',
