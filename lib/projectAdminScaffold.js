 ...
 * and a Logout link. admin-login wears the ordinary public header, because a
 * signed-out visitor is the only person who ever sees it.
 *
...
const LOGOUT_HTML =
  '<p style="text-align: center;"><a href="/admin-logout"><strong>Logout</strong></a></p>';
 ...
 * and changes three things: the navigation module lists the admin pages, the
 * visitor-facing calls to action are dropped, and a Logout link is added.
 *
...
function buildAdminHeaderSection(sourceSection, options = {}) {
  ...
  for (const module of modules) {
    if (module?.type === 'navigation') {
      sawNavigation = true;
      navigationColumn = safeText(module.column) || 'main';
      nextModules.push({
        ...module,
        id: `module-${makeId()}`,
        name: 'Admin Menu',
