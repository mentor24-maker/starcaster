...
/**
 * Are these two the SAME uuid — by Postgres's reading, not JavaScript's?
 *
 * Postgres stores a `uuid` canonically and compares it case-insensitively, so
 * `A115C635-...` and `a115c635-...` are one value to the database and two
 * different strings to `!==`. The id therefore comes back lowercase from a
 * read while the caller's copy keeps whatever casing they typed.
 *
 * Review reproduced the consequence end to end: `updateSession` handed an
 * UPPERCASE session id refused that session's OWN source with a 400 saying
 * "must belong to this session", while the same call in lowercase succeeded
 * and `select count(*) where id='A115C635-...'` found the row either way. The
 * row existed, the source genuinely belonged to it, and the answer was a
 * refusal that stated something untrue — the failure shape this ticket has
 * been about since round one, arriving through a string comparison.
 *
 * Whitespace is trimmed for the same reason the readers trim: a caller who
 * pasted an id with a trailing newline means the same uuid.
 */
function sameUuid(a, b) {
  const left = safeText(a, 120).toLowerCase();
  const right = safeText(b, 120).toLowerCase();
...
async function createSession(input, scope = null) {
  ...
  // `.ok` is checked here like every other call site. It was read as
  // `readField(...).value` alone, which is safe only while neither field has a
  // distinct snake_case spelling — `title` and `state` are single words, so
  // readField can never return its supplied-twice refusal for them today. It
  // becomes a live bug the moment a field like `recordedAt` joins this list and
  // the 400 is swallowed into an undefined value, so the check goes in now
  // rather than as the next round of this ticket.
  const suppliedTitle = readField(input, 'title');
  ...
  // Whether the caller SUPPLIED a state is `.present`, never truthiness.
  // `safeText(false, 40)` is '' and `'' || 'new'` is 'new', so
  // `createSession({ state: false })` answered ok/201 and stored 'new' — a
  // value the caller never asked for, under a success — while `{ state: 0 }`,
  // `{ state: {} }` and `{ state: 'bogus' }` were all correctly 400s.
  // `updateSession` had always used `.present` and refused `false` properly;
  // only the create path coerced. Same defect as createSource's layerRole.
...
/**
 * One session by its exact title, or `data: null` when there is not one.
 *
 * Ingest (4/8) files everything into a HOLDING session named for its lane and
 * day, and needs to know whether today's already exists. It used to answer
 * that by scanning `listSessions(200, scope)` — which orders by `recorded_at`
 * desc, so once a project passes about 200 sessions any BACKFILLED older day
 * falls off the end of the page and a fresh holding session is made on every
 * single ingest. A list read is not a lookup, and the miss looked exactly like
 * a genuine "there isn't one". Found by review on 2026-09-16.
 *
 * A miss is `data: null` and NOT a 404, for the same reason the source finders
 * are: "we have not got this yet" and "the database is unreachable" have to
 * stay on different paths, or a caller retries a question that was already
 * answered.
 */
async function findSessionByTitle(title, scope = null) {
  const wanted = safeText(title, MAX_TITLE_LENGTH);
  if (!wanted) return { ok: false, status: 400, error: 'title is required' };
  const query = await scopedListQuery(
    table(),
    `title=eq.${encodeURIComponent(wanted)}&select=*&order=created_at.asc&limit=1`,
    scope
  );
  const res = await sbQuery({ method: 'GET', table: table(), query });
  if (!res.ok) return res;
  const found = Array.isArray(res.data) ? res.data[0] : res.data;
  return { ok: true, status: 200, data: found ? rowToSession(found) : null };
}
...
async function updateSession(id, patch, scope = null) {
  ...
  if (suppliedAudio.present) {
    ...
    if (audioSourceId) {
      ...
      if (!sameUuid(owner.data.sessionId, sessionId)) {
        return {
          ok: false,
          status: 400,
...
module.exports = {
  ...
  confidenceOrError,
  // Exported so a test can pin uuid equality without driving a whole update.
  sameUuid,
  createSession,
  getSessionById,
  findSessionByTitle,
