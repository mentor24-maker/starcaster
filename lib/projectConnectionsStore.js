...
/**
 * The CIPHERTEXT of that token is longer than the token: base64 alone is
 * 4/3 the byte length, and a multibyte character is up to four bytes before
 * that. Reading the stored column back through the PLAINTEXT cap truncated
 * anything whose ciphertext passed 8000 characters — a token that saved as 201
 * and then could never be decrypted again (review round 1, defect 3). The
 * worst legal case is 8000 four-byte characters: 32000 bytes, ~42700 base64
 * characters, so this cap clears it with room.
 */
const MAX_CIPHERTEXT_LENGTH = 60000;
...
/**
 * Every field updateConnectionStatus accepts — the lifecycle facts, and
 * deliberately NOT the tokens, the label, the avatar or the scopes. Those
 * belong to saveConnection, which requires a token precisely so that "record
 * that a post failed" can never arrive dressed as a full re-save and blank
 * the grant (review round 1, defect 2).
 */
const CONNECTION_STATUS_FIELDS = [
  'provider', 'accountId', 'status', 'expiresAt', 'lastVerifiedAt', 'lastError',
];
...
/**
 * Refuse any query that did not actually get scoped to a project.
 *
 * scopedIdQuery and scopedListQuery return the query UNCHANGED when the
 * tenant-columns probe fails — and a probe can fail for reasons that have
 * nothing to do with the table, a 502 on a cold start being the documented
 * one. Sending that unchanged query reads (or patches, or deletes) EVERY
 * project's rows, which here is every client's OAuth tokens. saveConnection
 * has always checked its stamp landed before inserting; this is the same
 * bargain for everything that is not an insert: no project filter, no query.
 * 503 because it is almost always transient — the probe is retried on the
 * next call, so retrying is the right response.
 */
function requireScopedQuery(query, action) {
  if (/project_id\.eq\./.test(String(query || ''))) return null;
  return {
    ok: false,
    status: 503,
    error: `Refusing to ${action}: the query could not be scoped to a project, `
      + "so it would touch every project's connections. Retry; if it persists, "
...
function rowToConnection(row) {
  ...
  return {
    ...
    hasAccessToken: Boolean(safeText(row.access_token_enc, MAX_CIPHERTEXT_LENGTH)),
    hasRefreshToken: Boolean(safeText(row.refresh_token_enc, MAX_CIPHERTEXT_LENGTH)),
  };
}
...
function readToken(row, prefix) {
  // The CIPHERTEXT cap, not the token cap — the stored form is ~4/3 longer
  // than what was encrypted, and truncating it here made every token past
  // ~5,970 characters undecryptable after a 201 save. See MAX_CIPHERTEXT_LENGTH.
  const enc = safeText(row?.[`${prefix}_enc`], MAX_CIPHERTEXT_LENGTH);
...
function encryptToken(plaintext, prefix) {
  const text = String(plaintext === 0 || plaintext ? plaintext : '').trim();
  if (!text) return { ok: true, columns: { [`${prefix}_enc`]: '', [`${prefix}_iv`]: '', [`${prefix}_tag`]: '' } };
  // Refused, not truncated. A token cut at 8000 characters still encrypts,
  // still saves as 201, and then fails at the provider as a 401 that looks
  // like the client revoked the grant. No real provider's token is this long;
  // one that is deserves an error naming the cap, not a quiet amputation.
  if (text.length > MAX_TOKEN_LENGTH) {
    return {
      ok: false,
      status: 400,
      error: `${prefix === 'access_token' ? 'accessToken' : 'refreshToken'} is `
        + `${text.length} characters, over the ${MAX_TOKEN_LENGTH} limit — refusing to store a truncated credential`,
    };
  }
 ...
 */
async function saveConnection(input, scope = null, options = null) {
  ...
  // A save with no access token is not a smaller save — on the update path it
  // PATCHes the whole row, so it would blank the tokens, the label and the
  // scopes of the grant already there and answer 200 (review round 1, defect
  // 2: the refresh token is handed over ONCE, so that row is unrecoverable).
  // Recording a status change or an error is updateConnectionStatus's job.
  if (!safeText(accessToken.value, MAX_TOKEN_LENGTH)) {
    return {
      ok: false,
      status: 400,
      error: 'accessToken is required — saveConnection writes the whole grant. '
        + 'To record a status change or an error on an existing connection, use updateConnectionStatus.',
    };
  }
  ...
  // .ok checked on every one — a dual-spelling conflict on these four used to
  // build a 400 and then store '' under a 201 (review round 1, defect 4).
  const accountLabel = readField(input, 'accountLabel');
  if (!accountLabel.ok) return accountLabel;
  const accountAvatarUrl = readField(input, 'accountAvatarUrl');
  if (!accountAvatarUrl.ok) return accountAvatarUrl;
  const connectedBy = readField(input, 'connectedByUserId');
  if (!connectedBy.ok) return connectedBy;
  const lastError = readField(input, 'lastError');
  ...
  const fields = {
    ...
    account_label: safeText(accountLabel.value, MAX_LABEL_LENGTH),
    account_avatar_url: safeText(accountAvatarUrl.value, MAX_URL_LENGTH),
    scopes: grantedScopes.value,
    status,
    expires_at: expiresAt.value,
    connected_by_user_id: safeText(connectedBy.value, 120)
      || safeText(scope?.userId || scope?.user_id, 120)
      || null,
    last_verified_at: lastVerifiedAt.value,
    last_error: safeText(lastError.value, MAX_ERROR_LENGTH),
    ...access.columns,
    ...refresh.columns,
  ...
  // `updated_at` is the ACTIVE-ACCOUNT SELECTOR, not bookkeeping — the same
  // fact `updateConnectionStatus` is built around, reaching this function
  // through the one path that also RE-SAVES a whole grant: a renewed token.
  //
  // Renewing on the way out of the resolver does mean to move the row (it is
  // the row that just posted), so the default stays true. The health sweep does
  // not: it renews every lapsed row on a schedule, and bumping there would walk
  // a client with two accounts on one platform onto whichever the sweep touched
  // last, on a timer, with every gate green (86bbpz1hu, review round 2).
  //
  // Only the update half is guarded. A brand-new row has no election to
  // preserve — its `updated_at` and `created_at` are the same instant.
  if (options?.bumpUpdatedAt === false) delete patch.updated_at;
  const query = await scopedIdQuery(
    table(),
    `${keyFilter(key.provider, key.accountId)}&select=*`,
    scope
  );
  // Unreachable while the insert guard above holds (a probe that failed there
  // never gets here) — but an unscoped PATCH would overwrite ANOTHER project's
  // grant, so the update path carries its own lock rather than borrowing one.
  const unscoped = requireScopedQuery(query, 'update a connection');
...
async function getConnection(input, scope = null) {
  ...
  // The same lock the insert has, at the door where it matters most: this is
  // the one function that decrypts. Reviewed round 1 with one simulated 502 on
  // the probe — the unfiltered query handed project B project A's token.
  const unscoped = requireScopedQuery(query, 'read a connection');
  if (unscoped) return unscoped;
...
/**
 * Record a lifecycle fact about an existing grant — it expired, a post failed,
 * a verify succeeded — WITHOUT the power to touch anything else.
 *
 * This exists because the alternative was worse: slice 6 recording "the token
 * errored" through saveConnection would PATCH the whole row and blank the
 * tokens, the label and the scopes under a 200 (review round 1, defect 2).
 * Only the fields the caller actually supplied are written; a field left out
 * is a field left alone, not a field reset to its default.
 */
async function updateConnectionStatus(input, scope = null, options = {}) {
  const unknown = unknownKeyError(input, CONNECTION_STATUS_FIELDS);
  if (unknown) return unknown;
  ...
  const projectId = safeText(scope?.projectId || scope?.project_id, 120);
  if (!projectId) {
    return { ok: false, status: 400, error: 'A project scope is required to update a connection' };
  }

  const patch = {};
  // `updated_at` is not bookkeeping here — it is the ACTIVE-ACCOUNT SELECTOR.
  // listConnections orders by it, the resolver posts with the first live row it
  // returns, and lib/connections/completeConnection.js says it outright:
  // "touching a row is how it is chosen". So a caller that writes a fact about
  // a connection without meaning to re-elect it — the health sweep in
  // lib/connections/verifySweep.js, which touches EVERY row on a schedule —
  // passes `{ bumpUpdatedAt: false }` and leaves the client's choice alone.
  // Default true, because every caller that existed before the sweep is
  // recording a real change and does mean to move the row.
  if (options?.bumpUpdatedAt !== false) patch.updated_at = new Date().toISOString();

  const suppliedStatus = readField(input, 'status');
  if (!suppliedStatus.ok) return suppliedStatus;
  if (suppliedStatus.present) {
    const status = safeText(suppliedStatus.value, 40);
    if (!CONNECTION_STATUSES.includes(status)) {
      return { ok: false, status: 400, error: `status must be one of ${CONNECTION_STATUSES.join(', ')}` };
    }
    patch.status = status;
  ...
  for (const [field, column] of [['expiresAt', 'expires_at'], ['lastVerifiedAt', 'last_verified_at']]) {
    const supplied = readField(input, field);
    if (!supplied.ok) return supplied;
    if (!supplied.present) continue;
    const stamp = timestampOrError(supplied.value, field);
    if (!stamp.ok) return stamp;
    patch[column] = stamp.value;
  }

  const suppliedError = readField(input, 'lastError');
  if (!suppliedError.ok) return suppliedError;
  if (suppliedError.present) patch.last_error = safeText(suppliedError.value, MAX_ERROR_LENGTH);

  // With bumpUpdatedAt off and no lifecycle field supplied there is nothing to
  // write, and PostgREST answers an empty body with a 400 that names none of
  // this. Refused here instead, so the caller learns it asked for nothing.
  if (!Object.keys(patch).length) {
    return {
      ok: false,
      status: 400,
      error: 'updateConnectionStatus was given nothing to write — supply at least one of '
        + `${CONNECTION_STATUS_FIELDS.filter((f) => f !== 'provider' && f !== 'accountId').join(', ')}`,
    };
  }
  ...
  if (!updated.ok) return updated;
  const stored = Array.isArray(updated.data) ? updated.data[0] : updated.data;
  if (!stored) return { ok: false, status: 404, error: 'Connection not found' };
  return { ok: true, status: 200, data: rowToConnection(stored) };
}
...
async function listConnections(limit = 200, scope = null) {
  ...
  const unscoped = requireScopedQuery(query, 'list connections');
  if (unscoped) return unscoped;
  const res = await sbQuery({ method: 'GET', table: table(), query });
  if (!res.ok) return res;
  return {
...
async function deleteConnection(input, scope = null) {
  ...
  // Unscoped, this DELETE removes the same-named grant from EVERY project.
  const unscoped = requireScopedQuery(query, 'delete a connection');
  if (unscoped) return unscoped;
