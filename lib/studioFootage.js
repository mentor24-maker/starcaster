...
function buildFootage({ sessions = [], sources = [], filters = {} } = {}) {
  ...
  for (const source of sources) {
    ...
    const key = session ? text(session.id) : NO_SESSION_ID;
  ...
  const shaped = [...groups.entries()].map(([key, { session, entries }]) => {
    ...
    return {
      id: key,
      title: session ? (text(session.title) || 'Untitled session') : NO_SESSION_TITLE,
  ...
  shaped.sort((a, b) => {
    // The no-session group always sorts last: it is a leftover, not a shoot.
    if (a.id === NO_SESSION_ID) return 1;
    if (b.id === NO_SESSION_ID) return -1;
