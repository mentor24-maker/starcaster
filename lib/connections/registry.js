...
const CATALOGUE = [
  ...
  },
  {
    ...
    iconKey: 'instagram',
    // The blurb names the account TYPE, because that is the single thing most
    // likely to stop a client connecting, and a sentence read before pressing
    // the button is worth more than the same sentence read as a refusal after.
    blurb: 'Post photos to your Instagram Business or Creator account, through the Facebook Page it is linked to.',
    readiness: 'ready',
    authKind: 'redirect',
    adapter: instagram,
  },
  {
    ...
    displayName: 'X',
    iconKey: 'x',
    // Names the account, because "your X account" reads as though it might mean
    // Starcaster's — which is exactly what it DID mean before slice 7, and the
    // thing this epic exists to change.
    // The second sentence is a limitation, not marketing, and it is here rather
    // than in a ticket comment because the card is the only place the client
    // ever reads. Text posting goes to X's v2 endpoint and works on both
    // schemes; image upload still goes to the v1.1 endpoint X deprecated for
    // self-serve tiers in March 2025, and the `media.write` scope this adapter
    // asks for is a v2 scope that does not authorise it. Nothing in the suite
    // reaches X, so no test here can settle it either way — saying "unproven"
    // is the honest reading, and it needs its own ticket with a live upload
    // (86bbpz1hu, review rounds 1 and 2).
    blurb: 'Post to your own X account. Sign in with X and Starcaster posts as you, not as Starcaster. '
      + 'Text posts are supported; posting images from a connected account is not proven yet.',
    readiness: 'ready',
    authKind: 'redirect',
    adapter: x,
  },
 ...
 *
 * The branching is split out as `adapterAnswer` so it can be exercised with an
 * entry that is not in the live catalogue. As of Connections 7 of 7 there is no
 * `coming_soon` card left — every platform in scope for the epic connects — so
 * the test that proves the two answers differ had nothing in the catalogue to
 * point at, and the alternative to this split was deleting the assertion
 * (86bbpz1hu).
 */
function adapterAnswer(entry, key) {
  if (!entry) {
...
function getAdapter(provider) {
  ...
  return adapterAnswer(getEntry(key), key);
}
