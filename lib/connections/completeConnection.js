...
async function storeAccounts({ provider, displayName, accounts, scope }) {
  ...
  /**
   * EVERY account is checked BEFORE any of them is written.
   *
   * This used to be one loop that checked and wrote in the same pass, so a
   * grant whose second account was unstorable had already written the first —
   * with `status: 'connected'` — by the time it refused. The caller then sent
   * the client to an error screen while the Connections card read green with an
   * empty credential behind it: a half-save, which is exactly the state
   * `NO_ACCOUNTS` above exists to prevent, arriving by the other door.
   *
   * Splitting the check out makes "all or nothing" a property of the shape
   * rather than of every adapter remembering to hand up a clean list. Found in
   * review of Connections 5 of 7 (86bbpz1gk), where the Instagram adapter could
   * produce exactly that mixed list; that adapter no longer can, and this is
   * the half that holds for the next one.
   */
