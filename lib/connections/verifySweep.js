 ...
 *
 * The one exception is `refreshBeforeUse` **called from the resolver**, which
 * writes through `saveConnection` (a new token is a whole-grant write) and so
 * does bump it. That is correct there and not a hole: it only ever runs on the
 * row the resolver has ALREADY chosen, so bumping keeps that same row first.
 *
 * `verifyConnection` now calls it too, to renew a lapsed row rather than write
 * it off — and passes `bumpUpdatedAt: false`, because THAT call is on a
 * schedule and touches whatever is due. Without it, a client holding two
 * accounts on one platform would find their posts moving to whichever the
 * sweep last renewed, which is Fact One's own failure arriving through the fix
 * for a different one.
 *
...
function accountFrom(connection) {
  return contract.account({
    ...
    accountAvatarUrl: safeText(connection?.accountAvatarUrl),
    accessToken: safeText(connection?.accessToken),
    // The refresh token is the whole point of handing an adapter an account at
    // all on platforms whose tokens expire: X renews with THIS value, not with
    // the access token, so omitting it made `refresh` structurally unable to
    // work no matter what the adapter did (86bbpz1hu). `getConnection` decrypts
    // it, the account vocabulary now carries it, and this is the line that
    // joins the two.
    refreshToken: safeText(connection?.refreshToken),
...
/**
 * Did the provider JUDGE this grant, or could we simply not reach it?
 *
 * The difference is the whole of review round 3 of 86bbpz1hu, and getting it
 * wrong is permanent. A judgment — "this refresh token is no longer valid" —
 * is a decision, and `expired` is the honest thing to write down: `liveness`
 * refuses that row for its STATUS, deliberately and for ever, because no
 * refresh token may overrule a grant the provider withdrew.
 *
 * An unreachable provider has decided nothing. A 502 on a cold start, a
 * timeout, a rate limit, a thrown socket error — the refresh token on that row
 * is still perfectly good, and the very next attempt would probably renew it.
 * Writing `expired` there took a client's X connection off the air
 * PERMANENTLY on one network blip, with the card still green and every
 * subsequent post going out on Dane's own account. Reproduced by review: the
 * adapter was asked once, the second pass never even tried.
 *
 * So: only the provider's own 4xx counts as a judgment. `408` and `429` are
 * 4xx by number and temporary by meaning, so they are excluded by name. A
 * missing or unparseable status is treated as unreachable — failing towards
 * "try again" costs one retry, and failing the other way costs the account.
 */
function providerJudgedTheGrant(res) {
 ...
 */
async function refreshBeforeUse({
  connection, scope, nowMs, windowMs = REFRESH_WINDOW_MS, store, registryRef, bumpUpdatedAt = true,
} = {}) {
  ...
  if (!res || res.ok !== true) {
    ...
    const judged = providerJudgedTheGrant(res);
    /**
     ...
     *
     * `expired` is written only when the clock has run out AND the provider
     * itself refused the grant, because `expired` is not a live status and the
     * resolver will never elect that row again. Review round 3: a single
     * unreachable-provider failure on a lapsed row used to write it, which took
     * the connection off the air for good on one blip and sent every post after
     * it to the shared keys. An unreachable provider has judged nothing, so a
     * lapsed row stays `expiring` — past its deadline, therefore still elected
     * for renewal by `liveness`, and renewed the moment the provider answers.
     */
    const status = (due.expired && judged) ? 'expired' : 'expiring';
    await connectionsStore.updateConnectionStatus(
      {
        ...
        status,
        lastError: judged
          ? `Could not renew this connection before it expires: ${why}`
          : `Could not reach ${provider} to renew this connection, so it has not been renewed yet `
            + `and will be retried: ${why}`,
      },
      scope,
      { bumpUpdatedAt: false }
    );
    return { ok: false, refreshed: false, connection, note: why, judged };
  }
  ...
  const saved = await connectionsStore.saveConnection(
    ...
    scope,
    // The resolver renews the row it is about to post with, so bumping keeps
    // that same row first — see Fact One. `verifyConnection` renews on a
    // SCHEDULE and passes false, because a sweep that re-elected each row it
    // happened to renew would move a client's posts between their own accounts
    // on a timer.
    { bumpUpdatedAt: bumpUpdatedAt !== false }
  );
  if (!saved.ok) {
...
async function verifyConnection({ row, scope, nowMs, store, registryRef, identityStore } = {}) {
  ...
  let expiry = refreshDue(row, nowMs);

  /** Past its deadline and with nothing that could move it — the clock is the answer. */
  const writeOffAsExpired = async (why) => {
    const written = await record({ status: 'expired', lastError: why });
    return { ...base, checked: true, healthy: false, status: 'expired', reason: why, written: written.ok };
  };

  /**
   * Past its deadline, and the renewal that could fix that did not get to
   * happen. Review round 3 of 86bbpz1hu, both halves of it.
   *
   * `cannotCheck` is wrong here because it writes no status at all, so an
   * expired row kept a GREEN card — the sweep's own reading said nothing while
   * the clock said plenty. `writeOffAsExpired` is wrong here too, and worse:
   * `expired` is not a live status, so the resolver never elects that row
   * again, and a momentary failure would end the connection permanently. That
   * is the exact defect this round was sent back for, and it must not be fixed
   * by reintroducing itself one branch over.
   *
   * `expiring` is both honest and recoverable: the card goes amber with the
   * sentence below under it, and `liveness` still elects a row past its
   * deadline for renewal — so the next attempt renews it. `healthy: null`
   * because nothing was learnt from the provider; this is an unknown, not a
   * pass and not a failure (DOCTRINE 3.11).
   */
  const markUnrenewedYet = async (why) => {
    const written = await record({ status: 'expiring', lastError: why });
    return {
      ...base,
      checked: false,
      healthy: null,
  ...
  if (expiry.expired && !row?.hasRefreshToken) {
    // Already past its expiry with no renewal credential: there is nothing for
    // a provider to tell us that the clock has not, and spending a live API
    // call to be told so is waste.
    return writeOffAsExpired(expiry.why);
  }

  const adapter = providers.adapterFor(provider);
  if (!adapter) {
    // An expired row still IS expired — that is the clock's reading, not the
    // provider's, and it needs no adapter. Only the renewal did.
    if (expiry.expired) {
      return writeOffAsExpired(
        `${expiry.why}, and ${provider} has no adapter on this build, so nothing could renew it`
      );
    }
  ...
  if (!full.ok) {
    const why = `the stored credential could not be read (${safeText(full.error) || full.status})`;
    // The clock's reading needs no credential — the same sentence the no-adapter
    // branch above makes. Left as a plain `cannotCheck`, an already-expired row
    // whose token would not decrypt kept its `connected` status and a green
    // card (review round 3, item 2). It is not written off either: failing to
    // read our OWN vault is our fault, not a judgment X made, and the refresh
    // token on that row may be perfectly good.
    if (expiry.expired) {
      return markUnrenewedYet(`${expiry.why}, and ${why}, so it could not be renewed`);
    }
    return cannotCheck(why);
  }
  let stored = full.data;

  /**
   * RENEW BEFORE WRITING IT OFF. Review round 2 of 86bbpz1hu.
   *
   * This branch used to stamp `expired` and return, without ever asking the
   * adapter — on a row whose refresh token was still perfectly good. `expired`
   * is not a live status, so once the sweep had been past, the resolver would
   * reject that connection for ever rather than until its next renewal. It was
   * dead code before this slice, because nothing stored an expiry; carrying
   * X's expiry into the vault made it the ordinary outcome, X's tokens lasting
   * about two hours.
   *
   * `bumpUpdatedAt: false` for the reason Fact One gives at the top of this
   * file: a sweep write must never re-elect the row it touched.
   */
  if (expiry.expired) {
    const renewed = await refreshBeforeUse({
      connection: stored,
      scope,
      nowMs,
      store: connectionsStore,
      registryRef: providers,
      bumpUpdatedAt: false,
    });
    if (!renewed.refreshed) {
      const why = safeText(renewed.note) || 'no reason given';
      // Judged or unreachable — see `providerJudgedTheGrant`. A provider that
      // refused the grant, or one that answered "there is nothing here to
      // refresh", has told us something and `expired` records it. A provider we
      // could not reach has told us nothing, and writing `expired` on that
      // would end the connection on a network blip.
      const unreachable = renewed.ok === false && renewed.judged !== true;
      if (unreachable) {
        return markUnrenewedYet(
          `${expiry.why}, and ${provider} could not be reached to renew it (${why}) — it will be retried`
        );
      }
      return writeOffAsExpired(`${expiry.why}, and it could not be renewed (${why})`);
    }
    // Renewed. Carry on into the ordinary check with the NEW token — a fresh
    // credential is exactly the one worth asking the provider about, and the
    // identity-drift comparison below still has to run.
    stored = renewed.connection || stored;
    // Re-read the clock against the NEW deadline. Left alone, `expiry.due`
    // would still be the old row's `true`, and the healthy branch below would
    // write `expiring` on a token with hours of life on it — an amber card on a
    // connection that was just fixed.
    expiry = refreshDue(stored, nowMs);
  }

  let verifyResult;
  try {
    verifyResult = await adapter.verify(accountFrom(stored));
  } catch (err) {
  ...
  let drift = identityDrift({ stored, verified });
  ...
  const outcome = outcomeFor({ verifyResult, stored, drift });
