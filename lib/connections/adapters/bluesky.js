...
function credsFrom(input = {}) {
  // Through contract.accountRef, so an account this adapter HANDED BACK works
  // as input unedited. It did not until round 3: the account came out as
  // `{ id, label }` and this function read `accountLabel`/`accountId`, so
  // verify, listAccounts and refresh all answered "Bluesky needs a handle and
  // an app password" for a connection that was perfectly healthy — a sentence
  // that would have sent a client to re-mint a credential that was never wrong.
  const ref = contract.accountRef(input);
  return {
    identifier: normalizeIdentifier(input.identifier || ref.accountLabel || ref.accountId),
    appPassword: safeText(input.appPassword || ref.accessToken),
    // `raw.serviceUrl` is how a self-hosted PDS survives the round trip: it is
    // the only place the account carries which server it signed in against.
    serviceUrl: (safeText(input.serviceUrl || ref.raw?.serviceUrl) || DEFAULT_SERVICE_URL).replace(/\/+$/, ''),
  };
}
...
function toAccount(session, creds) {
  ...
  return contract.account({
    accountId: safeText(session?.did),
    accountLabel: handle,
...
async function verify(input = {}) {
  ...
  // The 409 needs BOTH names off the same account: the handle is what signs in
  // and the DID is what was saved, and a handle can move to another DID. Round
  // 2's version read `input.accountId` and the account offered `id`, so on a
  // real round trip there was nothing to compare and the check could not fire.
  const expectedId = contract.accountRef(input).accountId;
  if (expectedId && account.accountId && expectedId !== account.accountId) {
    return contract.fail(409, `This app password now signs in as ${account.accountLabel}, not the account it was saved for. `
      + 'Connect it again to update which account Starcaster posts to.');
  }
  return contract.ok(200, { valid: true, accountId: account.accountId, accountLabel: account.accountLabel });
}
