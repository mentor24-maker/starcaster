...
/**
 * KNOWN, AND DELIBERATELY NOT FIXED HERE (86bbpz1ed, review round 1).
 *
 * `/me/accounts` wants the USER token, and `exchange` hands accounts back
 * carrying the PAGE token — so feeding this adapter its own output makes Meta
 * refuse. Slice 2's conformance sweep stubs a 200, which proves the field is
 * read, not that Meta would accept it.
 *
 * Slice 3 (the resolver) never calls `listAccounts` — measured: nothing outside
 * the adapters and the contract's own docs calls it at all — so fixing it here
 * would be a change nothing exercises, in a slice whose Non-goals forbid new
 * provider behaviour. It belongs to the slice that first sweeps stored
 * connections for health, which is slice 6 (86bbpz1gv): either the user token
 * is stored alongside the Page token, or `listAccounts` means something
 * narrower for this provider. Whoever takes that slice decides.
 */
...
module.exports = {
  ...
  /**
   * The Meta plumbing, shared rather than copied. Connections 5 of 7 (86bbpz1gk).
   *
   * Instagram is not another platform with its own sign-in — it IS this grant,
   * read from a different angle: the same Meta app, the same consent screen,
   * the same registered redirect_uri, the same code-for-token exchange. The
   * only thing that differs is which fields are asked of `/me/accounts`
   * afterwards.
   *
   * These four were already here and already correct; the alternative was a
   * second copy of the token exchange in instagram.js, which is how one of them
   * gets a fix and the other does not. Exporting is a pure addition — no line
   * above changed, so the working production Facebook flow is byte-identical.
   */
  fetchJson,
