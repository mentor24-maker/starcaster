 ...
 * list of accounts without knowing whose they are. The provider's own row is
 * kept on `raw` because the Page shape (`access_token`, `name`) is what
 * routes/engage.js and projectSocialCredentialsStore already speak.
 */
function toAccount(page) {
  return {
    id: safeText(page?.id),
    label: safeText(page?.name),
    avatarUrl: '',
...
async function verify({ accountId, accessToken } = {}) {
  const pageId = safeText(accountId);
  const token = safeText(accessToken);
  if (!pageId || !token) {
 ...
 */
async function revoke({ accountId, accessToken } = {}) {
  const pageId = safeText(accountId);
  const token = safeText(accessToken);
  if (!pageId || !token) {
