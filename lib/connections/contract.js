...
const ACCOUNT_FIELDS = Object.freeze([
  'accountId', 'accountLabel', 'accountAvatarUrl', 'accessToken', 'refreshToken', 'raw',
]);
...
function storableAccount(input = {}) {
  ...
  // Written only when there IS one. `saveConnection` accepts `refreshToken`,
  // but writing '' for the platforms that have none would replace a stored
  // value with an empty one on any re-save — which is how a working X grant
  // would lose the only thing that can renew it.
  if (ref.refreshToken) storable.refreshToken = ref.refreshToken;
  return storable;
}
