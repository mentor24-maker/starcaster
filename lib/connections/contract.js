 ...
 * already accepts — with ONE exception, and it matters: `raw`. Every account
 * carries it, and `saveConnection` runs a strict allowlist that does not
 * include it, so `saveConnection(projectId, { provider, ...account })` answers
 * `400 Unknown field: raw`. An account is therefore NOT storable exactly as it
 * stands; it is storable through `storableAccount()` below, which is the one
 * place that translation lives — because a translation each caller writes for
 * itself is how two vocabularies grow back.
 */
const ACCOUNT_FIELDS = Object.freeze([
  'accountId', 'accountLabel', 'accountAvatarUrl', 'accessToken', 'refreshToken', 'expiresAt', 'raw',
]);
...
function accountRef(input = {}) {
  ...
  return {
    ...
    refreshToken: safeText(source.refreshToken),
...
/**
 * An account in the shape `saveConnection` actually accepts: its fields, and
 * nothing it would refuse.
 *
 * `raw` is the entire reason this helper exists. Adapters emit it because it is
 * the provider's own row — Bluesky parks the PDS address there, Facebook the
 * Page — and `saveConnection` rejects any field outside CONNECTION_FIELDS, so
 * spreading an account into it answers `400 Unknown field: raw`. Measured
 * against the real store, not assumed.
 *
 * What this DROPS is not cosmetic, and a caller should know it rather than
 * discover it. `raw` is where the second half of a two-part credential lives,
 * and there is nowhere in `project_connections` for it to land. A provider
 * whose credential is a PAIR therefore cannot be completed from a stored
 * connection at all — lib/connections/resolveCredentials.js refuses those
 * outright rather than pairing a client's half with the platform's other half,
 * which authenticates as nobody.
 *
 * ── What slice 7 changed, and what it deliberately did not ─────────────────
 *
 * `refreshToken` now rides through (86bbpz1hu). It is NOT the same thing as
 * giving `raw` a home: the store has always had a `refresh_token_enc` column
 * and `saveConnection` has always accepted the field — what was missing was
 * anywhere in the ACCOUNT vocabulary to carry one, so an adapter that received
 * a refresh token had no way to hand it over and `verifySweep.accountFrom`
 * could never hand it back. Slice 6 built a whole refresh path on top of that
 * gap; X is the first platform whose tokens actually expire (hours, not
 * months), so it is the first place the gap could bite.
 *
 * `expiresAt` rides through too, and it was the second half of the same gap —
 * missed in round 1 and found in review. A refresh token nothing knows the
 * deadline for is a refresh token nothing ever spends: see the note on the
 * field itself below.
 *
 * `raw` still has no home, and X does not need one — its OAuth 2.0 credential
 * is a single bearer token, not the OAuth 1.0a pair the note above was written
 * about. Bluesky's self-hosted-PDS case is still waiting on a column.
 */
function storableAccount(input = {}) {
  const ref = accountRef(input);
  const storable = {
    accountId: ref.accountId,
    accountLabel: ref.accountLabel,
  ...
  /**
   * Both credential fields are written UNCONDITIONALLY, and the empty string is
   * a real value here rather than an absence.
   *
   * This used to omit `refreshToken` when it was empty, with a comment saying
   * that writing '' "would replace a stored value with an empty one on any
   * re-save". That is not what happens. `saveConnection` reads the field with
   * `readField`, which answers `undefined` for an absent key; `encryptToken`
   * turns both `undefined` and '' into the same three empty columns, and they
   * are spread into the row either way (lib/projectConnectionsStore.js). The
   * omission therefore did exactly the thing it was written to avoid — it just
   * did it silently, and the next adapter author would have trusted the comment.
   *
   * Nothing protects a stored credential from a partial save, because
   * `saveConnection` writes the WHOLE grant by design and refuses outright when
   * `accessToken` is missing. That refusal is the guard; this was never one.
   */
  storable.refreshToken = ref.refreshToken;
  /**
   * And the expiry, which is the field this whole helper was dropping.
   *
   * `expiresAt` reached here from the adapter and went no further: the account
   * vocabulary did not carry it and this function did not write it, so
   * `expires_at` was NULL on every row a grant created. `verifySweep.refreshDue`
   * opens with `if (!expiresAt) return { due: false, why: 'this token has no
   * recorded expiry' }`, so nothing ever renewed — an X connection worked for
   * about two hours and was then dead, with `refreshBeforeUse` still answering
   * `ok: true` and the card still green (86bbpz1hu, review round 1).
   *
   * It is written unconditionally for the same reason as the token above: a
   * platform whose tokens do not expire supplies '', the store writes NULL, and
   * `refreshDue` correctly says there is nothing to renew. The dangerous value
   * is a missing one that reads as "never expires".
   */
  storable.expiresAt = ref.expiresAt;
  return storable;
}
