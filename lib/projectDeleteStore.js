...
/**
 * The column exists but is the wrong TYPE — dev_sessions.project_id was
 * declared BIGINT while every project id on this platform is text, so
 * Postgres rejects the filter outright (22P02) rather than returning no rows.
 *
 * A wrong-typed column provably cannot hold this project's rows, so there is
 * nothing to delete there and the purge continues. But it is a schema defect,
 * not a normal absence, so it is collected and reported rather than swallowed
 * the way a missing column is — a delete that quietly skips a table it was
 * asked to clear is indistinguishable from one that cleared it.
 */
function typeMismatchError(result) {
  const text = String(result?.error || '').toLowerCase();
  return text.includes('invalid input syntax for type');
}
...
async function deleteByProjectId(table, projectId) {
  ...
  if (!result.ok && typeMismatchError(result)) {
    return { ok: true, skipped: true, schemaMismatch: table };
  }
  return result;
}
...
async function purgeProjectAssociatedData(projectId) {
  ...
  for (const table of messagingTables) {
    ...
    if (!result.ok) return result;
    if (result.schemaMismatch) schemaMismatches.push(result.schemaMismatch);
  }
  ...
  return {
    ...
    purge: {
      ...
      // Tables whose project_id is the wrong type. They hold no rows for this
      // project by definition, but the schema needs fixing.
      schemaMismatches,
    },
  };
}
