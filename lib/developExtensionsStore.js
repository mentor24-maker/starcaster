...
function table() {
  return tableConfig().developExtensions;
}
...
async function ensureDefaultExtensions(rows) {
  ...
  const defaults = [
    {
      ...
      launchPageId: 'developExtensionIconBuilderPage',
      isFeatured: true,
      usageCount: 1,
    },
    {
      ...
      launchPageId: 'developExtensionScreenshotPage',
      isFeatured: true,
      usageCount: 1,
    },
    {
      ...
      launchPageId: 'developExtensionThumbnailPage',
      isFeatured: true,
      usageCount: 1,
    },
  ];
  ...
  for (const definition of defaults) {
    ...
    const created = await createExtension(definition);
...
async function listExtensions(limit = 500) {
...
async function updateExtension(id, input) {
  ...
  const res = await sbQuery({
    method: 'PATCH',
    table: table(),
    query: `id=eq.${extensionId}&select=*`,
    headers: { Prefer: 'return=representation' },
    body: row,
  });
  if (!res.ok) return res;
...
async function trackExtensionUse(id) {
  const extensionId = Number(id || 0) || 0;
  if (!extensionId) return { ok: false, status: 400, error: 'id is required' };
