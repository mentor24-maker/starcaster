...
function rowToWebsitePeer(row) {
  ...
  return {
    ...
    last_harvested_at: String(row.last_harvested_at || row.updated_at || row.created_at || ''),
...
async function createWebsitePeer(input, scope = null) {
  ...
  const row = await scopedInsertRow(table(), {
    ...
    last_harvested_at: String(input?.last_harvested_at || new Date().toISOString()),
  }, scope);
  const res = await sbQuery({
    method: 'POST',
    table: table(),
...
async function upsertWebsitePeersFromRun(run, scope = null) {
  ...
  const sourceRow = await scopedInsertRow(table(), {
    ...
    last_harvested_at: String(run?.finished_at || run?.updated_at || new Date().toISOString()),
  }, scope);
