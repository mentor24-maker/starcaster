...
// The one door, so these requests are counted (task 86bbugcpa).
const { clickupFetch, underTestRunner, YIELDED_STATUS } = require('../scripts/lib/clickup.cjs');
...
/*
 * Through the SHARED door (2026-09-04, task 86bbugcpa).
 *
 * This one was the quietest of the five. It never appeared in the uniqueness
 * guard's output because it does not write `fetch(` — it resolves its
 * transport indirectly (`deps.fetchImpl || globalThis.fetch`), which the
 * detector could not see. So the forwarder spent requests against the one
 * company token from the bug reporter's own request path, uncounted, and the
 * guard read clean the whole time.
 *
 * `deps.fetchImpl` still works and the tests still use it — but it is handed
 * to the door now instead of replacing it, so it is accounted for rather than
 * invisible. TWO COUNTERS, and a substitute transport lands on one of them
 * (task 86bc0wrxg): the door's OWN counter counts the ATTEMPT, so a caller
 * cannot make this pass look cheaper by bringing its own `fetch`; the
 * MACHINE-WIDE LEDGER — the one the live relay and the scheduled jobs read —
 * records only what was actually taken from the company token, and a faked
 * transport takes nothing.
 *
 * Do not read the old wording back into this: until 2026-09-15 the comment
 * here said a substitute transport was "still a counted request" full stop,
 * which is what a future change would have acted on. This is the one
 * production file with a live `deps.fetchImpl` seam, so this is where that
 * false guarantee would have been believed. `spendsClickUpBudget` in
 * `scripts/lib/clickup.cjs` is where the two counters part company.
 */
/*
 * A test run never reaches the real ClickUp (2026-09-15, task 86bc0zuvb).
 *
 * Tests drive the real public submit route, and a test run with the company
 * keys loaded (`doppler run -- npm run test:builder`, which a build pass did
 * twice that night) carries a real CLICKUP_API_TOKEN. Every fixture report
 * then became a real task held for the operator: 34 fake "Bug report:" tickets
 * in his lane by morning. A test that means to exercise this module brings its
 * own transport (`deps.fetchImpl`); one that did not has no business spending
 * the company token, so it is refused here, before the request.
 *
 * `underTestRunner` itself lives in `scripts/lib/clickup.cjs` now, because the
 * door asks the same question for its own reason (task 86bc0wrxg). The two
 * call sites read DIFFERENT environments on purpose: this one reads the env
 * the caller handed in, so a test can pass `env: {}` and prove the refusal
 * turns on the runner rather than on the token — the door reads the REAL
 * `process.env`, so that same deliberate `{}` cannot also switch off the rule
 * keeping invented traffic off the operator's shared ledger.
 */
async function clickupRequest(method, path, body, deps) {
  if (!deps?.fetchImpl && underTestRunner(deps?.env || process.env)) {
    return { ok: false, status: 0, json: null, text: '', error: 'refused: a test run may not reach the real ClickUp — inject deps.fetchImpl' };
  }
  const timeoutMs = Number.isFinite(deps?.timeoutMs) ? deps.timeoutMs : REQUEST_TIMEOUT_MS;
  const signal = (typeof AbortSignal !== 'undefined' && AbortSignal.timeout)
    ? AbortSignal.timeout(timeoutMs)
    : undefined;
  const out = await clickupFetch(
    `${API_BASE}${path}`,
    {
      method,
  ...
  // THE THIRD OUTCOME (task 86bc0w6my). The FIFTH call site, and one the
  // ticket that found the other four did not list — worth saying out loud,
  // because it is the reason this fix ends in a guard test rather than in four
  // edits. It cannot fire today: this runs on the bug reporter's request path,
  // where nothing declares STARCASTER_CALLER, and the default is `interactive`
  // — which never yields. It is handled anyway, because "cannot fire today"
  // depends on a default in another file, and the failure it prevents is a
  // TypeError on a visitor's request.
  if (out.yielded) {
    return {
      ok: false,
      ...
      json: null,
      text: '',
      error: out.yielded.why || 'a scheduled job stopped at the ClickUp reserve',
    };
  }
  ...
  return { ok: out.res.ok, status: out.res.status, json: out.json, text: out.text };
}

function describeFailure(label, out) {
  // A STAND-DOWN IS NOT A FAILED REQUEST (task 86bc0w6my). `HTTP YIELDED (the
  // ClickUp reserve)` is not an HTTP status and reads as a garbled one — the
  // same DOCTRINE 2.2 shape the sentinel was made a string to avoid.
  if (out.status === YIELDED_STATUS) {
    return `${label} was not sent — a scheduled job stopped at the ClickUp reserve${out.error ? `: ${out.error}` : ''}`;
  }
