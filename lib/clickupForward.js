...
const { clickupFetch, YIELDED_STATUS, underTestRunner } = require('../scripts/lib/clickup.cjs');
 ...
 * to the door now instead of replacing it, so a substitute transport is still
 * a counted request. A caller does not get to opt out of the budget by
 * bringing its own `fetch`.
 */
/*
 ...
 */
/*
 * `underTestRunner` MOVED to the shared door (2026-09-15, task 86bc125u6) and
 * is imported above. The door's ledger needs the same predicate — a test run
 * spends nothing of the token, so it must not be counted against the reserve —
 * and two copies of "what is a test run" is two things that can come to
 * disagree, quietly, in opposite directions. The behaviour here is unchanged.
 */
...
async function clickupRequest(method, path, body, deps) {
  ...
  const out = await clickupFetch(
    ...
    // The door reads `env` for WHO IS ASKING and for the reserve. Handing it
    // `deps.env` when a caller supplied one is what lets a test drive the
    // reserve without mutating the real `process.env` — and, since a test run
    // no longer spends the ledger (task 86bc125u6), it is the ONLY way a test
    // can still force the yield this module has to survive.
    (deps?.fetchImpl || deps?.env)
      ? { ...(deps.fetchImpl ? { fetchImpl: deps.fetchImpl } : {}), ...(deps.env ? { env: deps.env } : {}) }
      : undefined,
  );
