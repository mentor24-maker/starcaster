...
const OPERATOR_USER_ID = Number(process.env.CLICKUP_OPERATOR_ID || 48012725);
...
function describeFailure(label, out) {
  const detail = out.json?.err || out.json?.error || String(out.text || '').slice(0, 200);
...
async function createHeldTask({ name, markdownDescription, tags = [], priority } = {}, deps = {}) {
  ...
  if (!landedHeld(check.data, assignees)) {
    ...
    if (!check.ok || !landedHeld(check.data, assignees)) {
      await clickupRequest('DELETE', `/api/v2/task/${taskId}`, undefined, deps);
