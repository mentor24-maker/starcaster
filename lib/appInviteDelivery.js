...
function buildInviteEmailHtml({ inviterName, projectName, signupUrl }) {
  ...
  const where = project
    ? `<p>You'll start in <strong>${project}</strong>.</p>`
  ...
  return `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111827; max-width: 560px;">
      ...
      <p style="margin: 2rem 0;">
        <a href="${url}" style="display: inline-block; padding: 12px 20px; background: #0b3d7a; color: #fff; text-decoration: none; border-radius: 6px; font-weight: 600;">Create your account</a>
      </p>
...
async function sendInviteEmail({ req, toEmail, rawToken, inviterName, projectName }) {
  ...
  return sendEmail({
    to,
    subject: project ? `You're invited to ${project} on StarCaster` : "You're invited to StarCaster",
    html: buildInviteEmailHtml({ inviterName, projectName: project, signupUrl }),
    text: `${safeText(inviterName) || 'A StarCaster admin'} has invited you to create a StarCaster account.\n\n${signupUrl}\n\nThis invitation expires in 7 days.`,
  });
}
