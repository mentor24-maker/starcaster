...
function resolveClashes(rows) {
  let n = 1;
  for (let clashes = destinationClashes(rows); clashes.length; clashes = destinationClashes(rows)) {
    for (const r of clashes) { n += 1; r.dest = { ...r.dest, path: renamed(r.dest.path, n) }; }
  }
  return rows;
}
