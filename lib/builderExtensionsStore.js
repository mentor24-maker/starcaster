...
async function ensureDefaultExtensions(rows) {
  ...
  const defaults = [
    ...
    },
    {
      ...
      status: 'active',
      tags: 'blog, tags, seo',
      summary: 'Add your existing blog tags to every post that clearly matches them — one click in the Blog Links manager, with Undo.',
      definition: 'Reads every blog post in the project and adds the existing tags that clearly fit it. Every tag gets a word profile built from the posts that already carry it, so a post about kids\' camps matches "junior tennis" without using those words; a post is tagged when it scores against that profile and at least one word in the post earned it. Never invents a tag, never touches a tag the author set, runs ten posts at a time with a progress count, and one click undoes exactly what the run added. Deterministic — no AI call, no model tokens.',
      launchPageId: '',
      isFeatured: true,
      usageCount: 0,
    },
