...
function extractDudaAlternating2Col(rawHtml) {
  ...
  for (const chunk of chunks) {
    ...
    // Only process 2-column rows — skip header/footer rows with 3+ columns.
    const groupDivs = chunk.match(/<div[^>]+class="[^"]*flex-element[^"]*\bgroup\b[^"]*"/gi) || [];
    if (groupDivs.length !== 2) continue;
