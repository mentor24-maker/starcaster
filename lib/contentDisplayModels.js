...
function sanitizeImportHtml(html) {
  ...
  // Promote a <p> whose sole content is a <b> wrapper to <h2>.
  out = out.replace(/<p>\s*<b>([\s\S]*?)<\/b>\s*<\/p>/gi, '<h2>$1</h2>');
  // Same for a bare <b> that is the only thing on its line.
  out = out.replace(/^<b>([\s\S]*?)<\/b>$/gim, '<h2>$1</h2>');
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const mod of (section.modules || [])) {
      if (mod.type === 'image' || mod.type === 'floating-image') {
        ...
        mod.text = sanitizeImportHtml(block.html);
