...
function sanitizeImportHtml(html) {
  ...
  // If the bold element contains a <br>, split there: text before → <h2>, text after → <p>.
  out = out.replace(/<p>\s*<(b|strong)>([\s\S]*?)<\/\1>\s*<\/p>/gi, (_m, _tag, content) => {
    const br = content.match(/^([\s\S]*?)<br\s*\/?>([\s\S]*)$/i);
    if (br) {
      const heading = br[1].trim();
      const body = br[2].trim();
      return `<h2>${heading}</h2>${body ? `<p>${body}</p>` : ''}`;
    }
    return `<h2>${content}</h2>`;
  });
  // Same for a bare <b>/<strong> that is the only thing on its line.
  out = out.replace(/^<(b|strong)>([\s\S]*?)<\/\1>$/gim, (_m, _tag, content) => {
    const br = content.match(/^([\s\S]*?)<br\s*\/?>([\s\S]*)$/i);
    if (br) {
      const heading = br[1].trim();
      const body = br[2].trim();
...
function identifyHeaderLines(html) {
  ...
  // If the bold content itself contains a <br>, split there first.
  out = out.replace(/^<(b|strong)>([\s\S]*?)<\/\1>(\s*<br\s*\/?>)/gim, (_m, _tag, content, trail) => {
    const br = content.match(/^([\s\S]*?)<br\s*\/?>([\s\S]*)$/i);
    if (br) {
      const heading = br[1].trim();
      const body = br[2].trim();
      return `<h2>${heading}</h2>${trail}${body ? '\n' + body : ''}`;
    }
    return `<h2>${content}</h2>${trail}`;
  });
...
/**
 * Like applyBlocksToSections but also runs user-defined handlers (from
 * contentHandlersStore) after the built-in sanitize+header pipeline.
 */
function applyBlocksToSectionsWithHandlers(layoutSections, blocks, userHandlers) {
  const { applyHandlers } = require('./contentHandlersStore');
  const sections = JSON.parse(JSON.stringify(layoutSections));
  const fillable = sections.filter((s) => !s.locked);
  const count = Math.min(blocks.length, fillable.length);
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const mod of (section.modules || [])) {
      if (mod.type === 'image' || mod.type === 'floating-image') {
        ...
        const base = identifyHeaderLines(sanitizeImportHtml(block.html));
        mod.text = userHandlers && userHandlers.length ? applyHandlers(base, userHandlers) : base;
      } else if (mod.type === 'heading') {
        const hM = block.html.match(/<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>/i);
        mod.text = hM ? hM[1].replace(/<[^>]+>/g, '').trim() : '';
      }
...
module.exports = { MODELS, getModel, listModels, detectModel, applyBlocksToSections, applyBlocksToSectionsWithHandlers, sanitizeImportHtml, identifyHeaderLines };
