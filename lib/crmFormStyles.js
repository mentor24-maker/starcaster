...
function crmFormStylesToRenderStyles(styles, legacyAccentColor, renderContext) {
  ...
  return {
    ...
    shell: {
      ...
      // Fill the column even when the cell's content alignment turns it into a
      // centering/right flex column. Without this, `width:auto` collapses the
      // form to its content width (empty inputs → tiny pills). A stretched flex
      // item's margin box fills the track, so this does NOT reintroduce the
      // margin-overflow that forced `auto` above. Ignored in normal block flow.
      alignSelf: 'stretch',
    },
    heading: {
      ...cssVars,
