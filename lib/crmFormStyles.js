...
/**
 * The choices a `select` field offers, cleaned the way crmConfigStore cleans
 * them: an array of trimmed, non-empty strings; anything else becomes [].
 */
function sanitizeFormFieldOptions(options) {
  if (!Array.isArray(options)) return [];
  return options.map((option) => String(option == null ? '' : option).trim()).filter(Boolean);
}

/**
 * The stored shape of one CRM form field.
 *
 * `options` is part of it, and that is the whole point of this helper existing:
 * three separate places used to rebuild a field as {key,label,type,required}
 * and drop the dropdown's choices on the floor — the read path, the write path,
 * and the styles-meta write path. A published page then received a `select`
 * with nothing in it, and the module (correctly) left the field out, so the
 * question was never asked and the answer was never collected, with the save
 * reporting success throughout. One helper so the three cannot drift again.
 */
function sanitizeFormField(field) {
  return {
    ...
    options: sanitizeFormFieldOptions(field?.options),
  };
}
...
function embedFormStylesMeta(fields, styles, legacyAccentColor) {
  const normalized = normalizeCrmFormStyles(styles, legacyAccentColor);
  const list = publicFormFields(fields).map(sanitizeFormField);
  list.push({
...
function crmFormStylesToRenderStyles(styles, legacyAccentColor, renderContext) {
  ...
  return {
    ...
    shell: {
      ...cssVars,
      // `auto`, not `100%` — a 100%-wide box plus a non-zero margin overflows
      // its column by the margin on each side.
      width: 'auto',
      maxWidth: '100%',
    ...
    // Grid track sizing lives in _builder-react.css so a container query can
    // stack label-over-field when the form sits in a narrow column.
    form: {
      ...cssVars,
      width: normalized.fieldWidth,
      maxWidth: '100%',
