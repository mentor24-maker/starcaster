...
import { normalizeProximityPlacement } from "./effect-placement";
...
/**
 * WHERE the effect sits — Window Center or In Place.
 *
 * These live in `effect-placement.ts` now (2026-09-25, Galaxy module 1/6):
 * the Galaxy module shares the same two placements and has no use for the
 * cursor-proximity driver. They are re-exported here under their original
 * names so nothing that imports them from this file had to change.
 */
export {
...
export function normalizeProximityEffectSettings(settings: Record<string, string>): void {
  ...
  settings.placement = normalizeProximityPlacement(settings.placement);
