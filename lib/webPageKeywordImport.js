...
async function distillKeywordsFromWebPage(page, scope = null) {
  const key = apiKey();
  ...
  await recordAiUsage({ provider: 'anthropic', model, feature: 'webpage_keyword_import', body, scope });
...
async function runWebPageToKeywordImport(options = {}) {
  ...
  for (const page of candidates) {
    ...
    const distilled = await distillKeywordsFromWebPage(page, scope);
    if (!distilled.ok) {
      transformErrors += 1;
