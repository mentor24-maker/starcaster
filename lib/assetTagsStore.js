...
// The one predicate that reads idx_messaging_tags_project_tag's refusal. It
// lives in the messaging store only because that store was written first — it
// is a fact about the SHARED table's index, not about Messaging, and both
// writers need it or one of them dumps a constraint name into a tenant's modal.
const { isDuplicateTag, tagLikePattern } = require('./messagingTagsStore');
...
/**
 * The whole registry, for the picker. The cap matches lib/messagingTagsStore.js
 * deliberately — both stores now read the SAME table, and a picker that shows a
 * shorter list than Messaging does is the two lists coming back by another
 * route.
 */
 ...
 *
 * It asks the database for the one row rather than filtering the whole list,
 * for the reason given on queryMessagingTagRows — and here it also removes a
 * cap that was load-bearing: while this filtered a 5000-row list, a project
 * past 5000 tags stopped finding its own existing tags, so the modal would
 * insert a duplicate and the unique index would refuse it.
 *
 * Raw rows, not rowToTag: createAssetTag hands what comes back to the caller as
 * the reused row, and the route maps it.
 */
async function findAssetTagByName(tag, scope = null) {
  ...
  const query = await scopedListQuery(
    t(),
    `select=*&tag=ilike.${encodeURIComponent(tagLikePattern(tag))}&order=tag.asc&limit=200`,
    scope
  );
  const res = await sbQuery({ table: t(), query });
...
/**
 * The row that WON, when the database refused this write as a duplicate.
 *
 * The lookup pre-check in createAssetTag closes the everyday case, but two
 * admins filing photos in the same second can both pass it, and once
 * idx_messaging_tags_project_tag exists the loser gets PostgREST's raw
 * `duplicate key value violates unique constraint ...` straight into the tag
 * modal. That is the same question the pre-check already answers, so it gets
 * the same answer: here "add" and "reuse" are one action.
 *
 * Narrow on purpose (CLAUDE.md landmine 15) — `isDuplicateTag` recognises the
 * uniqueness refusal and nothing else, and a lookup that comes back empty is
 * NOT treated as a duplicate. Returns null when this was not that case, and
 * the caller returns the refusal untouched.
 */
async function existingAfterDuplicate(res, tag, scope) {
  if (!isDuplicateTag(res)) return null;
  const raced = await findAssetTagByName(tag, scope);
  if (!raced.ok) return null;
  const rows = Array.isArray(raced.data) ? raced.data : [];
  if (!rows.length) return null;
  return { ok: true, status: 200, data: rows, existed: true };
}
...
async function createAssetTag(input, scope = null) {
  ...
  const raced = await existingAfterDuplicate(res, tag, scope);
  if (raced) return raced;

  if (!source || !isMissingSourceColumn(res)) return res;
  ...
  if (!retry.ok) {
    const racedBare = await existingAfterDuplicate(retry, tag, scope);
    if (racedBare) return racedBare;
    return retry;
  }
