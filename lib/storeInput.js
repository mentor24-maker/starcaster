 ...
 *
 * Four normalisations, all for the same reason:
 *
 ...
 * - **A lowercase `z`** (`...T12:34:56z`) is valid ISO 8601 and is likewise not
 *   in the ECMAScript grammar, which spells the UTC designator `Z` and only
 *   `Z`. `ISO_DATETIME_RE` admits both cases, so a `z` reached the same
 *   fallback parser the colon-less offset did — three lines from the fix for
 *   it. Found by the review pass on this PR: a claim can be false in a way its
 *   own new code introduces.
 * - **A fraction that is not exactly three digits** (`.7`, `.123456789`) is
 *   valid ISO 8601 and is not in the ECMAScript format either, which spells
 *   the fraction `.sss`. `ISO_DATETIME_RE` deliberately admits `\d{1,9}`
 *   because ffprobe and Postgres both emit other widths, so this one arrives
 *   routinely rather than as junk. Padding and truncating to three cannot move
 *   the instant: the spec'd parser reads exactly three digits and discards the
 *   rest, which is truncation, and that is measured in the test below.
 *
 * The docstring's first sentence is the whole contract, so it has to stay
 * literally true — the last three normalisations above are each the difference
 * between "always" and "usually, on V8". It is a named, exported function
 * rather than three lines inside timestampOrError because every one of those
 * fixes is invisible from the outside on V8: the returned instant is identical
 * either way, so a test written against the return value passes whether the
 * colon is written in or not. A fix nothing can fail on is a fix that gets
 * deleted. Here the property is the string.
 */
function toSpecDateTime(text, dateOnly) {
  ...
  normalized = normalized.replace(
    /(\d{2}:\d{2}:\d{2})\.(\d{1,9})/,
    (_all, time, digits) => `${time}.${digits.padEnd(3, '0').slice(0, 3)}`,
  );
  // A missing zone becomes Z rather than the machine's own; see the note below.
  // The `z` case is already uppercased above, so this test spells only `Z` —
  // it must not re-admit the form the line above exists to remove.
