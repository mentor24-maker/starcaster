...
const NOT_REPORTING_WHY = {
  ...
  // PARKED, NOT FORGOTTEN — 2026-09-20, task 86bbjv68y.
  //
  // This role is ready to beat: workers/studio/daemon.js records one every five
  // minutes, whatever the tick concluded, and the local stamp is written the
  // first time it runs. What does not exist yet is the INSTALL. The role is
  // owned by mac-mini (lib/nodeRoles.js) and that machine has been unreachable
  // since 2026-09-10 — no Tailscale, no LAN route, expected back ~2026-10-01,
  // the same blockage that moved both loop lanes to the MacBook in #727.
  //
  // Registering it as an emitter today would put a QUIET alarm on the bus every
  // six hours for eleven days, about a daemon nobody can start and nobody can
  // clear. An alarm that cannot be acted on trains everyone to filter the bus,
  // which costs more than it buys — and this row still reports NOT REPORTING
  // WITH THE REASON rather than as healthy, so nothing is being hidden.
  //
  // WHEN IT GRADUATES, the entry is:
  //   intervalMs: 60 * 60 * 1000  (declared hourly; the daemon beats every five
  //                                minutes, so a two-hour ffmpeg encode can
  //                                never read as a dead daemon)
  //   beatMeans:  'liveness'      (the queue's `blocked` column is where the
  //                                quality of the WORK is answered)
  // which makes quietAfterFor six hours — the number slice 7/8's acceptance
  // criterion asked for.
  'studio-worker': 'Nothing has been installed to beat from yet. The daemon emits a beat every five '
    + 'minutes (workers/studio/daemon.js), but its launchd job has never been installed, because the '
    + 'role is owned by mac-mini and that machine has been unreachable since 2026-09-10 — expected '
    + 'back around 2026-10-01, the same blockage as #727. It becomes an emitter the day '
    + 'scripts/install_studio_worker.sh runs there; until then a six-hourly QUIET alarm nobody can '
    + 'clear would only teach everyone to filter the bus.',
};
...
/**
 * WHAT TO SAY ABOUT A BLOCKED LANE, said once (task 86bc3t0n1, round 3). The
 * bus post and the local stale alarm both carry it, and the local one used to
 * carry the usage-limit paragraph instead — two wordings for one finding is how
 * one of them ends up saying the opposite.
 */
const BLOCKED_NOT_WAITING = [
...
/** The two-line terminal summary under a blocked headline — roll call and stale-check alike. */
const BLOCKED_SUMMARY = [
  'This is not a machine problem, not a schedule problem, and not something that clears with time —',
  'an expired login, most likely. Nothing comes out of these lanes until somebody signs in on that machine.',
];
...
function renderStalePost({ quiet, node, now = Date.now() }) {
  // A BLOCKED ROW IS ITS OWN FINDING HERE TOO (task 86bc3t0n1, round 3). A
  // blocked row also carries `standDown`, so asking that first handed an
  // expired login the usage-limit paragraph — "every pass stood down and exited
  // cleanly" — underneath a reason that had just said a human has to sign in.
  // This is the alarm that fires on a machine that is AWAKE, which is the one
  // the 2026-09-18 outage would have tripped, so it is the last place the
  // disproven story may survive.
  const anyBlocked = quiet.some((q) => q.blocked);
  return [
    anyBlocked
      ? '⛔ **A scheduled job on this machine cannot work at all, and it will not fix itself.**'
      : '🔇 **A scheduled job has stopped running on the machine that owns it.**',
    '',
    ...quiet.map((q) => `- **${q.role}** on **${node}** — ${q.reason}`
      // No threshold on a blocked row: it has none by design, and quoting one
      // invites the reader to wait for it.
      + (q.blocked ? ` (last beat ${q.at}).` : ` (threshold ${durationText(q.thresholdMs)}; last beat ${q.at}).`)
      // Three different findings wearing one alarm, and the remedies differ.
      // "It stopped firing" sends somebody to launchd; "it keeps firing and
      // does nothing" sends them to the usage limit; "it keeps firing and
      // cannot work at all" sends them to sign in. Saying the wrong one is how
      // an alarm stops being believed.
      + (q.blocked
        ? ' _The runner is ALIVE and on schedule — every pass fired and exited with an error in about a second.'
          + ' Nothing is wrong with the schedule, and waiting will not help._'
        : q.standDown
          ? ' _The runner is ALIVE and on schedule — every pass fired, stood down and exited cleanly. Nothing is'
            + ' wrong with the machine or the schedule; the passes are not able to do any work._'
    ...
    'reassuring.',
    '',
    ...(anyBlocked ? [...BLOCKED_NOT_WAITING, ''] : []),
    'Where to look:',
    '```',
    ...(anyBlocked
      ? [
        `# FIRST: sign Claude Code in again on ${node} — open a terminal there, run claude, type /login`,
