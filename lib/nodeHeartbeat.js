 ...
 * ACTUALLY reached the bus: `stale-<role>` exists exactly when a post went out,
 * so a healthy job nobody ever reported posts nothing here, ever. That is what
 * keeps this from becoming the "all is well" x365 the non-goals forbid.
...
function alarmCloseoutPlan({ fresh = [] } = {}) {
  ...
  for (const entry of fresh) {
    ...
    // Order matters only for the reader: `stale` last, so the announce decision
    // reads directly under the clear that authorises it.
    for (const kind of ['quiet', 'failed', 'stale']) {
      ...
      if (kind === 'stale') announce.push({ role, quietSince: raw });
    }
  }
