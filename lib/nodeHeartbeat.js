...
function standDownSince({ prior, kind, at } = {}) {
  ...
  if (kind !== BEAT_STOOD_DOWN) return { since: '', note: '' };

  const p = prior || {};
  if (!p.readable) {
    return { since: now, note: 'the previous beat stamp could not be read, so this stand-down is counted as starting now' };
  }
  if (!p.found) return { since: now, note: '' };

  const priorBeat = p.beat || {};
  if (String(priorBeat.kind || BEAT_RAN) !== BEAT_STOOD_DOWN) return { since: now, note: '' };

  const carried = String(priorBeat.standingDownSince || '').trim();
  if (!carried || !Number.isFinite(Date.parse(carried))) {
    return {
      since: now,
      note: 'the previous stand-down recorded no readable start instant, so this one is counted as starting now',
    };
  }
...
function recordBeat({
  ...
  beat.kind = kind === BEAT_STOOD_DOWN ? BEAT_STOOD_DOWN : BEAT_RAN;
  if (beat.kind === BEAT_STOOD_DOWN) {
...
function standDownCell(row, now) {
  if (String((row && row.kind) || BEAT_RAN) !== BEAT_STOOD_DOWN) return 'ran';
  ...
  return `**stood down** — nothing claimed for ${forText} (${(row && row.why) || 'no reason recorded'})`;
}
...
function renderRollCall(rows, { now = Date.now() } = {}) {
  ...
  return [
    ...
    '',
    'Each row is the last time a scheduled job on that machine finished. The last column says whether',
    'that pass did its work or STOOD DOWN — fired, found it could not work (a usage limit), and exited',
    'cleanly. A stand-down beats like any other pass, so without that column 90 hours of doing nothing',
    'reads as 90 hours of perfect health; it did, from 2026-09-16 to 2026-09-19. A job that',
...
function rollCallReport({
  ...
  if (!readable) {
    return { readable: false, why, beating: [], overdue: [], stoodDown: [], notReporting: [], silent: false };
  }
  ...
  for (const role of Object.keys(roles)) {
    ...
    } else if (String(row.kind || BEAT_RAN) === BEAT_STOOD_DOWN) {
  ...
  return {
    ...
    stoodDown,
    notReporting,
    silent: overdue.length > 0 || alarmingStandDowns.length > 0,
  };
}
...
function rollCallPushPlan({ entries = [], now = Date.now(), everyMs = PUSH_EVERY_MS } = {}) {
  ...
  for (const entry of entries) {
    ...
    const kind = String((beat.beat && beat.beat.kind) || BEAT_RAN) === BEAT_STOOD_DOWN ? BEAT_STOOD_DOWN : BEAT_RAN;
    ...
    push.push({
      ...row,
      why: kindChanged
        ? `its last pass ${kind === BEAT_STOOD_DOWN ? 'stood down' : 'ran'} where the pushed row says it ${lastPushKind === BEAT_STOOD_DOWN ? 'stood down' : 'ran'} — a change of kind is pushed at once, not at the next daily slot`
...
function alarmCloseoutPlan({ fresh = [] } = {}) {
  ...
  for (const entry of fresh) {
    ...
    const beatKind = String((entry && entry.beatKind) || BEAT_RAN);
    ...
    for (const kind of ['quiet', 'failed', 'stale', 'standdown']) {
      ...
      if (kind === 'standdown' && beatKind === BEAT_STOOD_DOWN) {
        keep.push({
          role,
          kind,
          why: 'its newest beat is itself a stand-down, and only a pass that does real work closes this alarm',
        });
        continue;
      }
...
function recencyReport({ entries = [], now = Date.now(), emitters = BEAT_EMITTERS } = {}) {
  ...
  for (const entry of entries) {
    ...
    // stopped working.
    const kind = String((beat.beat && beat.beat.kind) || BEAT_RAN);
