 ...
 * against.
 *
 * FRESHNESS IS THE GATE, NOT "A BEAT ARRIVED". A relay carrying a stamp from
 * Tuesday must not close Tuesday's alarm on Friday — that is the same safety
 * property `rollCallPushPlan` protects, and breaking it here would produce
 * clear-then-realarm churn instead of an honest silence. So the input is
 * `recencyReport().fresh`: roles whose newest local beat is inside their own
 * threshold, judged by the same arithmetic the alarm was raised with.
 *
 * TWO OUTPUTS, BECAUSE THEY ARE TWO DIFFERENT ACTS. `clear` is bookkeeping —
 * every fresh role's suppression stamps go, whether or not anything was ever
 * said. `announce` is the only good news this system ever posts, and it is
 * restricted to the roles whose silence was ACTUALLY reported to the bus: the
 * `stale-<role>` stamp exists exactly when a post went out, so a healthy job
 * that was never reported quiet posts nothing here, ever. That is what keeps
 * this from becoming the "all is well" x365 the non-goals forbid.
 *
 * Pure. `quietSince` is a plain map of role -> whatever the caller's
 * `stale-<role>` stamp says (absent or empty means no alarm was posted); the
 * stamp IO stays in `scripts/node_heartbeat.mjs`, so exactly one file knows
 * where those stamps live (vault doctrine NODES, principle P1).
 */
function alarmCloseoutPlan({ fresh = [], quietSince = {} } = {}) {
  const clear = [];
  const announce = [];
  ...
  for (const entry of fresh) {
    ...
    if (!role) continue;
    clear.push(role);
    const since = String((quietSince && quietSince[role]) || '').trim();
    if (since) announce.push({ role, quietSince: since });
  }

  return { clear, announce };
}
