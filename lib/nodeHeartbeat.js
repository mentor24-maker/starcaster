...
function rollCallPushPlan({ entries = [], now = Date.now(), everyMs = PUSH_EVERY_MS } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (!dueAgain({ lastAt: lastPushAt, now, everyMs })) {
      held.push({ role, why: `its row was pushed within the last ${ageText(everyMs).replace(' ago', '')}` });
      continue;
    }
    push.push({ role, at, why: lastPushAt ? `its local beat (${at}) is newer than its last push (${lastPushAt})` : `its local beat (${at}) has never been pushed` });
  }
...
function alarmCloseoutPlan({ fresh = [] } = {}) {
  ...
  for (const entry of fresh) {
    ...
    // of `failed`, which is the one kind that never announces.
