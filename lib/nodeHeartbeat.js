...
const BEAT_EMITTERS = {
  ...
  'studio-worker': {
    // THE CADENCE IS DECLARED AS AN HOUR AND THE DAEMON BEATS EVERY FIVE
    // MINUTES, and the gap is the point rather than sloppiness. `quietAfterFor`
    // is intervalMs x 6 floored at three hours, so an hour here is a six-hour
    // silence threshold — which is the number Studio 7/8 asked for. Beating
    // twelve times inside each declared interval is what makes that honest: a
    // single ordinary job here is a two-hour ffmpeg encode of a wedding video,
    // and a beat recorded only between jobs would report the busiest daemon in
    // the building as dead.
    //
    // LIVENESS, NOT SUCCESS, for the same reason the loop lanes are: what this
    // catches is the daemon not running at all — a reboot that came back to a
    // login screen, a launchd job that never loaded, a crash loop inside
    // ThrottleInterval. Whether the WORK is any good is a different question
    // with a different surface: the queue's own `blocked` column, which keeps
    // every job's reason and can be read by a person.
    intervalMs: 60 * 60 * 1000,
    beatMeans: 'liveness',
    why: 'Kept alive by launchd (scripts/install_studio_worker.sh); workers/studio/daemon.js records '
      + 'a beat every five minutes whatever the tick concluded, including a tick that found nothing '
      + 'to do.',
  },
};
