...
async function callClaude({ model, system, prompt, content, schema, tools, expectText = false, maxTokens = 4000, apiKey }) {
  ...
  // Every wizard round funnels through callClaude(), so one meter here covers
  // all of them. `usage` above is still returned for the per-run display; this
  // row is the durable ledger entry. No scope reaches this module, so the row
  // is unattributed to a tenant.
  await recordAiUsage({ provider: 'anthropic', model: servedBy, feature: 'theme_wizard', body: payload });
...
/**
 * Describe a reference site's hero/banner area (spec: the operator can paste a
 * URL whose top-of-page treatment they want the looks to learn from). Prose in,
 * prose out — it feeds the candidate prompt, not a schema.
 */
async function describeHeroReference({ url }, apiKey) {
  const target = clean(url);
  if (!target) return { ok: true, text: '', usage: null };
  return callClaude({
    model: MODEL_UTILITY,
    system: 'You look at websites and describe their visual design precisely, for a designer who cannot see them.',
    prompt: `Fetch ${target} and describe ONLY the hero/banner area at the top of the page: `
      + 'how the imagery is treated (overlay? tint? crop?), where the headline sits and how it is set, '
      + 'what the calls-to-action look like, and how the band below the hero relates to it. '
      + 'Be concrete about colours and contrast. If the page cannot be read, say so plainly.',
    tools: [{ type: 'web_fetch_20260209', name: 'web_fetch', max_uses: 3 }],
    maxTokens: 2000,
    apiKey,
    expectText: true,
...
async function generateCandidate({ styleBrief, direction, lockedValues = {}, parentPatch = null, feedback = '', heroImage = null }, apiKey) {
  ...
  // A hero banner turns the palette from taste into observation: the image
  // goes in as an image block and the look is KEYED to it — colours pulled
  // from or deliberately set against what is actually in the photograph.
  const content = heroImage && heroImage.url
    ? [
        { type: 'image', source: { type: 'url', url: heroImage.url } },
        {
          type: 'text',
          text: 'This is the hero banner this look is keyed to. It will sit at the top of the page '
            + 'under your heroOverlay tint, with headlines in your palette on top of it. Draw the '
            + 'palette from this image — its dominant tones, its light, one colour worth amplifying — '
            + 'and choose an overlay tint and opacity that makes text readable on THIS image without '
            + 'burying it.'
            + (clean(heroImage.note) ? `\n\nThe operator says about the banner area: ${clean(heroImage.note).slice(0, 2000)}` : '')
            + '\n\n' + prompt,
        },
      ]
    : null;
