...
async function distillTagsFromWebPage(page, scope = null) {
  const key = apiKey();
  ...
  await recordAiUsage({ provider: 'anthropic', model, feature: 'webpage_tag_import', body, scope });
...
async function runWebPageToTagImport(options = {}) {
  ...
  for (const page of candidates) {
    ...
    const distilled = await distillTagsFromWebPage(page, scope);
    if (!distilled.ok) {
      transformErrors += 1;
    ...
    for (const tag of distilled.tags) {
      ...
      } else if (isDuplicateRefusal(result)) {
        // Already there — which is what this importer was asking for. See
        // lib/importDuplicateSkip.js: a forced re-run reports every existing
        // tag this way, and counting those as errors made it a 500.
        skipped += 1;
        existingByContent.add(key);
      } else {
