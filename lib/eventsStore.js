...
const { readRecurrence, readOverrides } = require('./eventRecurrence');
 ...
 *
 * The rule and the collision handling now live in lib/slugify.js, shared with
 * the blog — an address is a promise about where something lives, and two
 * copies of that rule drift apart quietly. Behaviour here is unchanged except
 * that accented letters fold ("Café" is "cafe") instead of being dropped.
 */
async function resolveSlug(requested, title, scope, excludeId = '') {
  return resolveUniqueSlug(
    requested,
    title,
    (candidate) => getEventBySlug(candidate, scope),
    excludeId
  );
}
...
function sanitize(input) {
  ...
  return {
    ...
    // '' = no category. An id that no longer exists reads the same way.
    categoryId:       String(input.categoryId ?? input.category_id ?? ''),
    // null = a one-off event. Read leniently: a hand-edited row drops its
    // unreadable rule rather than taking the calendar down with it.
    recurrence:          readRecurrence(input.recurrence),
    recurrenceOverrides: readOverrides(input.recurrenceOverrides ?? input.recurrence_overrides),
...
function toRow(e) {
  return {
    ...
    recurrence_overrides: e.recurrenceOverrides,
