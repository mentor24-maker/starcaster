...
async function resolveTypeIdForFormat(formatName, scope) {
  const formatRes = await getMessagingFormatByName(formatName, scope);
  if (!formatRes.ok) return formatRes;
  const typeId = Number(formatRes.data?.id || 0) || 0;
  if (!typeId) {
    return {
      ok: false,
      status: 404,
      error: `No Content Format id found for "${formatName}".`,
    };
  }
  return { ok: true, status: 200, data: typeId };
}
...
async function prepareContentItemRow(input, scope) {
  ...
  const typeRes = await resolveTypeIdForFormat(formatName, scope);
