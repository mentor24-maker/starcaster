...
function inputToRow(input) {
  ...
  // Publish fields are optional columns (docs/develop_landing_page_publish_migration.sql).
...
async function propagateCanonicalSection(savedSectionId, updatedSection, scope) {
  ...
  const pagesResult = await listLandingPages(5000, scope);
  ...
  for (const page of pagesResult.data) {
    ...
    await updateLandingPage(page.id, { layoutSections: updatedSections }, scope);
  }
}
