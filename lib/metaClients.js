...
function getFacebookCredentials() {
  ...
  return {
    accessToken: safeText(process.env.FACEBOOK_ACCESS_TOKEN) || safeText(saved.access_token),
    pageId: safeText(process.env.FACEBOOK_PAGE_ID) || safeText(saved.page_id),
    appId: safeText(process.env.FACEBOOK_APP_ID) || safeText(saved.app_id),
    appSecret: safeText(process.env.FACEBOOK_APP_SECRET) || safeText(saved.app_secret),
    baseUrl: addNoTrailSlash(safeText(process.env.FACEBOOK_BASE_URL) || safeText(saved.base_url) || 'https://graph.facebook.com/v22.0'),
  };
}

function isFacebookConfigured(creds = getFacebookCredentials()) {
  return Boolean(creds.accessToken && creds.pageId);
}
