...
// The DB cascades this delete to the session's candidates and jobs
// (docs/SQL/theme_wizard_setup.sql), so one row removal retires the whole run.
async function deleteSession(id, scope = null) {
  const sessionId = safeText(id);
  if (!sessionId) return { ok: false, status: 400, error: 'session id is required' };
  const query = await scopedIdQuery(tableSessions(), `id=eq.${encodeURIComponent(sessionId)}&select=*`, scope);
  ...
  if (!removed) return { ok: false, status: 404, error: 'Session not found' };
  return { ok: true, status: 200, data: rowToSession(removed) };
}
...
async function createCandidate(input, scope = null) {
  ...
  const row = await scopedInsertRow(tableCandidates(), {
    ...
    // NULL, never ''. Round 1 has no parent, and parent_candidate_id carries a
    // foreign key — an empty string is a real value that matches no row, so
    // every first-round insert would be rejected.
    parent_candidate_id: safeText(input?.parentCandidateId) || null,
...
async function updateCandidate(id, input, scope = null) {
  ...
  if (input?.parentCandidateId !== undefined) row.parent_candidate_id = safeText(input.parentCandidateId) || null;
