...
function tableLibraryEntries() { return tableConfig().themeLibraryEntries; }
function tableSnapshots() { return tableConfig().themeWizardApplySnapshots; }
...
async function createApplySnapshot(input, scope = null) {
  const snapshotId = safeText(input?.id, 120) || nextId('twas');
  const row = await scopedInsertRow(tableSnapshots(), {
    id: snapshotId,
    session_id: safeText(input?.sessionId),
    project_id: safeText(input?.projectId),
    pages: input?.pages || [],
  ...
  const res = await sbQuery({ method: 'POST', table: tableSnapshots(), query: 'select=*', headers: { Prefer: 'return=representation' }, body: [row] });
  if (!res.ok) return res;
  const created = Array.isArray(res.data) ? res.data[0] : res.data;
  return { ok: true, status: 201, data: rowToSnapshot(created) };
}

async function getLatestApplySnapshot(sessionId, scope = null) {
  const safeSessionId = safeText(sessionId);
  if (!safeSessionId) return { ok: false, status: 400, error: 'session id is required' };
  const query = await scopedListQuery(tableSnapshots(), `select=*&session_id=eq.${encodeURIComponent(safeSessionId)}&order=created_at.desc&limit=1`, scope);
  const res = await sbQuery({ method: 'GET', table: tableSnapshots(), query });
  if (!res.ok) return res;
  const row = Array.isArray(res.data) ? res.data[0] : res.data;
