...
const { detectModel: detectContentModel } = require('./contentDisplayModels');
const {
...
const {
  ...
  deleteDirectAcquireRun: deleteRunFromStore,
  purgeOldAcquireRuns: purgeOldRunsFromStore,
  patchDirectAcquireRunPages: patchRunPagesFromStore,
...
/**
 * Lightweight structural strip: removes non-content chrome (scripts, nav, header,
 * footer, etc.) but PRESERVES tag attributes so content-model extractors can
 * read image URLs, data-widget-type, data-dm-image-path, etc.
 * Max 150 K chars returned — large enough for any interior page.
 */
function extractRawContent(html) {
  ...
  // DudaMobile sites expose a .dmBody container followed by .dmFooterContainer
  const dmBodyM = s.match(/class="[^"]*dmBody[^"]*"[^>]*>([\s\S]*)/i);
  if (dmBodyM) {
    const footerIdx = dmBodyM[1].search(/class="[^"]*(?:dmFooter|dmFooterContainer)[^"]*"/i);
    s = footerIdx >= 0 ? dmBodyM[1].substring(0, footerIdx) : dmBodyM[1];
  } else {
    // Standard sites: prefer <main> or <article>
    ...
    if (mainM) s = mainM[0];
      ...
      if (articleM) s = articleM[0];
    }
  }

  return s.substring(0, 150000);
}

function extractContentHtml(html, maxChars) {
  const max = Number(maxChars) || 10000;
  let s = String(html || '')
    .replace(/<script[\s\S]*?<\/script>/gi, '')
    .replace(/<style[\s\S]*?<\/style>/gi, '')
    .replace(/<noscript[\s\S]*?<\/noscript>/gi, '')
    ...
    .replace(/<aside[\s\S]*?<\/aside>/gi, '')
    .replace(/<form[\s\S]*?<\/form>/gi, '');
  // Prefer a focused content region if present
  const mainM = s.match(/<main[\s\S]*?<\/main>/i);
  if (mainM) { s = mainM[0]; }
  else {
    const articleM = s.match(/<article[\s\S]*?<\/article>/i);
  ...
  // Remove void/media elements that have no text value
  s = s.replace(/<(img|input|link|meta|source|embed|area|base|col|param|track|wbr)[^>]*\/?>/gi, '');
  // Strip all tag attributes (security + cleanliness)
  s = s.replace(/<(\/?[\w]+)[^>]*>/g, '<$1>');
  // Unwrap pure layout/structural tags, keeping their inner content
  s = s.replace(/<\/?(div|section|article|main|body|html|span|figure|picture)[^>]*>/gi, ' ');
  // Condense whitespace
  ...
  if (s.length > max) {
    ...
    if (lastOpen > lastClose) s = s.slice(0, lastOpen).trim();
  }
  return s;
}
...
async function rerankKeywordCandidatesWithAi({ sourceUrl, pages, exclusions, candidates }) {
  ...
  // rerankKeywordCandidatesWithAi() takes no scope, so this row is unattributed.
  await recordAiUsage({
    provider: 'anthropic',
    model: 'claude-sonnet-5',
    feature: 'direct_acquire_keyword_rerank',
    body,
  });
...
async function fetchSitemapUrls(baseUrl, limit = 80) {
  let origin;
  try { origin = new URL(baseUrl).origin; } catch { return []; }
  const candidates = [`${origin}/sitemap.xml`, `${origin}/sitemap_index.xml`];
  for (const sitemapUrl of candidates) {
    let xml;
    try { xml = await fetchPage(sitemapUrl); } catch { continue; }
    const locs = [];
    // Create regex fresh each iteration — g-flag regexes retain lastIndex across calls
    const locRegex = /<loc>\s*([\s\S]*?)\s*<\/loc>/gi;
    let m;
    while ((m = locRegex.exec(xml)) !== null) {
      const loc = m[1].trim();
      // A sitemap index contains sub-sitemap URLs — fetch the first sub-sitemap
      if (loc.endsWith('.xml')) {
        try {
          const subXml = await fetchPage(loc);
          let sm;
          const subRegex = /<loc>\s*([\s\S]*?)\s*<\/loc>/gi;
          while ((sm = subRegex.exec(subXml)) !== null) {
            const sub = sm[1].trim();
            if (!sub.endsWith('.xml') && sub.startsWith(origin)) locs.push(sub);
            if (locs.length >= limit) break;
          }
        } catch { /* ignore broken sub-sitemap */ }
        break;
      }
      if (loc.startsWith(origin)) locs.push(loc);
      if (locs.length >= limit) break;
    }
...
async function fetchSiteSearchSnippets(sourceDomain) {
  const config = resolveWebSearchConfig();
  if (!config.configured) return [];
  const result = await fetchWebSearchBatch(`site:${sourceDomain}`, 0, config);
  if (!result.ok || !Array.isArray(result.items) || !result.items.length) return [];
  return result.items.map((item) => ({
    url: String(item.link || '').trim(),
    title: String(item.title || '').trim(),
    meta_desc: String(item.snippet || '').trim(),
    meta_keywords: [],
    headings: [],
    body_text: String(item.snippet || '').trim(),
    body_snippet: String(item.snippet || '').trim(),
    body_html: '',
    body_raw: '',
    emails: [],
    phones: [],
...
async function crawlProjectPagesForKeywords(input) {
  ...
  const snippetChars = Math.max(100, Math.min(Number(input.body_snippet_chars) || 2000, 5000));
  ...
  // Seed the queue from sitemap.xml so JS-rendered sites still yield content pages
  try {
    const sitemapUrls = await fetchSitemapUrls(sourceUrl, maxPages * 4);
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    try {
      ...
      const pageRecord = {
        ...
        body_html: extractContentHtml(html, 10000),
        body_raw: extractRawContent(html),
        emails,
        phones,
  ...
  // If the crawl yielded very few keyword signals (JS-rendered homepage with no crawlable content),
  // supplement with search-engine snippets which come from the fully-rendered index.
  const candidateCount = buildGlobalKeywordCandidates(pages, keywordExclusions).length;
  if (candidateCount < 8) {
    try {
      const snippetPages = await fetchSiteSearchSnippets(sourceDomain);
      snippetPages.forEach((p) => pages.push(p));
    } catch { /* search supplement is optional */ }
  }
...
async function runDirectAcquire(input, scope = null) {
  ...
  pages.forEach((page) => {
    ...
    delete page.image_items; // body_html is kept — used by Builder populate
  });
  const run = {
    run_id: runId,
    source_url: sourceUrl,
  ...
  // Auto-detect content model from the first page that has raw HTML.
  if (!run.detectedModelId) {
    const samplePage = pages.find((p) => p.body_raw);
    if (samplePage) {
      const detected = detectContentModel(samplePage.body_raw);
      if (detected) run.detectedModelId = detected.id;
    }
  }
...
async function deleteDirectAcquireRun(runId, scope = null) {
  return deleteRunFromStore(runId, scope);
}

async function purgeOldAcquireRuns(scope = null) {
  return purgeOldRunsFromStore(scope);
}

async function patchDirectAcquireRunPages(runId, options, scope = null) {
  return patchRunPagesFromStore(runId, options, scope);
}
