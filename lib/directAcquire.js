...
function countPageKeywordUnits(value, options = {}) {
  ...
  const tokens = tokenizeKeywordTerms(value).slice(0, maxTokens);
...
function buildGlobalKeywordCandidates(pages, exclusions = []) {
  ...
  (Array.isArray(pages) ? pages : []).forEach((page, pageIndex) => {
    ...
    applyCounts(countPageKeywordUnits(sources.title, { maxTokens: 20, maxNgramSize: 3 }), 7.5, 'title');
    applyCounts(countPageKeywordUnits(sources.meta_desc, { maxTokens: 40, maxNgramSize: 3 }), 4.5, 'meta_desc');
    sources.meta_keywords.forEach((value) => {
      addSource(value, 12, 'meta_keywords', pageIndex);
      applyCounts(countPageKeywordUnits(value, { maxTokens: 12, maxNgramSize: 3 }), 6.5, 'meta_keywords');
    });
    sources.headings.forEach((value, index) => {
      addSource(value, index < 2 ? 10 : 7.5, 'heading', pageIndex);
      applyCounts(countPageKeywordUnits(value, { maxTokens: 24, maxNgramSize: 3 }), index < 2 ? 5.5 : 4.2, 'heading');
    });
    applyCounts(countPageKeywordUnits(sources.body_text, { maxTokens: 2200, maxNgramSize: 3 }), 1, 'body');
  });
    ...
    .map((entry) => {
      ...
      const fieldBoost = entry.field_hits.has('title') || entry.field_hits.has('heading') || entry.field_hits.has('meta_keywords')
        ...
        : entry.field_hits.has('meta_desc')
          ? 1.15
          : 1;
      const phraseBoost = entry.term_count === 1 ? 0.82 : entry.term_count === 2 ? 1.2 : 1.34;
      const singletonPenalty = entry.term_count === 1 && df === 1 && !entry.field_hits.has('title') && !entry.field_hits.has('heading')
        ? 0.45
        : 1;
      const score = entry.raw_score * (0.6 + idf) * fieldBoost * phraseBoost * singletonPenalty;
      return {
