...
async function buildPeerSitesSummary(keywordSummary, sourceUrl) {
  const queries = (Array.isArray(keywordSummary?.top_keywords) ? keywordSummary.top_keywords : [])
    .slice(0, 10)
    .map((item) => String(item?.keyword || '').trim())
    .filter(Boolean);
  if (!queries.length) {
    return { configured: false, provider: 'google_custom_search', error: 'No keyword candidates available for peer site discovery.', peers: [], suggested_models: [], searched_keywords: [] };
  }
  const config = getGoogleSearchConfig();
  if (!config.configured) {
    return { configured: false, provider: 'google_custom_search', error: 'Google Programmable Search credentials are not configured.', peers: [], suggested_models: [], searched_keywords: queries };
  }
...
async function runDirectAcquire(input) {
  ...
  const peerSummary = await buildPeerSitesSummary(keywordSummary, sourceUrl);
