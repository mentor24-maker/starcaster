...
async function fetchGoogleSearchBatch(query, startIndex, config) {
...
async function buildPeerSitesSummary(keywordSummary, sourceUrl, options = {}) {
  ...
  const queries = (Array.isArray(keywordSummary?.top_keywords) ? keywordSummary.top_keywords : [])
    ...
    .filter(Boolean);
  if (!queries.length) {
    return { enabled: true, configured: false, provider: 'google_custom_search', error: 'No keyword candidates available for peer site discovery.', peers: [], suggested_models: [], searched_keywords: [], raw_results_count: 0, unique_domains_count: 0, errors: [] };
  }
  const config = getGoogleSearchConfig();
  if (!config.configured) {
    return { enabled: true, configured: false, provider: 'google_custom_search', error: 'Google Programmable Search credentials are not configured.', peers: [], suggested_models: [], searched_keywords: queries, raw_results_count: 0, unique_domains_count: 0, errors: [] };
  }
...
async function runDirectAcquire(input) {
  ...
  const maxPages = Math.max(1, Math.min(Number(input.max_pages) || 5, 100));
  const peerSitesLimit = Math.max(5, Math.min(Number(input.peer_sites_limit) || 20, 100));
  const imagesLimit = Math.max(5, Math.min(Number(input.images_limit) || 20, 100));
...
function getLatestDirectAcquireRun() {
  const store = readStore();
  return Array.isArray(store.runs) && store.runs.length ? store.runs[0] : null;
}
