...
function getBlueskyCredentials() {
  const stored = environmentValues('bluesky');
...
/**
 * This project's own Bluesky connection if it has a live one, otherwise the
 * platform-wide credentials. Connections 3 of 7 (86bbpz1ed).
 *
 * A refusal comes back BLANKED — no identifier, no app password — so a lookup
 * that could not answer cannot be passed into `createPost` and go out under
 * Starcaster's own handle. `isConfigured` is false on it, and `createSession`
 * checks `ok === false` first so the resolver's own sentence reaches the caller
 * instead of the generic "credentials are missing".
 */
async function resolveBlueskyCredentials({ projectId, userId } = {}) {
  const global = getBlueskyCredentials();
  const pid = safeText(projectId);
  if (!pid) return { ...global, source: 'environment' };

  const resolved = await resolveCredentials('bluesky', pid, { userId });
  if (!resolved.ok) {
    return {
      ok: false,
  ...
  // No connection for this project: this is the shared-keys path, and it must
  // hand back EXACTLY what `getBlueskyCredentials` hands an unscoped caller.
  //
  // Reading `resolved.data.values` here instead looked equivalent and is not.
  // Those come from the shared-keys lookup in lib/apiSettings, where a value
  // saved on the Settings screen outranks the environment variable;
  // `getBlueskyCredentials` above gives the environment variable priority.
  // (The name of that lookup is spelled out in lib/apiSettings rather than
  // here: the guard test that keeps publishers from reaching around the
  // resolver greps this file for it.)
  //
  // With both set and different, the same account posted as one handle when a
  // project was named and another handle when it was not — a wrong-account
  // post, which looks exactly like success. Round 2 of 86bbpz1ed found it; one
  // precedence, decided in one place, is the fix.
  if (resolved.data.source !== 'connection') return { ...global, source: 'environment' };

  const values = resolved.data.values;
  return {
    identifier: normalizeIdentifier(values.identifier),
    appPassword: safeText(values.app_password),
    serviceUrl: (safeText(values.service_url) || global.serviceUrl).replace(/\/+$/, ''),
    source: 'connection',
  };
}
...
async function createSession(creds = getBlueskyCredentials()) {
  // Before the generic "credentials are missing" sentence, because a refusal
  // means the lookup FAILED rather than that nothing is saved, and those two
  // send whoever reads them to completely different places.
