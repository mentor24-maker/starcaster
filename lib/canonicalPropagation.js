...
async function runPropagation({ level, masterId, scope, applyToPage, applyToTemplate, options = {}, deps = {} }) {
  ...
  await inBatches(targets, async (page) => {
    ...
    // Reported off the DECISION, not off the write, and before the write is
    // even attempted: a drifted copy this push deliberately left alone was
    // left alone whatever then happened to the rest of the page. This is the
    // line that lets a page appear in `skipped` while it is also written for
    // some other copy's sake - the case that used to report nothing at all,
    // so the operator was never told his hand edit had been preserved.
