  ...
  await inBatches(targets, async (page) => {
    ...
    try {
      ...
      if (res && res.ok === false) tally.failed += 1;
        ...
        // `drifted` says a copy ON THIS PAGE has local changes; it does NOT
        // say one was written over. A page holding a hand-edited copy AND an
        // untouched one is written for the untouched copy's sake while the
        // edited one is left exactly as it was — and reporting that as "1 page
        // with local changes was overwritten" tells the operator his edit is
        // gone when it is still there. Only a forced push overwrites drift.
        if (outcome.drifted && options.overwriteDrifted === true) {
