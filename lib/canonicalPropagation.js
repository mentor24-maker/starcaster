 ...
 * section list and returns
 * `{ sections, changed, driftSkipped, driftOverwritten }` - `changed` false
 ...
 *
 * The two drift numbers are COUNTS OF COPIES, not page-level flags, and that
 * is the whole point of them. A page may hold several sections following one
 * master, so "was anything drifted here" and "was anything written here" are
 * different questions with different answers, and a single boolean cannot
 * carry both - which is landmine 17 / DOCTRINE §5.30, the slip that made a
 * dialog name a page as left alone and then publish it. Counting per copy is
 * what lets one page be written for a clean copy's sake AND report the
 * drifted copy it left behind.
 *
 * They land in three buckets, and the units are deliberately different:
 * `skipped` is PAGES THIS PUSH DID NOT WRITE (every following copy on them
 * drifted), `overwritten` is pages where a forced push flattened a drifted
 * copy, and `writtenWithPreservedEdits` is the third case that had no name
 * until 2026-09-08 - a page written for a clean copy's sake that still carries
 * a hand edit, reported with the COUNT of copies left alone. `skipped` and
 * `updatedPages` never overlap; the new bucket is a strict subset of
 * `updatedPages` and says so.
 *
 * @param {object}   input
...
async function runPropagation({ level, masterId, scope, applyToPage, applyToTemplate, options = {}, deps = {} }) {
  ...
  await inBatches(targets, async (page) => {
    ...
    if (!outcome.changed) {
      ...
      //
      // `skipped` means PAGES THIS PUSH DID NOT WRITE, and that is the only
      // thing it has ever meant. Three surfaces read it on that understanding:
      // the toast adds it to `updated` as a disjoint set, the editor's
      // "Overwrite anyway?" banner offers exactly these pages to a force run,
      // and docs/SAVED_SECTIONS.md states it in as many words. Widening it to
      // "pages with at least one copy left alone" moves the per-copy report
      // one layer out instead of removing it: the toast then counts one page
      // twice, and the banner offers to overwrite a page a Save & Publish has
      // already published. A page written for some other copy's sake goes in
      // `writtenWithPreservedEdits` below, never here.
      if (outcome.driftSkipped > 0) tally.skipped.push({ pageId: page.id, name: page.name });
      return;
    }
    ...
    try {
      ...
      if (res && res.ok === false) recordFailedPage(tally, page, outcome);
      else {
        tally.updated += 1;
        // WHICH pages, not just how many. A caller that wants to publish what
        // this push rewrote has to name them: POST /api/builder/publish
        // otherwise publishes every pending draft in the project, including
        // half-finished work that has nothing to do with this section.
        tally.updatedPages.push({ pageId: String(page.id), name: page.name || '' });
        // Only copies actually written OVER land here. A page holding a
        // hand-edited copy AND an untouched one is written for the untouched
        // copy's sake while the edited one is left exactly as it was — and
        // reporting that as "1 page with local changes was overwritten" tells
        // the operator his edit is gone when it is still there. The count is
        // zero unless a forced push really did flatten a drifted copy, so no
        // second reading of `options.overwriteDrifted` is needed here: the
        // level that made the decision is the one that reports it.
        if (outcome.driftOverwritten > 0) {
          tally.overwritten.push({ pageId: page.id, name: page.name });
        }
        // The fact that had nowhere to go until now: this page WAS written,
        // and it still carries a hand-edited copy the push left exactly as it
        // was. Reported here, inside the success branch, so the claim is
        // "written, and the edit survived" rather than a decision that a
        // failed PATCH would have thrown away. `copies` is a count because the
        // unit is the copy — a page can hold several — and the sentence the
        // operator reads must not add copies into the page count.
        if (outcome.driftSkipped > 0) {
          tally.writtenWithPreservedEdits.push({
            pageId: String(page.id),
            name: page.name || '',
            copies: outcome.driftSkipped,
          });
        }
      }
    } catch {
      recordFailedPage(tally, page, outcome);
    }
  });
...
/**
 * A page whose PATCH did not land — named, and carrying the one fact the
 * failure branch used to throw away.
 *
 * `failed` was a bare counter, so a page holding one clean copy and one
 * hand-edited copy whose write failed incremented it and landed in NEITHER
 * `skipped` nor `writtenWithPreservedEdits`. The toast then said "1 page could
 * not be updated. Reload and save again to finish." and said nothing at all
 * about the hand edit still sitting on it — the operator retried blind, and
 * the retry is the moment the "Overwrite anyway?" banner appears (task
 * 86bbwe530).
 *
 * `preservedCopies` counts COPIES, not pages, for the same reason
 * `writtenWithPreservedEdits` does: one page can hold several copies of one
 * master, and the sentence the operator reads must not add copies into a page
 * count (landmine 17 / DOCTRINE §5.30).
 */
function recordFailedPage(tally, page, outcome) {
  tally.failed += 1;
  tally.failedPages.push({
    pageId: String(page.id),
    name: page.name || '',
    preservedCopies: Number(outcome && outcome.driftSkipped) || 0,
  });
}
...
async function propagateToTemplates({
  ...
  await inBatches(templates, async (template) => {
    ...
    try {
      ...
      //
      // Spread the template, exactly as the page path above spreads the page,
      // and for the same reason (landmine 13) with a sharper edge here:
      // `inputToRow` in builderPageTemplatesStore builds all 34 columns
      // unconditionally from `input?.x`, and safeText(undefined) is ''. So a
      // partial `{ layoutSections }` does not patch one column - it PATCHes
      // every column and blanks the ones it never mentioned, taking the
      // template's name, subject, colours, image ids and feature copy with it,
      // and resetting pageBackground and theme. There is no revision row to
      // restore any of it from. `listPageTemplates` returns full
      // `rowToPageTemplate` objects, so the spread round-trips - and the test
      // "every column inputToRow writes is one rowToPageTemplate reads back"
      // fails if a future column is added to one half of that pair only.
      const res = await updatePageTemplate(
        template.id,
        { ...template, layoutSections: outcome.sections },
        scope
      );
...
function normalizeTemplateFrame(sections, masterId) {
  ...
  return { sections: next, changed: normalized > 0, normalized, instances: 0, locked: 0, driftSkipped: 0, driftOverwritten: 0 };
}
...
async function propagateCanonicalSection(savedSectionId, updatedSection, scope, options = {}) {
  ...
  const { hasSectionDrifted, sectionContentHash, writeLayoutSectionsToRow } = require('./builder');
  const previousSection = options.previousSection || null;
  const overwriteDrifted = options.overwriteDrifted === true;

  /**
   * Record, on every copy this push just rewrote, a hash of the content that
   * is actually going to be STORED for it.
   *
   * Hashed from `writeLayoutSectionsToRow`, which is the function
   * `updatePage` itself calls to build the row — not from a second
   * normalization that merely ought to agree with it. The stored form is not
   * the object in hand: a strict whitelist coerces defaults, drops unknown
   * fields, and reads a section's POSITION (a first row can never
   * `joinWithPrevious`). Hashing the copy in isolation, or through a parallel
   * code path, stamps a value the page never stores — and the stamp then
   * never matches again, which costs the fix silently rather than loudly.
   *
   * Anything that stops it hashing truthfully leaves the copy UNSTAMPED rather
   * than stamped wrong. An absent stamp costs nothing — `hasSectionDrifted`
   * falls back to the master comparison it has always made — while a wrong one
   * would have to be cleared by a later push to stop lying.
   */
  function stampWrittenCopies(sections, writtenIds) {
    if (!writtenIds.size) return sections;
    let storedById = null;
    try {
      const row = writeLayoutSectionsToRow({ sections });
      const stored = row && Array.isArray(row.sections) ? row.sections : [];
      storedById = new Map(stored.map((section) => [String(section && section.id), section]));
    } catch {
      return sections;
    }
    return sections.map((section) => {
      if (!writtenIds.has(section.id)) return section;
      const stored = storedById.get(String(section.id));
      if (!stored) return section;
      return { ...section, canonicalSourceHash: sectionContentHash(stored) };
    });
  }

  function applyToPage(sections) {
    let changed = false;
    // Per COPY, not per page: this page may hold several sections following
    // this master, and they need not have the same answer.
    let driftSkipped = 0;
    let driftOverwritten = 0;
    let instances = 0;
    let locked = 0;
    // The ids this push actually rewrites, so their lineage can be stamped
    ...
    const next = sections.map((section) => {
      ...
      if (hasDrift) driftOverwritten += 1;

      changed = true;
      instances += 1;
      writtenIds.add(section.id);
      // Replace content with the master's, keep only this instance's own id
      // and its provenance. The lineage stamp is applied below, once the whole
      // list exists — it depends on the section's position in it.
      return markFollowing({ ...updatedSection, id: section.id, savedSectionId }, true);
    });

    return {
      sections: changed ? stampWrittenCopies(next, writtenIds) : next,
      changed,
      driftSkipped,
      driftOverwritten,
      instances,
...
async function propagateCanonicalModule(savedModuleId, canonicalModules, scope, options = {}) {
  ...
  function applyToSections(sections, ctx, { skipFrameSections = false } = {}) {
      ...
      for (let i = 0; i < modules.length; i += 1) {
        ...
        for (const master of masters) {
          ...
          nextModules.push(markFollowing({
            ...master,
            // Unique within this page, and across runs: the old id folded in
            // Date.now(), so two pushes inside the same millisecond minted the
            // same ids twice. `minted` restarts per page, so two pages in ONE
            // run do mint matching ids - which is fine, because a module id
            // only has to be unique inside the page that holds it.
    ...
    return { sections: next, changed, driftSkipped: 0, driftOverwritten: 0, instances, locked };
  }
