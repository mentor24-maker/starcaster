...
function rowToRevision(row, { includeLayout = false } = {}) {
  ...
  const revision = {
    ...
    // Shared across every revision minted by ONE canonical-section push, so
    // the whole fan-out can be undone as a single action. Empty on an ordinary
    // single-page save, which is most rows.
    propagationRunId: safeText(row.propagation_run_id, 64),
 ...
 */
const OPTIONAL_COLUMNS = ['saved_by', 'saved_by_name', 'change_summary', 'propagation_run_id'];
  ...
  try {
    const row = await scopedInsertRow(
      table(),
      {
        ...
        propagation_run_id: safeText(options.propagationRunId, 64) || null,
...
/**
 * The run id an undo of `runId` stamps on the revisions IT banks.
 *
 * Deterministic on purpose, and load-bearing twice over:
 *  - "has this run already been undone?" is answered by listing the derived
 *    id — any row under it means yes — with no schema change;
 *  - the undo's own restore points are grouped, so an undo that dies partway
 *    (the 2026-07-22 serverless-timeout class) leaves a handle to finish or
 *    reverse the job instead of 20 reverted pages, 26 not, and no thread.
 * Sliced to the column's 64-char cap: at UUID lengths that permits four
 * nested undo-of-undo levels before truncation, which no real history walks.
 */
function undoRunIdFor(runId) {
  return `undo-${String(runId || '').trim()}`.slice(0, 64);
}

/**
 * Every revision minted by one propagation run, newest first.
 *
 * Returns the ids and page ids only -- the caller restores each page through
 * the ordinary single-revision path, which is what keeps "undo a run" and
 * "undo one page" the same operation rather than two that can drift apart.
 */
async function listRunRevisions(runId, scope = null) {
  const id = safeText(runId, 64);
  if (!id) return { ok: false, status: 400, error: 'propagation run id is required' };

  const query = await scopedListQuery(
    table(),
    `select=id,page_id,name,created_at&propagation_run_id=eq.${encodeURIComponent(id)}` +
  ...
  if (!res.ok) {
    return {
      ok: false,
      status: res.status || 500,
      error: 'Undo is unavailable. Run docs/SQL/builder_page_revisions_propagation_run_migration.sql in Supabase.',
      code: 'PROPAGATION_UNDO_UNAVAILABLE',
    };
  }
