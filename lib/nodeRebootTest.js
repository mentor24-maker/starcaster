...
function rebootTestReport({ boot, stored, node, owned } = {}) {
  ...
  // WHAT DOES THIS MACHINE OWN — the standard the record is graded against.
  // Asked here, after the record's own problems have been reported, because
  // "never verified" is a better sentence than "I do not know what you own".
  ...
  const observedRoles = new Set(record.roles.map((r) => r.role));
  ...
  const missing = record.roles.filter(didNotComeBack);
  if (missing.length > 0) {
  ...
  if (unverified.length > 0) {
    return unknown(
      ...
      `${node} owns ${inventory.probeable.length} probeable role${plural(inventory.probeable.length, '', 's')} (${inventory.probeable.join(', ')}), `
        + `and the record covers ${record.roles.length} (${record.roles.map((r) => r.role).join(', ')}). `
  ...
  const confirmed = inventory.probeable.length;
