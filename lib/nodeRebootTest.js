...
function rebootTestReport({ boot, stored, node, owned } = {}) {
  ...
  // cannot check. (`pulse-pipelines` was itself retired on 2026-09-12, split
  // into `channel-steward` and `librarian-sweep`, so that example is now
  // literally what a stale row looks like: a name no registry carries.)
  // The FAIL line had the same seam. This is the only joiner:
  ...
  // NOTHING HERE A REBOOT COULD TAKE AWAY. `macbook-pro` reaches this branch,
  // and what it owns has shrunk twice without changing the answer: two roles
  // when this was written (`db-refresh`, which deliberately has no schedule,
  // and `pulse-pipelines`, whose installer was another slice), then just
  // `db-refresh` once the Pulse pipelines moved to the Mini on 2026-09-07. So
  // it still has nothing probeable at all — and the branch is written against
  // the COUNT of probeable roles rather than against a machine name, which is
  // why neither move touched it. Until this branch existed, one of the two known
  // nodes
