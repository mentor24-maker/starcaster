...
const STORE_FILE = path.join(__dirname, '..', 'data', 'builder_modules.json');
const SUPPORT_CACHE = new Map();

function t() {
  return tableConfig().builderModules;
}
...
function sanitize(input) {
  ...
  const modules = Array.isArray(input.modules)
    ? input.modules
    : (Array.isArray(input.modules_json) ? input.modules_json : []);
  return {
    ...
    moduleClass: safeText(input.moduleClass || input.module_class),
    moduleType: safeText(input.moduleType || input.module_type) || 'header',
    modules,
    settings,
    isPrivate: input.isPrivate === true || input.is_private === true,
...
function inputToRow(input) {
  ...
  const row = {
    ...
    is_private: module.isPrivate === true,
  };
  if (module.classId) row.class_id = module.classId;
  if (module.moduleClass) row.module_class = module.moduleClass;
  if (Array.isArray(module.modules) && module.modules.length) row.modules = module.modules;
  return row;
}
...
async function createModule(input, scope = null) {
  ...
  if (writeResult && !writeResult.ok) return null;
  return module;
}
...
async function deleteModule(id, scope = null) {
  const moduleId = safeText(id);
  if (!moduleId) return { ok: false, error: 'id is required' };

  if (await supportsSupabaseModules()) {
    const query = await scopedIdQuery(t(), `id=eq.${encodeURIComponent(moduleId)}`, scope);
    const result = await sbQuery({ method: 'DELETE', table: t(), query, headers: { Prefer: 'return=representation' } });
    if (result.ok) return { ok: true };
    if (!isMissingTableError(result.error)) return { ok: false, error: result.error || 'Supabase delete failed' };
  }
  ...
  if (store.modules.length === originalLength) return { ok: false, error: 'Module not found' };
  writeStore(store);
  return { ok: true };
}
