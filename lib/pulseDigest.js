...
/**
 * How many findings of each kind the bus post lists before pointing at the
 * report. On 2026-09-13 a run with 382 findings rendered a post over ClickUp's
 * 40,000-character limit, the post AND its fallback were refused, and the pulse
 * exited 1 every hour without beating (task 86bbzwxrw). The ticket carries
 * every finding; the post only has to make someone open it.
 */
const POST_ITEM_LIMIT = 12;
...
function renderPulsePost({ items, node, now, everyMs, digestUrl = '' }) {
  ...
  const lines = [];
  const more = (count) => `- …and ${count} more — every one is on ${digestUrl || `the "${DIGEST_TASK_NAME}" ticket`}.`;

  lines.push('🩺 **The pipeline pulse found something.**', '');
  if (alarms.length) {
    lines.push(`**${alarms.length} alarm${alarms.length === 1 ? '' : 's'}**`);
    for (const a of alarms.slice(0, POST_ITEM_LIMIT)) lines.push(`- ${truncate(a.detail || a.title, 300)}`);
    if (alarms.length > POST_ITEM_LIMIT) lines.push(more(alarms.length - POST_ITEM_LIMIT));
    lines.push('');
  }
  if (cannot.length) {
    lines.push(`**${cannot.length} could-not-tell** — not an all-clear; these are readings that could not be taken.`);
    for (const c of cannot.slice(0, POST_ITEM_LIMIT)) lines.push(`- ${truncate(c.detail || c.title, 300)}`);
    if (cannot.length > POST_ITEM_LIMIT) lines.push(more(cannot.length - POST_ITEM_LIMIT));
    lines.push('');
  }
