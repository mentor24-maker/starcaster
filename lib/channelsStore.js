...
async function listChannels(scope = null) {
  const query = await scopedListQuery(
    t().channels,
    'select=id,channel,channel_type,contact_id,user_name,email,password,password_enc,password_iv,password_tag,key_version,created_at&order=channel.asc,user_name.asc&limit=5000',
    scope
  );
  return sbQuery({
    table: t().channels,
...
async function getChannelById(channelId, scope = null) {
  ...
  const query = await scopedIdQuery(
    t().channels,
    `select=id,channel,user_name,email,password,password_enc,password_iv,password_tag,key_version,created_at&id=eq.${id}&limit=1`,
    scope
  );
