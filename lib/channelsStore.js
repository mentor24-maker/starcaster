'use strict';

const { sbQuery, tableConfig } = require('./supabase');
const { scopedListQuery, scopedIdQuery, scopedInsertRow, scopedPatchRow } = require('./projectScope');
const { encryptSecret, decryptSecret } = require('./channelsCipher');

function t() { return tableConfig(); }

function rowToChannel(row) {
  if (!row) return null;
  const hasEncrypted =
    Boolean(row.password_enc) &&
    Boolean(row.password_iv) &&
    Boolean(row.password_tag);
  const hasPassword = hasEncrypted || Boolean(String(row.password || ''));
  return {
    id: Number(row.id || 0) || 0,
    channel: String(row.channel || ''),
    channelType: String(row.channel_type || 'organic'),
    contactId: row.contact_id ? String(row.contact_id) : null,
    userName: String(row.user_name || ''),
    email: String(row.email || ''),
    passwordMasked: hasPassword ? '********' : '',
    hasPassword,
    openclawProfile: String(row.openclaw_profile || ''),
    createdAt: String(row.created_at || ''),
  };
}

function isFacebookPersonalChannelName(value) {
  const key = String(value || '').trim().toLowerCase();
  return key === 'facebook personal' || key === 'facebook_personal';
}

const CHANNEL_SELECT_FIELDS = 'id,channel,channel_type,contact_id,user_name,email,password,password_enc,password_iv,password_tag,key_version,openclaw_profile,created_at';

async function listChannels(scope = null) {
  const query = await scopedListQuery(
    t().channels,
    `select=${CHANNEL_SELECT_FIELDS}&order=channel.asc,user_name.asc&limit=5000`,
    scope
  );
  return sbQuery({
    table: t().channels,
    query,
  });
}

async function createChannel(input, scope = null) {
  const enc = encryptSecret(input.password);
  if (!enc.ok) return { ok: false, status: 500, error: enc.error };

  const row = await scopedInsertRow(t().channels, {
    channel: String(input.channel || '').trim(),
    channel_type: String(input.channelType || 'organic').trim(),
    contact_id: input.contactId || null,
    user_name: String(input.userName || input.user_name || '').trim(),
    email: String(input.email || '').trim(),
    openclaw_profile: String(input.openclawProfile || input.openclaw_profile || '').trim(),
    password: '',
    ...enc.data,
  }, scope);
  return sbQuery({
    method: 'POST',
    table: t().channels,
    query: 'select=*',
    headers: { Prefer: 'return=representation' },
    body: [row],
  });
}

async function getChannelById(channelId, scope = null) {
  const id = Number(channelId || 0) || 0;
  if (id <= 0) return { ok: false, status: 400, error: 'channel id is required' };
  const query = await scopedIdQuery(
    t().channels,
    `select=${CHANNEL_SELECT_FIELDS}&id=eq.${id}&limit=1`,
    scope
  );
  const res = await sbQuery({
    table: t().channels,
    query,
  });
  if (!res.ok) return res;
  const row = Array.isArray(res.data) ? res.data[0] : null;
  if (!row) return { ok: false, status: 404, error: 'Channel not found' };
  return { ok: true, status: 200, data: row };
}

async function updateChannel(channelId, patch, scope = null) {
  const id = Number(channelId || 0) || 0;
  if (id <= 0) return { ok: false, status: 400, error: 'channel id is required' };

  const row = {};
  if (patch.channel != null) {
    row.channel = String(patch.channel || '').trim();
  }
  if (patch.channelType != null) {
    row.channel_type = String(patch.channelType || 'organic').trim();
  }
  if (patch.contactId !== undefined) {
    row.contact_id = patch.contactId || null;
  }
  if (patch.userName != null || patch.user_name != null) {
    row.user_name = String(patch.userName || patch.user_name || '').trim();
  }
  if (patch.email != null) {
    row.email = String(patch.email || '').trim();
  }
  if (patch.openclawProfile != null || patch.openclaw_profile != null) {
    row.openclaw_profile = String(patch.openclawProfile || patch.openclaw_profile || '').trim();
  }
  if (patch.password != null) {
    const enc = encryptSecret(patch.password);
    if (!enc.ok) return { ok: false, status: 500, error: enc.error };
    row.password = '';
    row.password_enc = enc.data.password_enc;
    row.password_iv = enc.data.password_iv;
    row.password_tag = enc.data.password_tag;
    row.key_version = enc.data.key_version;
  }

  if (!Object.keys(row).length) {
    return { ok: false, status: 400, error: 'No fields to update' };
  }

  const scopedRow = await scopedPatchRow(t().channels, row, scope);
  const query = await scopedIdQuery(t().channels, `id=eq.${id}&select=*`, scope);
  return sbQuery({
    method: 'PATCH',
    table: t().channels,
    query,
    headers: { Prefer: 'return=representation' },
    body: scopedRow,
  });
}

async function deleteChannel(channelId, scope = null) {
  const id = Number(channelId || 0) || 0;
  if (id <= 0) return { ok: false, status: 400, error: 'channel id is required' };
  const query = await scopedIdQuery(t().channels, `id=eq.${id}&select=*`, scope);

  return sbQuery({
    method: 'DELETE',
    table: t().channels,
    query,
    headers: { Prefer: 'return=representation' },
  });
}

async function getChannelCredentialsById(channelId, scope = null) {
  const rowRes = await getChannelById(channelId, scope);
  if (!rowRes.ok) return rowRes;

  const dec = decryptSecret(rowRes.data);
  if (!dec.ok) return { ok: false, status: 500, error: dec.error };

  return {
    ok: true,
    status: 200,
    data: {
      id: Number(rowRes.data.id || 0) || 0,
      channel: String(rowRes.data.channel || ''),
      userName: String(rowRes.data.user_name || ''),
      email: String(rowRes.data.email || ''),
      password: dec.data.plaintext,
      openclawProfile: String(rowRes.data.openclaw_profile || ''),
    },
  };
}

async function getChannelOpenClawProfile(channelId, scope = null) {
  const rowRes = await getChannelById(channelId, scope);
  if (!rowRes.ok) return rowRes;
  const row = rowRes.data;
  return {
    ok: true,
    status: 200,
    data: {
      id: Number(row.id || 0) || 0,
      channel: String(row.channel || ''),
      userName: String(row.user_name || ''),
      openclawProfile: String(row.openclaw_profile || ''),
      isFacebookPersonal: isFacebookPersonalChannelName(row.channel),
    },
  };
}

async function getChannelCredentialsByChannel(channelName, scope = null) {
  const name = String(channelName || '').trim();
  if (!name) return { ok: false, status: 400, error: 'channel name is required' };

  const query = await scopedListQuery(
    t().channels,
    `select=${CHANNEL_SELECT_FIELDS}` +
      `&channel=eq.${encodeURIComponent(name)}&order=id.desc&limit=1`,
    scope
  );
  const res = await sbQuery({
    table: t().channels,
    query,
  });
  if (!res.ok) return res;
  const row = Array.isArray(res.data) ? res.data[0] : null;
  if (!row) return { ok: false, status: 404, error: 'Channel not found' };

  const dec = decryptSecret(row);
  if (!dec.ok) return { ok: false, status: 500, error: dec.error };

  return {
    ok: true,
    status: 200,
    data: {
      id: Number(row.id || 0) || 0,
      channel: String(row.channel || ''),
      userName: String(row.user_name || ''),
      email: String(row.email || ''),
      password: dec.data.plaintext,
      openclawProfile: String(row.openclaw_profile || ''),
    },
  };
}

module.exports = {
  rowToChannel,
  listChannels,
  createChannel,
  getChannelById,
  updateChannel,
  deleteChannel,
  getChannelCredentialsById,
  getChannelCredentialsByChannel,
  getChannelOpenClawProfile,
  isFacebookPersonalChannelName,
};
