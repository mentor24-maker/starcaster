...
function rowToLevelEvent(row) {
  if (!row) return null;
  const module = row.develop_modules || row.developModules || null;
  return {
...
async function listLevelEvents(limit = 1000, scope = null) {
  ...
  const query = await scopedListQuery(
    table(),
    `select=*,develop_modules(name)&order=updated_at.desc,created_at.desc&limit=${safeLimit}`,
    scope
  );
...
async function createLevelEvent(input, scope = null) {
  ...
  const res = await sbQuery({
    method: 'POST',
    table: table(),
    query: 'select=*,develop_modules(name)',
...
async function updateLevelEvent(id, input, scope = null) {
  ...
  const query = await scopedIdQuery(table(), `id=eq.${encodeURIComponent(eventId)}&select=*,develop_modules(name)`, scope);
  const res = await sbQuery({
    method: 'PATCH',
    table: table(),
    query,
