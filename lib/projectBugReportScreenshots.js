 ...
 *
 *  - ORPHAN CLEANUP IS NOT BUILT YET. An abandoned form leaves a
 *    `bug-report-pending` asset (row + blob). There is no scheduled sweep in
 *    this repo today (the one cron is publish-due, and deleteAsset removes
 *    only the row), so this does NOT promise one — it is filed as task
 *    86bbjk5wj. What IS handled here: a blob whose row failed to insert is
 *    deleted immediately (below), so the only residue is fully-recorded
 *    pending rows the future sweep will collect with:
 *
 *      pending rows older than N hours, not referenced by any report's
 *      screenshot_asset_ids → delete the row AND its blob.
 */
