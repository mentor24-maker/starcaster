 ...
 *
 *  - ORPHAN CLEANUP IS A SEPARATE, SCHEDULED SWEEP. An abandoned form leaves
 *    a `bug-report-pending` asset (row + blob) that nothing here will ever
 *    collect. What IS handled below: a blob whose row failed to insert is
 *    deleted on the spot. The rest — fully-recorded pending rows — is
 *    lib/bugReportOrphanSweep.js, which deletes pending rows older than N
 *    hours that no report's `screenshot_asset_ids` references, blob first and
 *    row second. It runs daily as a Vercel cron
 *    (/api/support/bug-reports/sweep-orphans) and by hand as
