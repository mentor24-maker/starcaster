...
function sanitizeAdminUser(row) {
  return {
    ...
    // Derived, not stored: the Bug Report module's "staff" visibility tier
    // (task 4/5) means "a signed-in tenant admin with the admin role"; editors
    // count as registered clients. Render-side gating is UX — the submit
    // endpoint (1/5) re-checks the session for any client/staff claim.
    isStaff: (safeText(row?.role) || 'editor') === 'admin',
...
/** Overwrite an admin's password hash. Used by the emailed-code reset flow. */
async function updateAdminUserPassword(id, projectId, newPassword) {
  const err = requireSupabase();
  if (err) return err;
  ...
  if (!uid || !pid) return { ok: false, status: 400, error: 'id and projectId are required' };
  if (pwd.length < 8) return { ok: false, status: 400, error: 'Password must be at least 8 characters' };
  ...
  const result = await sbQuery({
    ...
    table: ADMIN_USERS_TABLE,
    query,
    body: { password_hash: hashPassword(pwd), updated_at: new Date().toISOString() },
    headers: { Prefer: 'return=representation' },
  });
...
/** Revoke every active session for an admin — called after a password change. */
async function deleteAdminSessionsForUser(adminUserId) {
  const err = requireSupabase();
  if (err) return err;
  ...
  const result = await sbQuery({
    method: 'DELETE',
    table: ADMIN_SESSIONS_TABLE,
    query: `admin_user_id=eq.${encodeURIComponent(uid)}`,
  });
  if (!result.ok) return { ok: false, status: result.status || 500, error: result.error || 'Unable to revoke sessions' };
  return { ok: true };
}
