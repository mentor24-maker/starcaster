...
async function uploadMediaSimple(mediaBuffer, mimeTypeInput, options = {}) {
  ...
  // A client's bearer token uploads through the same endpoint, authorised by
  // the `media.write` scope the adapter asks for (lib/connections/adapters/x.js).
  // The multipart body is identical in both schemes — only the header differs,
  // which is exactly the split `authHeaderFor` exists to hold.
