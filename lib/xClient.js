...
async function checkAuth() {
  const creds = getXCredentials();
  if (!creds.apiKey || !creds.apiSecret || !creds.accessToken || !creds.accessTokenSecret) {
    return { ok: false, status: 400, error: 'X credentials are missing' };
  }
  ...
  try {
    ...
    attempts.push({ endpoint: verifyUrl, ok: false, status: 502, message: err.message || String(err) });
  ...
  attempts.push({ endpoint: verifyUrl, ok: response.ok, status: response.status, message: response.ok ? 'OK' : (response.statusText || 'Failed') });

  if (!response.ok) {
    const detail = Array.isArray(payload?.errors) && payload.errors.length
      ? payload.errors.map((e) => e?.message || e?.detail || JSON.stringify(e)).filter(Boolean).join(' | ')
      : (payload?.detail || payload?.title || response.statusText || 'Unknown X API error');
    return { ok: false, status: response.status, error: `X auth failed: ${detail}`, attempts };
  }
