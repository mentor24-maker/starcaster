...
const DEFAULT_TEMPLATE = {
  ...
  // The card's own border (2026-09-02, task 86bbtvnr5). It used to come only
  // from the cardStyle preset, with the "default" branch hard-coding this grey
  // and no way for an operator to change it. These defaults ARE that preset; a
  // template saved before the split derives its own below.
  cardBorderWidth: 1,
...
// Translate a pre-split template's cardStyle preset into the explicit border it
// used to imply, so nothing an existing tenant saved changes appearance.
// Mirrored in components/builder-template-preview.tsx.
function presetBorderWidth(cardStyle) {
  return cardStyle === 'shadow' ? 0 : 1;
}

function presetBorderColor(cardStyle, accentColor) {
...
function mergeTemplate(saved) {
  ...
  return {
    ...
    // ABSENT, not falsy: a saved width of 0 means "no border" and must survive.
    cardBorderWidth:   saved.cardBorderWidth === undefined
      ? presetBorderWidth(String(saved.cardStyle || DEFAULT_TEMPLATE.cardStyle))
      : clampNum(saved.cardBorderWidth, 0, 16, DEFAULT_TEMPLATE.cardBorderWidth),
    cardBorderColor:   saved.cardBorderColor === undefined
      ? presetBorderColor(
          String(saved.cardStyle || DEFAULT_TEMPLATE.cardStyle),
          String(saved.accentColor || DEFAULT_TEMPLATE.accentColor)
        )
      : String(saved.cardBorderColor),
    rows,
  };
}
...
async function saveCardTemplate(template, scope = null) {
  ...
  if (await supportsSupabase()) {
    ...
    const result = await sbQuery({
      ...
      // WITHOUT THIS HEADER THE UPSERT IS NOT AN UPSERT (fixed 2026-09-02, task
      // 86bbt72jw). PostgREST only merges a conflicting row when it is asked to;
      // `on_conflict=` alone just names the column, so the second save for a
      // project came back 409 "duplicate key value violates unique constraint"
      // — every save after the first, for the life of the feature. Every other
      // store in this repo already sends it (builderPublishStore, trainingStore,
      // promoLeads, and eight more); this one never did.
      headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
      body: row,
    });
    if (result.ok || result.status === 200 || result.status === 201 || result.status === 204) return safe;

    // AND IT MUST NOT FALL THROUGH. Supabase is the store whenever it is
    // configured; the local file below is the no-database path. Falling through
    // wrote to a filesystem that is READ-ONLY on Vercel (CLAUDE.md landmine 6)
    // and then returned `safe`, so the API answered 200 with the operator's new
    // values while the database still held the old ones. The save looked
    // perfect and changed nothing — which is why the 409 went unnoticed.
    throw new Error(
      `Could not save the card template: ${result.error || `Supabase returned ${result.status}`}`
    );
  }
