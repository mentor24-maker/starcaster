...
function table() {
  return tableConfig().messagingReports;
}
...
function reportInputToRow(input) {
  return {
...
async function listMessagingReports(limit = 200) {
  ...
  return {
    ok: true,
    status: 200,
    data: Array.isArray(res.data) ? res.data.map(rowToReport) : [],
  };
}

async function createMessagingReport(input) {
  const row = reportInputToRow(input);
  const res = await sbQuery({
    method: 'POST',
    table: table(),
    query: 'select=*',
  ...
  return {
    ok: true,
    status: 201,
    data: rowToReport(created),
  };
}

async function updateMessagingReport(id, input) {
  const reportId = Number(id || 0) || 0;
  if (!reportId) return { ok: false, status: 400, error: 'id is required' };
  const row = reportInputToRow(input);
  const res = await sbQuery({
    method: 'PATCH',
    table: table(),
    query: `id=eq.${reportId}&select=*`,
    headers: { Prefer: 'return=representation' },
    body: row,
  });
  if (!res.ok) return res;
  const updated = Array.isArray(res.data) ? res.data[0] : res.data;
  if (!updated) return { ok: false, status: 404, error: 'Report not found' };
  return {
    ok: true,
    status: 200,
    data: rowToReport(updated),
  };
}

async function deleteMessagingReport(id) {
  const reportId = Number(id || 0) || 0;
  if (!reportId) return { ok: false, status: 400, error: 'id is required' };
  const res = await sbQuery({
    method: 'DELETE',
  ...
  if (!deleted) return { ok: false, status: 404, error: 'Report not found' };
  return {
    ok: true,
    status: 200,
    data: rowToReport(deleted),
  };
}
