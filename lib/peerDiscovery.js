...
const PEER_DISCOVERY_VERSION = 'peer-discovery-v1';
...
function summarizeSearchErrors(errors) {
  ...
  if (/does not have the access to Custom Search JSON API/i.test(primary)) {
    return 'Google Custom Search API is not enabled for this API key. In Google Cloud Console, open APIs & Services → Library, enable "Custom Search API" for the project linked to GOOGLE_CUSTOM_SEARCH_API_KEY, then retry.';
  }
  if (/invalid.*api key|API key not valid/i.test(primary)) {
    return 'Google Custom Search API key is invalid. Check GOOGLE_CUSTOM_SEARCH_API_KEY in Settings → APIs.';
  }
  if (/search engine id|cx.*invalid/i.test(primary)) {
    return 'Google Programmable Search Engine ID is invalid. Check GOOGLE_CUSTOM_SEARCH_ENGINE_ID (cx) in Settings → APIs.';
  }
...
function buildFilterReason(record, options) {
  ...
  if (HARD_EXCLUDE_HOSTS.has(domain)) return 'mega_hub';
...
async function searchGoogleForKeywords(keywords, sourceDomain, resultsPerKeyword = RESULTS_PER_KEYWORD) {
  const config = getGoogleSearchConfig();
  if (!config.configured) {
    return {
      configured: false,
      error: 'Google Programmable Search credentials are not configured.',
      queryResults: [],
      errors: [],
    };
  }
  const queryResults = [];
  const errors = [];
  const safePerKeyword = Math.max(10, Math.min(Number(resultsPerKeyword) || RESULTS_PER_KEYWORD, 30));
  const pageCount = Math.ceil(safePerKeyword / 10);
  const starts = Array.from({ length: pageCount }, (_, index) => index * 10 + 1);
  await mapWithConcurrency(keywords, 2, async (keyword) => {
    const results = [];
    for (const start of starts) {
      const batch = await fetchGoogleSearchBatch(keyword, start, config);
      if (!batch.ok) {
...
function rankCandidates(candidates, fingerprint, keywordCount) {
  return candidates
    .map((record) => {
      const classification = detectPeerSiteSignals(record, registrableDomainOf(fingerprint.source_url || ''));
      const primaryModel = classification.primary || 'Direct Competitors';
      const referenceRole = suggestedReferenceRole(primaryModel);
      const { similarity, reasons } = finalScore(record, fingerprint, keywordCount);
      const peerBoost = referenceRole === 'peer' ? 2 : 0;
      return {
        domain: record.domain,
        url: record.url,
        title: record.title || record.homepage_title || '',
        snippet: record.snippet || record.homepage_meta || '',
        rank: record.rank,
        hits: record.hits,
        matched_keywords: record.matched_keywords,
        similarity_score: similarity,
        sort_score: similarity + peerBoost,
        website_model: primaryModel,
        other_models: classification.other || [],
        suggested_reference_role: referenceRole,
        reasons,
        homepage_fetched: Boolean(record.homepage_title || record.homepage_meta),
        fetch_error: String(record.fetch_error || '').trim(),
      };
    })
    .sort((a, b) => b.sort_score - a.sort_score || b.similarity_score - a.similarity_score || a.rank - b.rank || a.domain.localeCompare(b.domain));
}
...
async function runPeerDiscovery(input, scope = null) {
  ...
  const resultsPerKeyword = Math.max(10, Math.min(Number(input?.results_per_keyword) || RESULTS_PER_KEYWORD, 30));
  const outputCount = Math.max(1, Math.min(Number(input?.output_count) || OUTPUT_COUNT, 20));
  const lightFetchCount = Math.max(outputCount, Math.min(Number(input?.light_fetch_count) || LIGHT_FETCH_COUNT, 30));
  ...
  const search = await searchGoogleForKeywords(searchedKeywords, sourceDomain, resultsPerKeyword);
  ...
  if (!search.queryResults.length) {
    const searchError = summarizeSearchErrors(search.errors)
      || 'Google search returned no peer candidates for the selected key phrases.';
    return {
      version: PEER_DISCOVERY_VERSION,
      source_url: sourceUrl,
      started_at: startedAt,
  ...
  const fetchTargets = scored.slice(0, lightFetchCount);
  const enriched = await lightFetchHomepages(fetchTargets);
  const ranked = rankCandidates(enriched, fingerprint, searchedKeywords.length);
  const results = ranked.slice(0, outputCount);
