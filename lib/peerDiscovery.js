...
const { applyHybridClassification } = require('./websitePeerClassifier');
...
async function runPeerDiscovery(input, scope = null) {
  ...
  const heuristicClassified = scored.map((record) => buildClassifiedCandidate(
    record,
    sourceDomain,
    fingerprint,
  ...
  const { results: classified, meta: classificationMeta } = await applyHybridClassification(heuristicClassified, {
    scope,
    sourceUrl,
    sourceDomain,
    fingerprint,
  ...
  const classificationErrors = Array.isArray(classificationMeta?.errors)
    ? classificationMeta.errors.filter(Boolean)
    : [];
  ...
  return {
    ...
    classification_provider: String(classificationMeta?.provider || 'heuristic'),
    classification_gemini_used: !!classificationMeta?.gemini_used,
    classification_gemini_tokens: Number(classificationMeta?.tokens || 0) || 0,
    classification_gemini_count: Number(classificationMeta?.classified_by_gemini || 0) || 0,
    classification_heuristic_count: Number(classificationMeta?.classified_by_heuristic || 0) || 0,
    classification_errors: classificationErrors.slice(0, 5),
    results,
    errors: [...search.errors, ...classificationErrors].filter(Boolean).slice(0, 10),
    error: '',
  };
}
