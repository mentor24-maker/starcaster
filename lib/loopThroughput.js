 ...
 */
function recentUndatedClosures({ tasks, now, windowMs = STALL_WINDOW_MS } = {}) {
  const floor = now - windowMs;
  return (Array.isArray(tasks) ? tasks : []).filter((t) => {
...
function renderStallPost({
  ...
  if (Number(undated) > 0) {
    lines.push(`- ${Number(undated)} finished ticket(s) carry no closure date, so the `
      + `${closed.length}-day count above reads a little low. The last 24h is unaffected — an `
      + 'undated closure inside the window makes this UNKNOWN rather than a stall.');
  }
  lines.push(
    '',
 ...
 */
function renderUnknownPost({ verdict: v, now, node = '' }) {
  return [
