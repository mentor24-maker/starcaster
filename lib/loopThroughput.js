 ...
 *
 * `queued` is the headline depth the ticket asks to print — the column a build
 * loop can actually claim from.
 *
...
function queueShape(tasks) {
  ...
  for (const t of Array.isArray(tasks) ? tasks : []) {
    ...
    shape.openWork += 1;
    if (status === 'queued') shape.queued += 1;
...
/**
 * The oldest open pull request whose ticket has been sent back to `Queued`.
 *
 * THE BUCKET IS READ FROM `wipCap.classifyPrs`, NOT DERIVED A SECOND TIME.
...
function reworkPrs({ prs, ticketStatusById, now } = {}) {
  ...
  const rows = groups.queued.map((number) => {
...
function renderReport({ verdict: v, closed, curve, plateau, rework, queue, now, undated = 0 }) {
  ...
  lines.push('');
  lines.push('Rework pull requests (open, ticket sent back to Queued)');
