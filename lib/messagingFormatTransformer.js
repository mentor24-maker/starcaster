...
function buildPrompt(sourceText, fromSpec, toSpec) {
  ...
  const toHashtags = toSpec && toSpec.slug === 'hashtag';
  const toMessagingTags = toSpec && toSpec.slug === 'tag';
  ...
  } else if (toMessagingTags) {
    instruction = `Extract plain content tags from the following ${fromLabel}. Return 5 to 10 tags, one per line. Prefer single words; use at most 3 words per tag with spaces between words. Use Title Case. Never use # symbols. Return nothing else — no labels, no explanations.`;
...
async function transformContent(sourceText, fromSpec, toSpec) {
      ...
      body: JSON.stringify({
        ...
        system: toLong
          ...
          : toHashtags
            ...
            : toMessagingTags
              ? 'You are a content repurposing assistant. Output plain tags only, one per line, no # symbols, Title Case, max 3 words per tag.'
  ...
  // transformContent() carries no project scope, so this row lands unattributed.
  await recordAiUsage({ provider: 'anthropic', model, feature: 'messaging_format_transform', body });
  ...
  if (toMessagingTags) {
    const { parseMessagingTagLines } = require('./messagingTagNormalize');
