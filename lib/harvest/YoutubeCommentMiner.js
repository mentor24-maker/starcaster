...
function normalizeTrainingFeedback(raw) {
  ...
  return toArray(parsed).map((row) => ({
    ...
    category: safeText(row?.category).toLowerCase(),
...
function buildTrainingSignals(feedbackRows = []) {
  ...
  toArray(feedbackRows).forEach((row) => {
    ...
    const category = safeText(row?.category).toLowerCase();
    const tags = toArray(row?.tags).map((tag) => safeText(tag).toLowerCase()).filter(Boolean);
    ...
    if (quality >= 4) {
      ...
      tags.forEach((tag) => excludeTerms.add(tag));
      if (category) {
...
function normalizeCategoryConfig(raw) {
  ...
  const normalized = toArray(parsed)
    .map((row) => {
      const name = safeText(row?.name) || 'general';
      const rationale = safeText(row?.rationale);
      const factor = safeText(row?.factor);
      const rankNum = Number(row?.value_rank);
      const valueRank = Math.max(1, Math.min(Number.isFinite(rankNum) ? rankNum : 3, 5));
      const tagsRaw = row?.match_tags;
      const matchTags = (Array.isArray(tagsRaw) ? tagsRaw : String(tagsRaw || '').split(/\r?\n|,/g))
        .map((item) => safeText(item).toLowerCase())
        .filter(Boolean)
        .slice(0, 20);
      return { name, rationale, factor, value_rank: valueRank, match_tags: Array.from(new Set(matchTags)) };
    })
    .filter((row) => Boolean(row.name));
  return normalized.length ? normalized : defaultCategoryConfig();
}
...
function resolveCategoryForTags(tags = [], categoryConfig = [], categoryBoost = {}) {
  ...
  let best = null;
  cfg.forEach((row, idx) => {
    const rowTags = toArray(row?.match_tags).map((t) => safeText(t).toLowerCase()).filter(Boolean);
    ...
    const boost = Number(categoryBoost[safeText(row?.name).toLowerCase()] || 0) || 0;
    const score = matchCount > 0 ? (matchCount * 100 + valueRank * 10 + (cfg.length - idx) + (boost * 15)) : (boost * 10);
    if (!best || score > best.score) {
      best = {
        name: safeText(row?.name) || 'general',
        rationale: safeText(row?.rationale),
        factor: safeText(row?.factor),
        value_rank: valueRank,
        match_tags: matchedTags,
        score,
      };
    }
  });
  if (best && best.score > 0) return best;
  const general = cfg.find((row) => safeText(row?.name).toLowerCase() === 'general');
  if (general) {
    return {
      name: safeText(general.name) || 'general',
      rationale: safeText(general.rationale),
      factor: safeText(general.factor),
  ...
  const first = cfg[0] || {};
  return {
    name: safeText(first.name) || 'general',
    rationale: safeText(first.rationale),
    factor: safeText(first.factor),
    value_rank: Math.max(1, Math.min(Number(first.value_rank || 3) || 3, 5)),
    match_tags: [],
    score: 0,
  };
}
...
function classifyComment(comment, tuning = {}) {
  ...
  const preferredTags = Array.isArray(tuning.preferred_tags) ? tuning.preferred_tags : [];
  ...
  preferredTags.forEach((tag) => {
    ...
    if (tags.includes(tag)) {
      score += 2;
      why.push(`trainer preferred tag: "${tag}"`);
    }
  });
  ...
  if (safeText(categoryResolved?.factor)) {
    why.push(`category factor: ${safeText(categoryResolved.factor)}`);
  }
  ...
  return {
    ...
    category_factor: safeText(categoryResolved?.factor),
    category_value_rank: Number(categoryResolved?.value_rank || 0) || 0,
    category_match_tags: toArray(categoryResolved?.match_tags),
    is_noise: isNoise,
  };
}
...
function buildReplyDraft(classified, comment) {
  ...
  const tags = toArray(classified?.tags).map((tag) => safeText(tag));
    ...
    try {
      ...
      const classifiedComments = comments.map((comment) => {
        const classified = classifyComment(comment, {
          ...
          preferred_tags: toArray(trainingSignals.preferred_tags),
