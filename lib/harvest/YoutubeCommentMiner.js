...
function resolveCategoryForTags(tags = [], categoryConfig = []) {
  ...
  cfg.forEach((row, idx) => {
    ...
    const score = matchCount > 0 ? (matchCount * 100 + valueRank * 10 + (cfg.length - idx)) : 0;
...
function classifyComment(comment, tuning = {}) {
  ...
  const categoryResolved = resolveCategoryForTags(tags, tuning.category_config);
