...
function defaultCategoryConfig() {
  return [
    {
      name: 'intent',
      rationale: 'Explicit “how do I start / what should I buy / what does it cost” language. High conversion potential; prioritize for direct follow-up.',
      value_rank: 5,
      match_hashtags: ['purchase_intent', 'solution_seeking'],
    },
    {
      name: 'pain_point',
      rationale: 'Clear friction, dissatisfaction, or stuck-state statements. Valuable for empathy-first engagement and diagnostic questions.',
      value_rank: 5,
      match_hashtags: ['pain_point', 'problem_signal'],
    },
    {
      name: 'growth',
      rationale: 'Identity-shift and self-improvement language ("want more," "outgrowing old self"). Strong fit for aspirational messaging.',
      value_rank: 5,
      match_hashtags: ['growth_openness', 'identity_shift'],
    },
    {
      name: 'question',
      rationale: 'Genuine information-seeking questions. Good candidates for trust-building, useful replies, and soft invitation to continue.',
      value_rank: 4,
      match_hashtags: ['question'],
    },
    {
      name: 'positive',
      rationale: 'Supportive/affirming comments with low friction. Useful for community warmth but usually lower urgency than intent or pain.',
      value_rank: 3,
      match_hashtags: ['positive_signal'],
    },
    {
      name: 'risk',
      rationale: 'Scam/fake/bot/skeptic cues. Requires credibility-first messaging and can be deprioritized unless strategically important.',
      value_rank: 2,
      match_hashtags: ['trust_risk'],
    },
    {
      name: 'general',
      rationale: 'No strong actionable signal detected. Keep as background unless combined with high-value attributes.',
      value_rank: 1,
      match_hashtags: [],
    },
  ];
...
function defaultAttributeConfig() {
  return [
    { name: 'motivated', rationale: 'Displays readiness to change behavior now (not "someday"). Prioritize because response likelihood and follow-through are high.', value_rank: 5, match_hashtags: ['growth_openness', 'purchase_intent', 'identity_shift'] },
    { name: 'self_involved', rationale: 'Language centers status/appearance over substance. Engage lightly unless paired with intent or growth signals.', value_rank: 2, match_hashtags: ['social_handle'] },
    { name: 'negative_outlook', rationale: 'Defeatist/cynical framing. Requires careful tone; avoid hard CTA and lead with acknowledgement + reframing.', value_rank: 2, match_hashtags: ['pain_point', 'trust_risk'] },
    { name: 'fan', rationale: 'High affinity and goodwill. Useful for amplification and relationship-building, but often lower conversion urgency.', value_rank: 3, match_hashtags: ['positive_signal'] },
    { name: 'introvert', rationale: 'Reflective and internal processing style. Better with thoughtful, non-pushy prompts and one clear next question.', value_rank: 3, match_hashtags: ['question', 'growth_openness'] },
  ];
}
...
function buildReplyDraft(classified, comment, trainingSignals = {}) {
  ...
  if (approach === 'intrigue') {
    return withContext(`${base} You’re pointing at something important. If you want, I can share one angle most people miss here.`);
  }
  if (approach === 'inquire') {
    return withContext(`${base} Quick question: what outcome are you aiming for first?`);
  }
  if (approach === 'direct_cta') {
    return withContext(`${base} If you’re ready, I can share a direct next step you can start today.`);
  }
  if (tags.includes('purchase_intent')) {
    return withContext(`${base} ${preferredResponseType === 'cta_direct'
  ...
  if (tags.includes('growth_openness')) {
    return withContext(`${base} That shift you described is real. If you want, I can share a simple path to move from feeling stuck to clear next steps.`);
  }
  if (tags.includes('pain_point') || tags.includes('solution_seeking')) {
    return withContext(`${base} That sounds frustrating. If you describe your exact setup, I can suggest a concrete fix path.`);
  }
  if (tags.includes('question')) {
    return withContext(`${base} Great question. The short answer is: start with a simple baseline, then iterate based on what your audience responds to.`);
  }
  if (tags.includes('positive_signal')) {
    return withContext(`${base} Glad this helped. Curious what part was most useful for you?`);
  }
  if (tags.includes('trust_risk')) {
    return withContext(`${base} Good call to be cautious. Verifying sources and outcomes before acting is the safest move.`);
  }
  return withContext(`${base} Appreciate the perspective.`);
}
