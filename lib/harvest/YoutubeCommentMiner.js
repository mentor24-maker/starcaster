...
function youtubeApiConfig() {
  const saved = getProviderValues('youtube') || {};
  const apiKey = safeText(process.env.YOUTUBE_API_KEY) || safeText(saved.api_key);
  return { apiKey };
}
...
function classifyComment(comment) {
  ...
  if (likeCount >= 5) score += 2;
  if (replyCount >= 2) score += 1;
  if (text.length < 15) score -= 2;
  if (has(/\b(first|lol|lmao|nice|cool|bro)\b/) && text.length < 35) score -= 2;
