...
function normalizeCategoryConfig(raw) {
  ...
  const normalized = toArray(parsed)
    .map((row) => {
      ...
      const treatment = safeText(row?.treatment || row?.factor);
      ...
      const matchTags = (Array.isArray(tagsRaw) ? tagsRaw : String(tagsRaw || '').split(/\r?\n|,/g))
        ...
        .filter(Boolean)
        .slice(0, 20);
      return { name, rationale, treatment, factor: treatment, value_rank: valueRank, match_hashtags: Array.from(new Set(matchTags)) };
    })
...
function resolveCategoryForTags(tags = [], categoryConfig = [], categoryBoost = {}) {
  ...
  cfg.forEach((row, idx) => {
    ...
    if (!best || score > best.score) {
      best = {
        ...
        treatment: safeText(row?.treatment || row?.factor),
        factor: safeText(row?.treatment || row?.factor),
  ...
  if (general) {
    return {
      ...
      treatment: safeText(general.treatment || general.factor),
      factor: safeText(general.treatment || general.factor),
  ...
  return {
    ...
    treatment: safeText(first.treatment || first.factor),
    factor: safeText(first.treatment || first.factor),
...
function classifyComment(comment, tuning = {}) {
  ...
  if (safeText(categoryResolved?.treatment || categoryResolved?.factor)) {
    why.push(`category treatment: ${safeText(categoryResolved.treatment || categoryResolved.factor)}`);
  }
  ...
  return {
    ...
    category_treatment: safeText(categoryResolved?.treatment || categoryResolved?.factor),
    category_factor: safeText(categoryResolved?.treatment || categoryResolved?.factor),
