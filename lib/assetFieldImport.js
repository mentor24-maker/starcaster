...
const { runWebPageToTagImport } = require('./webPageTagImport');
const { isDuplicateRefusal } = require('./importDuplicateSkip');
const { normalizeMessagingTag, normalizeMessagingTagKey, parseMessagingTagLines } = require('./messagingTagNormalize');
const {
  FAMILY,
  getFormatSpec,
  listFormatSlugs,
...
const HANDLERS = {
  ...
  'web-page:tag': (options) => runWebPageToTagImport(options),
};
...
async function runGenericTextToTextImport(fromSlug, toSlug, options = {}) {
  ...
  const existingByContent = new Set();
  for (const row of existingRes.data || []) {
    const raw = toSpec.isLongform ? row.content : row[toSpec.contentField];
    const text = toSpec.slug === 'tag'
      ? normalizeMessagingTagKey(raw)
  ...
  // A create refused because the row already exists is this importer's job
  // already done, not a failure. Without it a forced re-run — `force` exists to
  // bypass `existingByContent`, so it asks for every row again — reported a 500
  // and `ok: false` even when it had created new rows. lib/importDuplicateSkip.js.
  const record = (result, dup, sourceId) => {
    if (result.ok) { created += 1; existingByContent.add(dup); return; }
    if (isDuplicateRefusal(result)) { skipped += 1; existingByContent.add(dup); return; }
  ...
  for (const candidate of candidates) {
    ...
    if (toSpec.slug === 'hashtag') {
      ...
      for (const tag of tags) {
        ...
        record(result, dup, sourceId);
      }
    } else if (toSpec.slug === 'tag') {
      const tags = parseMessagingTagLines(transformed.content);
      for (const tag of tags) {
        const dup = normalizeMessagingTagKey(tag);
        if (!dup) continue;
        if (!force && existingByContent.has(dup)) continue;
        const result = await toStore.create(buildCreatePayload(toSpec, tag, { topic, category }), scope);
        record(result, dup, sourceId);
  ...
  return {
    ok: errors.length === 0,
    status: errors.length ? 500 : 200,
    data: { ...resultData, transformErrors, created, skipped, errors },
    error: errors.length ? errors[0].error : '',
  };
}
