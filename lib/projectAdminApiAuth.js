...
const PROJECT_ADMIN_SESSION_DENY_PREFIXES = Object.freeze([
  ...
  '/api/assets/generate',
  // Reading a schedule flyer with Claude bills Alphire's model account per
  // upload. Dane decided 2026-09-12 (task 86bbztj0e) it is his tool, not a
  // tenant's: the Event Manager shows the button only to a platform login.
  '/api/event-harvest',
...
function isPublicTenantContentReadRoute(pathname, method, req) {
  ...
  /*
   * EVENT CATEGORIES — the calendar's venue names and colours (task 86bbzt25g).
   * The list only: a visitor's calendar draws its legend from it, and every
   * name and colour in it is painted on the page anyway. routes/eventCategories.js
   * strips it to id/name/colour/order for a caller with no session. Creating,
   * renaming, recolouring and deleting still need one, and so does a single
   * category by id, which no public module asks for.
   */
  if (pathname === '/api/event-categories') return true;
  /*
   * BLOG RELATIONS — one post's hand-picked related list, and nothing more.
   *
   * The blog-related-posts module renders on a PUBLISHED tenant page, where
   * the visitor has no login. Without this opening the module's hand-picked
   * mode 401s for every visitor, so the Relate Checked button in the blog
   * links manager would write rows that nothing on a live site can ever read.
   *
   * Narrow on purpose, matching the blog and events shape above: only the
   * single-post form. The unfiltered list — every relation in the project —
   * still needs a session, so an unauthenticated caller cannot enumerate the
   * whole graph by leaving the filter off.
   */
  if (pathname === '/api/blog/relations') {
    return Boolean(String(readSearchParams(req).get('postId') || '').trim());
  }
