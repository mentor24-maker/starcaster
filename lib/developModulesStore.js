...
function ensureFile() {
  ...
  if (!fs.existsSync(STORE_FILE)) {
    fs.writeFileSync(STORE_FILE, JSON.stringify({ modules: [] }, null, 2), { mode: 0o600 });
...
function sanitize(input) {
  ...
  return {
    ...
    settings: {
      text: safeText(settings.text),
      headingLevel: safeText(settings.headingLevel || settings.heading_level) || 'H1',
      textColor: safeText(settings.textColor || settings.text_color) || '#173c61',
    },
...
function inputToRow(input) {
  ...
  return {
    ...
    settings: {
      text: safeText(module.settings?.text),
      headingLevel: safeText(module.settings?.headingLevel) || 'H1',
      textColor: safeText(module.settings?.textColor) || '#173c61',
    },
  };
}
