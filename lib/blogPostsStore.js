...
const { resolveUniqueSlug } = require('./slugify');
...
/**
 * Is the Supabase table there?
 *
 * The probed COLUMN matters. `blog_post_categories` is a pure join table --
 * its primary key is (post_id, category_id) and it has no `id` column at all
 * -- so probing `select=id` there came back "column ... does not exist",
 * which isMissingTable() reads as the TABLE being absent. The store then fell
 * through to the local JSON file on every single call, for reads and writes
 * alike, and said nothing.
 *
 * That is not a cosmetic miss. On Vercel the filesystem is read-only, so the
 * fallback write vanishes (landmine 6): assigning a category to a blog post
 * appeared to succeed and never persisted, and filtering posts by category
 * returned nothing. Each table now names a column it actually has.
 */
const PROBE_COLUMN = { posts: 'id', join: 'post_id' };
...
async function supportsSupabase(which = 'posts') {
  ...
  const column = PROBE_COLUMN[which] || 'id';
  const probe = await sbQuery({ table, query: `select=${column}&limit=1` });
...
/**
 * The address this post will live at, normalized and unique in its project.
 * Derived from the title when none was given — the Create Post form has always
 * said "auto-generated if blank" and the store never did it, which left five
 * of Delray's published posts with no address at all (86bbu23n7).
 */
async function postSlugFor(requested, title, scope, excludeId = '') {
  return resolveUniqueSlug(
    requested,
    title,
    (candidate) => getPostBySlug(candidate, scope),
    excludeId
  );
}
...
async function createPost(input, scope = null) {
  ...
  const post = sanitize({
    ...
    slug:                 await postSlugFor(input.slug, input.title, scope),
    status,
  ...
  if (await supportsSupabase()) {
    ...
    if (existResult.ok) {
      ...
      const merged = sanitize({
        ...
        status: newStatus,
        publishedAt,
        // Normalized on every write, and re-derived from the title when the
        // field is cleared — so an address is never left blank or raw.
        slug: await postSlugFor(
