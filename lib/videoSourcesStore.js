...
function isDuplicateHashError(res) {
  ...
  const constraint = String(res?.raw?.constraint || res?.raw?.details || '').toLowerCase();
  ...
  // The three signals used to be OR'd together, so a bare `duplicate key`
  // matched on its own and any future unique violation on this table would be
  // reported as "already in the catalog for this project (same content hash)"
  // — a message about a column the collision never touched. Only the primary
  // key can reach it today (a gen_random_uuid collision, so effectively never),
  // which is why this is a latent defect rather than a live one; slice 2/8 adds
  // the second index that makes it live. When the database refuses for a reason
  // this cannot name, createSource returns the refusal as itself.
  return message.includes(CONTENT_HASH_INDEX) || constraint.includes(CONTENT_HASH_INDEX);
}
