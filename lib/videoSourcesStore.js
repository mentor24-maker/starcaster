...
const PROBE_TEXT_FIELDS = [
  ...
  ['driveMd5', 'drive_md5', 200],
];

/** The probe columns a caller may write, camelCase, in one list. */
const PROBE_FIELD_NAMES = [
  ...PROBE_NUMBER_FIELDS.map(([key]) => key),
  ...PROBE_INTEGER_FIELDS.map(([key]) => key),
  ...PROBE_TEXT_FIELDS.map(([key]) => key),
];

/**
 * Every field each entry point accepts; anything else is refused rather than
 * ignored (see unknownKeyError). `sessionId` is absent from update on purpose —
 * moving a file to another session is not a patch, and accepting the key only
 * to drop it is the failure this whole list exists to close.
 */
const SOURCE_CREATE_FIELDS = [
  'sessionId', 'layerRole', 'state', 'recordedAt', 'syncOffsetMs', ...PROBE_FIELD_NAMES,
];
const SOURCE_UPDATE_FIELDS = [
  'layerRole', 'state', 'recordedAt', 'syncOffsetMs', ...PROBE_FIELD_NAMES,
];
 ...
 *
 * OVER-LENGTH IS REFUSED, NOT TRIMMED. This function was written to stop a
 * quiet wrong value and then did the quiet wrong thing itself for text past
 * the limit: it sliced and reported success. Review reproduced the same
 * manufactured collision the paragraph above is about, this time without any
 * junk input at all —
 *
 *     createSource({ contentHash: <203 chars> })       -> ok / 201, stored 200
 *     createSource(<a DIFFERENT file, same first 200>) -> 409 "already in the
 *                                                          catalog"
 *
 * Two files with nothing in common and the caller is told one is a copy of the
 * other. A shortened `localPath` or `proxyPath` is worse still: it is a file
 * reference that points nowhere, and slices 4/8-6/8 are the code that writes
 * those. Every probe text is checked the same way rather than only the three
 * that can be shown to bite today — a limit that silently applies to some
 * columns and not others is the next round of this.
 *
 * The limit is measured AFTER trimming, because trailing whitespace is not
 * part of the value and refusing a 200-character hash with a stray newline
 * would be its own untrue answer.
 *
  ...
  if (typeof value === 'string' || (typeof value === 'number' && Number.isFinite(value))) {
    ...
    if (text.length > limit) {
      return {
        ok: false,
        status: 400,
        error: `${field} must be ${limit} characters or fewer (got ${text.length})`,
      };
    }
...
function rowToSource(row) {
  ...
  return {
    ...
    driveMd5: safeText(row.drive_md5, probeLimit('driveMd5')),
...
/**
 * The unique index that makes a content hash unique per project. Named here so
 * the matcher below can ask WHICH constraint collided; declared once because
 * a copy of this string in a test that drifts from the SQL is a test that
 * passes against a message the code will never see.
 * docs/SQL/video_studio_setup.sql.
 */
const CONTENT_HASH_INDEX = 'idx_video_sources_project_content_hash';
 ...
 *
 * The index name is read from the MESSAGE only, because that is the one place
 * it arrives. Measured 2026-09-13 by driving a real duplicate through
 * createSource against local Postgres and dumping the error body sbQuery
 * returns as `raw` — PostgREST sends exactly four keys:
 *
 *   code    '23505'
 *   details 'Key (project_id, content_hash)=(…, …) already exists.'
 *   hint    null
 *   message 'duplicate key value violates unique constraint "idx_video_sources_project_content_hash"'
 *
 * No `constraint` key, and `details` carries the key COLUMNS rather than the
 * index name. An earlier version read `raw.constraint || raw.details` as a
 * second signal; both are unreachable, so that branch could never run and its
 * test asserted a shape the database does not produce (review round 2).
 */
function isDuplicateHashError(res) {
  const message = String(res?.error || '').toLowerCase();
  const uniqueViolation = (
    String(res?.raw?.code || '') === '23505' ||
    message.includes('23505') ||
    message.includes('duplicate key')
  );
  if (!uniqueViolation) return false;
  // ...and it must be THE content-hash index, not merely SOME unique index.
  // The signals used to be OR'd together, so a bare `duplicate key` matched on
  // its own and any future unique violation on this table would be reported as
  // "already in the catalog for this project (same content hash)" — a message
  // about a column the collision never touched. Only the primary key can reach
  // it today (a gen_random_uuid collision, so effectively never), which is why
  // this is a latent defect rather than a live one; slice 2/8 adds the second
  // index that makes it live. When the database refuses for a reason this
  // cannot name, createSource returns the refusal as itself.
  return message.includes(CONTENT_HASH_INDEX);
}
...
async function createSource(input, scope = null) {
  const unknown = unknownKeyError(input, SOURCE_CREATE_FIELDS);
  if (unknown) return unknown;

  const suppliedSession = readField(input, 'sessionId');
  if (!suppliedSession.ok) return suppliedSession;
  const sessionId = safeText(suppliedSession.value, 120);
  ...
  // Whether the caller SUPPLIED a role is `.present`, never truthiness. The
  // default used to be substituted with `suppliedRole.value || 'reference'`,
  // which reads `false` and `0` as "not supplied" and swaps in the default
  // BEFORE the value is ever validated — so `createSource({ layerRole: false })`
  // answered ok/201 and stored 'reference', a value the caller never asked for,
  // under a success. Every other junk value (`'hero'`, `{}`, `0`) was already a
  // 400, and `updateSource` refused `false` correctly because it had always
  // used `.present`. Only the create path coerced, and only for the one input
  // that is both falsy and non-blank.
  const suppliedRole = readField(input, 'layerRole');
  if (!suppliedRole.ok) return suppliedRole;
  let layerRole = 'reference';
  if (suppliedRole.present) {
    layerRole = normalizeLayerRole(suppliedRole.value);
    if (!layerRole) {
      return { ok: false, status: 400, error: `layerRole must be one of ${LAYER_ROLES.join(', ')}` };
    }
  }

  // Same fix, same reason: `safeText(false, 40)` is '' and `'' || 'new'` is
  // 'new', so `createSource({ state: false })` stored 'new' under a 201 while
  // `{ state: 0 }` was correctly a 400. Presence decides the default now, so a
  // supplied value is always validated and an absent one always defaults.
  const suppliedState = readField(input, 'state');
  if (!suppliedState.ok) return suppliedState;
  let state = 'new';
  if (suppliedState.present) {
    state = safeText(suppliedState.value, 40);
  ...
  for (const [key, column, reader] of [
    ...
    const supplied = readField(input, key);
    if (!supplied.ok) return supplied;
    const parsed = reader(supplied.value, key);
    if (!parsed.ok) return parsed;
  ...
  for (const [key, column, limit] of PROBE_TEXT_FIELDS) {
    const supplied = readField(input, key);
    if (!supplied.ok) return supplied;
    const parsed = textOrError(supplied.value, key, limit);
    if (!parsed.ok) return parsed;
...
/**
 * "Is this file already in the catalog?" — the two ways of asking, for ingest.
 *
 * Both answer `{ ok: true, data: <source|null> }`. **A miss is `data: null`,
 * never a 404**, and that is the whole point of these two rather than a
 * getSourceById-shaped read: the caller's question is not "fetch this row", it
 * is "have we got this already", and both answers are ordinary. A 404 would
 * make the normal case travel the error path, where the one failure that
 * matters — the database being unreachable — becomes indistinguishable from
 * "no, go ahead and download 3.57 GB".
 *
 * Scoped, so "already in the catalog" means already in THIS project's catalog.
 * The dedupe index is per project for the same reason (see the SQL).
 */
async function findSourceByDriveFileId(driveFileId, scope = null) {
  const id = safeText(driveFileId, probeLimit('driveFileId'));
  if (!id) return { ok: false, status: 400, error: 'driveFileId is required' };
  const query = await scopedListQuery(
    table(),
    `drive_file_id=eq.${encodeURIComponent(id)}&select=*&order=created_at.asc&limit=1`,
    scope
  );
  const res = await sbQuery({ method: 'GET', table: table(), query });
  if (!res.ok) return res;
  const found = Array.isArray(res.data) ? res.data[0] : res.data;
  return { ok: true, status: 200, data: found ? rowToSource(found) : null };
}

/** The other half: the same BYTES, whatever Drive file they arrived as. */
async function findSourceByContentHash(contentHash, scope = null) {
  const hash = safeText(contentHash, probeLimit('contentHash'));
  if (!hash) return { ok: false, status: 400, error: 'contentHash is required' };
  const query = await scopedListQuery(
    table(),
    `content_hash=eq.${encodeURIComponent(hash)}&select=*&order=created_at.asc&limit=1`,
    scope
  );
  const res = await sbQuery({ method: 'GET', table: table(), query });
  if (!res.ok) return res;
...
async function updateSource(id, patch, scope = null) {
  ...
  const unknown = unknownKeyError(patch, SOURCE_UPDATE_FIELDS);
  if (unknown) return unknown;

  const next = {};
  const suppliedRole = readField(patch, 'layerRole');
  if (!suppliedRole.ok) return suppliedRole;
  if (suppliedRole.present) {
    const layerRole = normalizeLayerRole(suppliedRole.value);
    if (!layerRole) {
      return { ok: false, status: 400, error: `layerRole must be one of ${LAYER_ROLES.join(', ')}` };
    }
  ...
  const suppliedOffset = readField(patch, 'syncOffsetMs');
  if (!suppliedOffset.ok) return suppliedOffset;
  if (suppliedOffset.present) {
    ...
    const offset = integerOrError(suppliedOffset.value, 'syncOffsetMs');
  ...
  // Both casings, the same as the create path — `duration_s` used to be read
  // straight past here while `durationS` applied, so a probe writing its output
  // under the COLUMN names got a 200 and stored nothing. See readField.
  for (const [fields, reader] of [
    [PROBE_NUMBER_FIELDS, numberOrError],
    [PROBE_INTEGER_FIELDS, integerOrError],
    [PROBE_TEXT_FIELDS, textOrError],
  ]) {
    for (const [key, column, limit] of fields) {
      const supplied = readField(patch, key);
      if (!supplied.ok) return supplied;
      if (!supplied.present) continue;
      const parsed = reader(supplied.value, key, limit);
      if (!parsed.ok) return parsed;
...
module.exports = {
  ...
  // The index name the matcher above requires, exported so its test asserts
  // against the one declaration rather than a second copy of the string.
  CONTENT_HASH_INDEX,
  ...
  // "Have we got this already?" — a miss is data:null, not a 404. See the note
  // above; ingest (4/8) is the caller, and it asks before downloading 3.57 GB.
  findSourceByDriveFileId,
