...
async function attachThumbnailForDriveAsset({
  ...
  let updatedRow = Array.isArray(updated.data) ? updated.data[0] : updated.data;

  // The original is already downloaded here, so the scaled copies cost one
  // resize rather than a second trip to Drive.
