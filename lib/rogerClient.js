...
async function queryOpenAI(systemPrompt, messagesOrPrompt, extraOptions = {}) {
  ...
  if (Array.isArray(messagesOrPrompt)) {
    messagesOrPrompt.forEach(msg => {
      // NOTE: OpenAI Vision requires a different format, but for text, this standard is fine. 
      // If we need image support via Anthropic/OpenAI in the future, we will map inlineData explicitly.
      messages.push({
...
async function consultRoger(messagesOrPrompt, extraOptions = {}) {
  ...
  try {
    ...
    console.warn(`[ROGER HYBRID INFERENCE] Gemini Failed (Status: ${geminiRes.status}). Error: ${geminiRes.error}. Attempting OpenAI Fallback...`);
    ...
    return { 
      ok: false, 
      error: `Primary (Gemini) failed: ${geminiRes.error}. Fallback (OpenAI) failed: ${openAIRes.error}` 
    };
