...
async function queryAnthropic(systemPrompt, messagesOrPrompt, extraOptions = {}) {
  ...
  if (!res.ok) {
    return { ok: false, error: data?.error?.message || "Anthropic API error", status: res.status };
  }
...
async function consultRoger(messagesOrPrompt, extraOptions = {}) {
  ...
  try {
    ...
    return { 
      ok: false, 
      error: `Primary (Gemini) failed: ${geminiRes.error}. Fallback (Claude) failed: ${claudeRes.error}` 
    };
