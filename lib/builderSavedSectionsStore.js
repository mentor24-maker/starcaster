...
const STORE_FILE = path.join(__dirname, '..', 'data', 'builder_saved_sections.json');
const SUPPORT_CACHE = new Map();

function t() {
  return tableConfig().builderSavedSections;
}
...
async function createSavedSection(input, scope = null) {
  ...
  if (writeResult && !writeResult.ok) return { ok: false, status: 500, error: 'Could not persist saved section' };
...
async function updateSavedSection(id, input, scope = null) {
  ...
  const updated = {
    ...
    section: newSection,
    isPrivate: input?.isPrivate !== undefined ? input.isPrivate === true : existing.isPrivate,
...
async function getSavedSection(id, scope = null) {
  const sectionId = safeText(id, 120);
  if (!sectionId) return { ok: false, status: 400, error: 'id is required' };

  if (await supportsSupabase()) {
    const query = await scopedIdQuery(t(), `id=eq.${encodeURIComponent(sectionId)}&select=*&limit=1`, scope);
    const res = await sbQuery({ method: 'GET', table: t(), query });
    if (res.ok) {
      const found = Array.isArray(res.data) ? res.data[0] : res.data;
      if (!found) return { ok: false, status: 404, error: 'Saved section not found' };
      return { ok: true, status: 200, data: sanitizeRecord(found) };
    }
    if (!isMissingTableError(res.error)) return { ok: false, status: res.status || 500, error: res.error };
  }

  const store = readStore();
  const match = store.savedSections.find((item) => {
    const s = sanitizeRecord(item);
    return safeText(s?.id) === sectionId && matchesScope(item, scope);
  });
  if (!match) return { ok: false, status: 404, error: 'Saved section not found' };
  return { ok: true, status: 200, data: sanitizeRecord(match) };
}
