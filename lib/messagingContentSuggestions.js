...
function anthropicModel() {
  return clean(process.env.ANTHROPIC_MODEL) || 'claude-sonnet-5';
}

// Fallback ladder, most capable first. Four entries until 2026-08-16, when three
// of them turned out to name models Anthropic retired in Feb and Apr 2026 — the
// two live tiers collapse to one id each. Ids carry no date suffix by design;
// 'claude-sonnet-5' is the whole name.
...
const FORMAT_SCHEMAS = {
  ...
  Posts: { kind: 'short', fieldLabel: 'Post', count: 10, usesHashtags: true, usesUrl: true },
...
async function loadProjectTraining(scope) {
  const training = await getTrainingPromptConfig(projectTrainingKey(scope)).catch(() => ({ context: '', guidelines: '' }));
  return constrainTrainingToProject(training || {}, scope);
}
...
function promptForShort(input, schema, training, scope) {
  return [
...
function promptForLong(input, training, scope) {
  return [
    projectBoundaryText(scope),
    '',
    input.feedback
...
async function prepareMessagingContentPrompt(input, scope = null) {
  ...
  const training = await loadProjectTraining(scope);
  const prompt_text = schema.kind === 'long'
    ? promptForLong(normalized, training || {}, scope)
    : promptForShort(normalized, schema, training || {}, scope);
  return {
    ok: true,
    status: 200,
    data: {
...
function topicSuggestionPrompt(input, training, website, scope) {
...
async function generateTopicSuggestionsWithAnthropic(input, training, website, scope) {
  ...
  try {
    res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1800,
        messages: [{ role: 'user', content: topicSuggestionPrompt(input, training, website, scope) }],
        system: 'You are an information architect generating topic taxonomies from project-specific website research. Output strict JSON only.',
      }),
  ...
  await recordAiUsage({
    provider: 'anthropic',
    model: modelRes.model,
    feature: 'messaging_topic_suggestions',
    body,
    scope,
  });
...
async function generateWithAnthropic(input, schema, training, scope) {
  const cfg = getProviderValues('anthropic');
  const apiKey = clean(cfg.api_key || process.env.ANTHROPIC_API_KEY);
  if (!apiKey) return { ok: false, status: 400, error: 'Anthropic API key not configured.' };
  const modelRes = await resolveAnthropicModel(apiKey);
  ...
  const prompt = schema.kind === 'long' ? promptForLong(input, training, scope) : promptForShort(input, schema, training, scope);
  let res;
  try {
    res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        system: schema.kind === 'long'
          ? 'You write publication-ready marketing and editorial drafts for exactly one active project. Output strict JSON only.'
          : 'You write sharp marketing content options for exactly one active project. Output strict JSON only.',
      }),
      signal: AbortSignal.timeout(35000),
    });
  } catch (err) {
  ...
  await recordAiUsage({
    provider: 'anthropic',
    model: modelRes.model,
    feature: schema.kind === 'long' ? 'messaging_content_long' : 'messaging_content_short',
    body,
    scope,
  });
...
async function generateMessagingContentSuggestions(input, scope = null) {
  const normalized = buildInput(input);
  const schema = schemaForFormat(normalized.format);
  if (!schema) return { ok: false, status: 400, error: 'A supported format is required.' };
  const training = await loadProjectTraining(scope);
  const generated = await generateWithAnthropic(normalized, schema, training || {}, scope);
...
async function generateMessagingTopicSuggestions(input, scope = null) {
  ...
  if (!website || !website.source_url) {
    return { ok: false, status: 400, error: 'No acquired website data is available yet. Run Acquire: Web first.' };
  }
  const training = await loadProjectTraining(scope);
  const generated = await generateTopicSuggestionsWithAnthropic(input || {}, training || {}, website, scope);
  if (!generated.ok) return generated;
  return {
    ok: true,
    status: 200,
