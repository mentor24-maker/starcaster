  ...
  const MODULE_TYPE_DEFINITIONS = [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'navDirection', label: 'Layout', control: 'select', options: [{ value: 'horizontal', label: 'Horizontal' }, { value: 'vertical', label: 'Vertical' }] },
        { key: 'navItems', label: 'Menu Structure', control: 'nav-menu-editor' },
        { control: 'section-header', label: 'Appearance' },
        { key: 'navItemSizing', label: 'Link Sizing', control: 'select', options: [{ value: 'auto', label: 'Auto (fit content)' }, { value: 'equal', label: 'Equal width' }, { value: 'custom', label: 'Custom (set width per link)' }] },
        { key: 'navAlignment', label: 'Alignment', control: 'select', options: [{ value: 'left', label: 'Left' }, { value: 'center', label: 'Center' }, { value: 'right', label: 'Right' }] },
        { key: 'navLevels', label: 'Submenu Levels', control: 'select', options: [{ value: '1', label: '1' }, { value: '2', label: '2' }, { value: '3', label: '3' }] },
        { key: 'navMarginV', label: 'Vertical Margin', control: 'number', min: 0, max: 80, step: 1 },
      ...
      if (field.control === 'nav-menu-editor') {
        ...
        const updateNavState = () => {
          ...
          preview.innerHTML = App.builderNavMenu?.buildNavigationModuleMarkup?.({
            ...
            navDirection: safeText(settings?.navDirection || settings?.variant) || 'horizontal',
            navItems: control.value,
            navItemSizing: settings?.navItemSizing,
            navAlignment: settings?.navAlignment,
            navLevels: settings?.navLevels,
            navMarginV: settings?.navMarginV,
