---
name: loop-review
description: Pick the next "review" task from the Starcaster "Loop Queue" ClickUp list, independently verify its pull request (build gates + code review + a real test pass), then either mark it "approved" and ping the operator to merge, or send it back to "todo" with notes. Designed to be run on a timer with `/loop 30m loop-review`. Never merges on its own.
---
...
1. **Claim the next task.** Find the oldest task with status `review` in the
...
4. **Decide.**
   - **Pass** → set the task to `approved`. Post to the operator (the ClickUp
     bus channel and/or a push notification) a compact message: task name, PR
     link, Vercel preview link, the "How to test" steps, and the line
   ...
   - **Fail** → set the task back to `todo` (or `changes-requested` if that
     status exists) and add a ClickUp comment listing precisely what to fix and
     why. The `loop-build` skill will pick it up again. Do not fix it yourself
     inside the review step — keep build and review separate so the check stays
     honest.
...
- **Never merge autonomously.** Approval ≠ merge. The operator merges (via CC
  on explicit command). This is the human checkpoint that keeps a bad change
  off the auto-deploying `main`.
- **Independence is the whole point.** Re-run everything; don't rubber-stamp.
- If the PR has merge conflicts with main, send it back to `todo` with a note
