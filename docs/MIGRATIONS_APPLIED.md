...
|---|---|---|
| 2026-09-12 | `docs/SQL/events_programs_setup.sql` | **Applied** by Dane in the Supabase SQL editor, confirmed by a read-only `npm run schema:check` the same day (the four `events` columns, `event_categories` with its key, index and trigger) before PR #669 merged. Snapshot not re-taken — see the standing drift note above. Repeating events, instructors and venue categories, for Delray Beach Tennis's weekly program guide (tickets 86bbzt259, 86bbzt25g, 86bbzt25j — one file so it is applied once). Adds four columns to `events`, each with a default or nullable: `recurrence` jsonb (null = one-off), `recurrence_overrides` jsonb not null default `[]`, `instructor` text default `''`, `category_id` text default `''`; and one new table `event_categories` with BOTH tenant columns (landmine 12), an updated_at trigger, and RLS on with no policies. Additive only: no existing row is rewritten, and every existing event reads back as a non-repeating event with no instructor or category. Idempotent — `if not exists` throughout, verified by running it twice against the local database (10 existing events untouched). **Must be applied before PR for 86bbzt259 merges**: the store writes the new columns on every save, and PostgREST refuses a column it does not know. |
| 2026-09-07 | `docs/SQL/blog_tag_runs_setup.sql` | **Applied** by Dane in the Supabase SQL editor at 1:13pm Mountain ("Success. No rows returned"), confirmed read-only at 1:28pm (table present, `blog_tag_runs_pkey`, `idx_blog_tag_runs_project_run`). Creates `blog_tag_runs` — the Auto-tag extension's run history: one row per post a run touched, holding ONLY the tags the run added, so Undo removes exactly those (tickets 86bbw4dcd / 86bbw4dch / 86bbw4dcp, PRs #649–#651). One new table and nothing else; BOTH tenant columns (landmine 12); RLS on with no policies, matching the lockdown. Idempotent — `if not exists` throughout, verified by applying it twice to the local database and by a rehearsal that ran and undid a full pass over Delray's 56 posts locally. `schema:check` shows the table's constraint and index as drift because the committed record carries only the CREATE TABLE block; leave the snapshot alone until the standing drift above is dealt with. |
