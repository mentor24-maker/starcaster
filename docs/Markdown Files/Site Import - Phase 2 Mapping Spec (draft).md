...
Status: **DRAFT — awaiting operator ratification.** Prerequisites:
...
| 4 | **Slugs: original path, suffixed on collision; home lands as `imported-home`.** Existing pages always win collisions; imported pages never claim the live home slug. | The operator promotes slugs deliberately at review time. An import must never shadow a live page. |
...
| 6 | **Nav is a separate, explicitly-flagged step** (`--nav`), never part of a default run. It writes the header `NavTree` into the Header saved-section **master's** `navigation` module (`settings.navItems`), creating the master if the project has none. Without `--nav`, nav links are accounted as `nav-consumed`, not mapped. | Invariant 1 — plus canonical propagation fans the master out to every linked page (Phase 0 §8), so touching it deserves its own deliberate flag. |
| 7 | **Assets: promote only referenced assets** into the project's `assets` library — one row per asset actually used by a mapped module, `location` = the existing `SiteImport/<jobId>/assets/...` Blob URL, and the **same URL written into the page JSON** so "Used In" matches (Phase 0 §6). Unreferenced downloads stay in the job namespace. | 463 rows of un-reviewed downloads would pollute the tenant's picker; referenced-only keeps the library honest. Caveat recorded: job-namespace URLs mean the job's Blob folder must outlive the import. |
...
A re-run **updates** those pages in place (replacing only sections marked
as import-owned — every mapped section carries
`title: "Imported: …"` and an `importSourceId` marker in its first
module's settings) and never duplicates. Deleting the drafts and clearing
the checkpoint resets cleanly.
...
```json
{
  "elements": { "total": 4507, "mapped": 4380, "placeholder": 96,
                 "navConsumed": 28, "skipped": 3 },
  "skipped": [ { "sourceId": "...", "class": "...", "reason": "..." } ],
  "pages": [ { "path": "/", "slug": "imported-home", "modules": 41,
               "placeholders": 4, "builderPageId": null } ],
  "assets": { "promoted": 214, "leftInNamespace": 249 }
}
```

Rule: `total === mapped + placeholder + navConsumed + skipped`, and
`skipped` entries must each carry a reason. A dry run whose numbers don't
reconcile refuses to `--apply`. The UI's job page shows the report next
to the Phase 1 coverage table.
...
5. `--nav` writes the Delray header tree into the scratch project's
   Header master and every imported page instantiates it.
6. This doc updated with deviations discovered during implementation.
