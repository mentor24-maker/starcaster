...
| 2026-08-16 | `fix_dev_sessions_project_id_type.sql` | | | ✓ | `dev_sessions.project_id` was declared BIGINT while every project id on the platform is TEXT, so Postgres refused any filter on it (`invalid input syntax for type bigint`). That broke **deleting any project at all** — the purge walks every project-scoped table and died on this one — and meant Ask Roger sessions could never be scoped to a project. All 31 rows were NULL, so nothing was converted. Operator ran it in the Supabase SQL editor; column verified as `text` afterwards. Ships with PR #284, which also stops one mistyped column aborting the whole delete. |
| 2026-08-18 | `enable_rls_lockdown.sql` | | | ✓ | RLS enabled on the 65 public tables that lacked it (all 136 now locked; no policies — the app uses the service key, which bypasses RLS). Closes both critical findings in Supabase's weekly security scan (`rls_disabled_in_public`, `sensitive_columns_exposed`). Operator ran it in the Supabase SQL editor; verified by re-snapshot (`docs/schema/production.sql`, 136/136). **Not applied locally** — the next `db:refresh` brings the state down automatically. Ships with PR #334. |
| | | | | | |
